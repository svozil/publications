...
Can a finite collection of sharp tests on a three-level quantum system
require complex coordinates?  Each individual measurement basis can be
chosen real, and each individual pure state can be made real by a suitable
change of basis.  The physical question concerns a \emph{common} description:
whether all the elementary alternatives in an overlapping family of
measurements can be represented by real rays in the same three-dimensional
space.  A vertex shared by several contexts must denote the same test in
each occurrence.  The hypergraphs studied here make such a simultaneous
real description impossible, even though they admit exact complex
realizations.

In this interpretation, a vertex represents a rank-one projector, and a
three-vertex context represents a complete projective measurement.  Its
outcomes are mutually exclusive and exhaustive.  The distinction between
$\C^3$ and $\R^3$ therefore concerns the organization of a finite family of
elementary experimental alternatives, rather than the appearance of
imaginary entries in a selected coordinate system.  It already arises at
the level of states and measurements, without specifying dynamics.

The first construction also has a preparation interpretation.  Its
overlapping contexts force
\[
 P_{a_1}+P_{a_2}=P_{b_1}+P_{b_2},
\]
where the four rays are distinct and the members of each pair are
nonorthogonal.  Preparing either pair with equal probabilities consequently
produces the same density operator when the preparation label is discarded.
The two pairs are \emph{pseudocontexts}: their probability sums agree
because of the surrounding context equations, although neither pair is a
complete family of mutually exclusive outcomes
\cite{2026-Navara-Svozil-pseudocontexts}.  Complex relative phases permit
these different two-state resolutions of one mixed state; the corresponding
real rank-one resolutions are rigid.  In the second construction the inner
pairs become orthogonal, removing this local rigidity obstruction, but the
perimeter still prevents a common real realization.

These observations motivate two complementary physical readings: the
freedom to resolve an unchanged mixed state into different elementary
preparations, and the phase compatibility required to combine overlapping
measurements into a single model.  Section~\ref{sec:physical} develops these
readings, distinguishes preservation of an orthogonality diagram from
preservation of full measurement probabilities, and identifies an
operational consequence of the displayed coordinates.  The mathematical
results concern faithful representations by rays; experimental applications
must retain the relevant assumptions about dimension, projector rank, and
the identification of repeated tests.

\subsection{Mathematical setting and relation to previous work}
...
\section{Physical interpretation and operational prospects}
\label{sec:physical}

\subsection{Pseudocontexts as equivalent preparations and measurement effects}

The covering identity~\eqref{eq:covering-identity} has two operational
interpretations.  First, for every density operator $\rho$,
\[
 \operatorname{tr}\rho(P_{a_1}+P_{a_2})
 =\operatorname{tr}\rho(P_{b_1}+P_{b_2}).
\]
This equality is state independent, whereas the common value generally
depends on $\rho$ and need not equal one.  The two nonorthogonal pairs
are therefore not two-outcome projective measurements.  More generally,
the same complementary-cover argument applies to any single assignment
$p(v)$ satisfying $\sum_{v\in E}p(v)=1$ for every context, independently of
the Born rule~\cite{2026-Navara-Svozil-pseudocontexts}.  The equality alone
does not distinguish real and complex quantum theory; the field dependence
enters through its faithful realization by rank-one projectors.

Second, the normalized common operator is a density operator,
\[
 \rho_*:=\tfrac12(P_{a_1}+P_{a_2})
        =\tfrac12(P_{b_1}+P_{b_2}).
\]
Thus two different equal-probability preparations become indistinguishable
by every subsequent measurement on the prepared system when their labels
are forgotten.  For the
coordinates of Sec.~\ref{sec:h20}, this freedom can be displayed explicitly:
\[
 \psi_\pm(\phi)=\frac{(\sqrt C,\ \pm e^{\ii\phi},\ 0)}{\sqrt{1+C}},
 \qquad
 \rho_* =\frac{\operatorname{diag}(C,1,0)}{1+C}.
\]
Every value of $\phi$ gives the same equal-weight mixture.  The inner pairs
in Eq.~\eqref{eq:h20-inner-vectors} correspond to $\phi=0$ and
$e^{\ii\phi}=\eta$.
This phase family describes the decompositions of the inner operator;
it does not assert that every phase extends to a faithful realization of
the entire octagon.

On the Bloch sphere of the common two-dimensional support, an equal-weight
decomposition is a chord with midpoint $\rho_*$.  Since $0<C<1$, that
midpoint is not the center.  The complex sphere admits a circle of chord
directions with this midpoint.  In a fixed real-state disk containing
$\rho_*$, only one unordered pair remains.  Lemma~\ref{lem:real-rigidity}
is the algebraic version of this restriction.  The freedom detected by
$\Htwenty$ is consequently more specific than the general nonuniqueness of
mixed-state ensembles: it requires distinct \emph{equal-weight two-state}
decompositions of a nonmaximally mixed state on its support.

There is also a measurement realization.  With
$S=P_{a_1}+P_{a_2}=P_{b_1}+P_{b_2}$, the two families
\[
 \{\tfrac12P_{a_1},\tfrac12P_{a_2},I-\tfrac12S\},\qquad
 \{\tfrac12P_{b_1},\tfrac12P_{b_2},I-\tfrac12S\}
\]
are positive-operator-valued measures: their effects are positive and sum
to the identity.  They provide different resolutions of the same
coarse-grained effect $S/2$ and have the same remaining outcome.

Finally, the ensemble interpretation connects to remote preparation
\cite{Hughston-Jozsa-Wootters-1993}.  The purification
\[
 |\Psi\rangle=
 \frac{\sqrt C\,|0\rangle|0\rangle+|1\rangle|1\rangle}{\sqrt{1+C}}
\]
has reduced state $\rho_*$ on the first system.  Measuring the second
system in the basis
$(|0\rangle\pm e^{-\ii\phi}|1\rangle)/\sqrt2$ prepares
$\psi_\pm(\phi)$, each with probability $1/2$.  This supplies a concrete
way to generate the two pseudocontext ensembles without changing the
unconditioned reduced state.  A steering or communication advantage from
the full hypergraph would require an additional protocol and a separate
performance bound.

\subsection{A common real structure and relative phase}

The physical content of the nonexistence theorems is invariant under a
common unitary transformation and independent choices of ray phases.
Equivalently, no faithful realization of either hypergraph in $\C^3$ admits
an antiunitary involution $K$, with $K^2=I$, that fixes every labelled
projector: $KP_vK^{-1}=P_v$ for all $v$.  Such an involution would define
a conjugation, and a basis of its fixed real subspace would make all the
projectors real.  Each invariant ray has a representative fixed by $K$,
so this would contradict the real three-dimensional no-go theorem.

For $\Htwentyone$, the two inner orthogonal pairs by themselves admit a
common real description.  Their equal mixtures are maximally mixed on
$c^\perp$, where real orthogonal decompositions are no longer unique.
The impossibility appears when these pairs must also fit the entire
perimeter.  This provides a global phase compatibility interpretation of
the closure obstruction.  It does not identify a dynamical geometric phase
around the drawn octagon: inner products of orthogonal rays vanish and
have no defined phase.

A phase-sensitive quantity can instead be formed from nonorthogonal rays.
For unit representatives, the Bargmann invariant
\[
 B(x,y,z):=\operatorname{tr}(P_xP_yP_z)
 =\langle x,y\rangle\langle y,z\rangle\langle z,x\rangle
\]
is invariant under ray rephasings and a common unitary transformation.
A nonreal value excludes a simultaneous real representation of those
projectors; such cyclic overlaps connect geometric phase to witnesses of
quantum imaginarity~\cite{MukundaEtAl2003Bargmann,
LiTan2025BargmannImaginarity}.  In the displayed $\Htwenty$ representation,
direct substitution yields
\[
 B(a_1,a_2,b_1)
 =\frac{(C-1)(C-\eta)(C+\bar\eta)}{(1+C)^3}
 =\frac{115+8\ii\sqrt{69}}{375}.
\]
The nonzero imaginary part makes the irreducible relative phase explicit.
This particular value is a property of the supplied coordinates, not a
claimed invariant value of every faithful representation of the hypergraph.
Complex conjugation preserves all orthogonalities and pairwise transition
probabilities while conjugating $B$.  The incidence structure therefore
does not select a handedness of phase, and no statement about physical
time-reversal violation follows.

\subsection{What a real simulation preserves}

Two kinds of realification have different operational meanings.  The
phase-adjusted construction in Sec.~\ref{sec:realification} assigns a
\emph{real rank-one} projector to each vertex and preserves exactly the
orthogonality relations.  It need not preserve nonzero transition
probabilities or the pseudocontext projector identity.  Each specified
triple is incomplete in the real six-dimensional space.

By contrast, write a complex operator as $A+\ii B$, with real matrices
$A,B$, and use the standard operator representation
\[
 \mathcal R(A+\ii B)=
 \begin{pmatrix}A&-B\\ B&A\end{pmatrix}.
\]
For a density operator and a projector, respectively, set
$\rho_{\R}=\mathcal R(\rho)/2$ and
$\Pi_v=\mathcal R(P_v)$.  Then
\[
 \operatorname{tr}_{\R}(\rho_{\R}\Pi_v)
 =\operatorname{tr}_{\C}(\rho P_v),\qquad
 \sum_{v\in E}\Pi_v=I_6.
\]
This construction preserves probabilities, context completeness, and the
pseudocontext identity, but each complex rank-one projector becomes a
\emph{real rank-two} projector.  The three outcomes remain a complete
projective measurement, with each outcome now a two-dimensional subspace
of the real model.  This distinction is consistent with the broader real
simulation constructions cited in the Introduction.

The rank change is essential if the nondegenerate two-projector identity
is retained.  The proof of Lemma~\ref{lem:real-rigidity} applies in any
ambient real dimension, because equality of the two positive sums confines
all four rays to the same rank-two support.  Enlarging dimension alone
therefore cannot realize that identity with four distinct real rank-one
projectors and nonorthogonal members within each pair.  Dimension three
is needed to \emph{derive} the identity from the complete contexts of
$\Htwenty$; the rigidity of the resulting identity is dimension independent.

\subsection{An operational consequence and experimental scope}

The displayed coordinates also give a compact transition-probability
consequence.  Regard a label $u$ as the pure preparation $P_u$, and a label
$v$ as the rank-one yes--no test $\{P_v,I-P_v\}$.  The response probability
is $Q(u,v)=\operatorname{tr}(P_uP_v)$, as in Eq.~\eqref{eq:Q}.  For the four
inner rays of $\Htwenty$,
\[
 \begin{aligned}
 Q(a_1,a_2)=Q(b_1,b_2)&=\frac{23}{75},\\
 Q(a_1,b_1)=Q(a_2,b_2)&=\frac{59}{75},\\
 Q(a_1,b_2)=Q(a_2,b_1)&=\frac{13}{25}.
 \end{aligned}
\]
These values follow directly from Eq.~\eqref{eq:h20-inner-vectors} and
$13C^2-49C+13=0$.  They cannot be reproduced by real unit vectors in any
dimension.  To see this, every three-ray subset has the sum of its three
squared overlaps equal to $121/75>1$.  For real inner products $x,y,z$,
positivity of the corresponding Gram determinant requires
$1-x^2-y^2-z^2+2xyz\ge0$, hence $xyz>0$.  Changing representative signs
can therefore make all six inner products positive.  In the order
$(a_1,a_2,b_1,b_2)$ the real Gram matrix would be
\[
 G=\begin{pmatrix}
 1&g&h&\ell\\ g&1&\ell&h\\
 h&\ell&1&g\\ \ell&h&g&1
...
where
\[
 g=\sqrt{\frac{23}{75}},\quad
 h=\sqrt{\frac{59}{75}},\quad
 \ell=\sqrt{\frac{13}{25}}.
\]
Its eigenvalue on $(1,1,-1,-1)$ is
\[
 1+g-h-\ell\simeq-0.05428<0.
\]
The sign is exact: $h^2+\ell^2=1+g^2$ and
$(h\ell)^2-g^2=64/625>0$ imply $h+\ell>1+g$.
This contradicts positivity of a Gram matrix.

The four-ray test uses the specific nonzero overlaps of the displayed
realization; the full hypergraph has the different role of forcing
incompatible real geometry from context incidences alone.  The finite
Gram-matrix gap provides a starting point for a quantitative noise
analysis, but a laboratory test must justify pure preparations, rank-one
tests, and the identification of equally labelled preparations and tests.
Allowing unrelated preparation and measurement vectors, or higher-rank
effects, changes the model being excluded.  Neither this test nor the
pseudocontext equality is, by itself, a device-independent or
Kochen--Specker contextuality test.

More broadly, the constructions suggest investigating the operational
cost of a real description: which distinctions between elementary
alternatives, which projector ranks, and which completeness requirements
must change when complex phases are removed?  The ensemble and overlap
consequences above identify concrete quantities for this investigation.
Applications to steering, communication, or a quantitative resource theory
remain prospects rather than consequences of the representation theorems.
...
Physically, the examples show how a finite arrangement of complete sharp
measurements can require complex amplitudes in a common three-dimensional
description.  In $\Htwenty$, the
forced pseudocontext permits different equal-weight pure-state resolutions
of the same mixed state; in $\Htwentyone$, the obstruction survives when
that local ensemble restriction disappears and resides in compatibility
with the perimeter.  Real simulation remains possible in a larger space,
with the retained probabilities and identities determining whether
projector ranks must also change.  These distinctions specify which
operational assumptions carry the separation and provide a finite
setting in which to investigate the physical role of relative phase
through preparations, measurements, and their shared experimental meaning.
