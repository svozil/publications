...
no-arbitrage supplies a distinct financial boundary. Repeatable success
therefore requires a market restriction, benchmark, risk premium, or
informational advantage. Time reversal is a simple stress test.
\end{abstract}
...
Gold's formulation begins even closer to the ordinary information available to
a trader: an accumulating history, not the source code of the market
generator. At each date an effective learner names a program for the observed
sequence. Success means that, after some finite but unknown date, the names stop
changing and the final program generates the entire sequence. Gold proved that
no effective learner identifies every binary-valued total recursive time
function, although effectively enumerable subclasses---including the
primitive-recursive time functions---are identifiable in this limiting sense
\cite{go-65,go-67}. Thus, for every proposed universal rule learner there is a
computable market history on which it fails to settle on a correct rule.

Universal computation creates a further, distinct obstruction. If admissible
...
intuition: particular low-complexity cases can sometimes be settled, but no
uniform algorithm extrapolates those finite successes. As the description
budget grows, the worst-case halting time eventually exceeds every computable
function of that budget. This is a terminating source-code question, not
Gold's open-ended learning protocol.
...
\ref{sec:computability} gives the diagonal countermarket, Gold's history-based
identification boundary, two Halting reductions, the Busy-Beaver horizon, and
the Martin-L\"of, computable, and Schnorr randomness hierarchy together with
its algorithmic-information interpretation.
...
procedure can uniformly settle all future events of all such markets.

This is not Gold's theorem. The reduction takes source code as input and
demands a terminating yes-or-no answer. Gold's learner instead receives an
accumulating data record, may change its mind, and is judged only by eventual
stabilization. Indeed, the particular price paths in
Eq.~\eqref{eq:market-halting} are identifiable in Gold's limiting sense: guess
the constant path while the price is one; if a quote of two appears, switch
once to the program for the observed step path. On a forever-flat path the
first guess is already correct, but the learner can never know at any finite
date that it is final. Halting undecidability therefore rules out a uniform
certificate; Gold's theorem rules out eventual identification when the
history class is expanded to all binary total recursive functions. Later work
on unsolvable inductive inference develops related distinctions
\cite{blum75blum,angluin:83,ad-91,2016-AlexanderKarlSvozil-ml,moore}.
None of these claims denies that restricted parametric, stochastic, or
otherwise structured model classes can be learned.
...
\begin{equation}
  ...
  \label{eq:busybeaver}
\end{equation}
with value zero if the set is empty. Here \(n\) is a program-description budget,
not a date. The output \(\operatorname{BB}(n)\) is a number of computation
steps; after a computation is embedded in a price generator, those steps become
possible trading dates. Thus a modest increase in the size of an admissible
market rule can entail a waiting time beyond any computably prescribed scale.

Every particular \(\operatorname{BB}(n)\) is a finite integer. For a chosen
machine and a small \(n\), exhaustive simulation supplemented by proofs that
the remaining programs never halt can sometimes determine its exact value.
This is not ``computability for small arguments'' in the uniform technical
sense: computability asks for one algorithm that works for every \(n\), and any
finite table can be hard-coded. Nor can a pattern fitted to the known initial
values justify extrapolation.

If a total computable function \(h(n)\) satisfied
\(\operatorname{BB}(n)\leq h(n)\), one could run every program of length at most
\(n\) for \(h(n)\) steps and decide which of them halt. Hence no computable
size-dependent upper horizon exists. More strongly, in the standard runtime
formulation, \(\operatorname{BB}(n)\) eventually exceeds every total computable
function of \(n\), up to fixed encoding conventions. Intuitively, a program can
contain a self-delimiting description of \(n\), compute any proposed
\(h(n)\), run longer, and halt; for sufficiently large \(n\), that program's
description still has length at most \(n\)~\cite{rado,chaitin-bb}.

Calude, Dinneen, and Shu provide a concrete finite glimpse of the same barrier.
They did not compute Busy Beaver values. For one explicit universal
self-delimiting register machine, they combined exhaustive computation with
mathematical proofs to settle halting for programs through 84 bits and thereby
certify 64 initial bits of its halting probability \(\Omega_U\). They describe
the finite recovery task as ``computable, but unrealistically difficult'' and
observe that, even if the first \(n\) bits of \(\Omega_U\) are supplied, the
time required to recover the relevant halting programs grows faster than every
computable function of \(n\). Their method is therefore a genuine finite
success, not a scalable algorithm for the uncomputable object
...
\(P_t=2\). Across encoded programs of length at most \(n\), the last eventual
halt may occur at \(\operatorname{BB}(n)\); the market wrapper changes
description length only by a fixed encoding overhead. Exact knowledge of this
number would turn silence beyond that date into a certificate that no later
move can occur. No computable rule supplies those certificates uniformly.
Equivalently, with \(\mathrm{TotComp}\) denoting the total computable
size-dependent waiting rules,
\begin{equation}
  \begin{aligned}
  ...
  \label{eq:bb-horizon}
\end{equation}
If the market wrapper adds \(c\) bits and \(h\) is nonmonotone, use the
computable envelope \(H(n)=\max_{k\leq n+c}h(k)\). The corresponding total
market remains flat beyond the proposed horizon for its own description size
and then moves. Every individual quote at a specified finite date remains
computable by bounded simulation. The generator does not compute the Busy
Beaver function, and a program attaining the maximum cannot be selected
uniformly in \(n\); the noncomputability lies in certification across the
encoded family. Each realized path is deterministic and computable, hence not
Martin-L\"of random under the fair-coin coding.

The comparison with Taylor approximation is qualitative, not merely a contest
between two steep curves. For an effectively specified analytic law \(f\), a
...
are real analytic in the interior of their domains. Discarding
\(R_2(\Delta)\) therefore gives a controlled \emph{local} linearization when an
effective remainder bound and a sufficiently small \(\Delta\) are available;
it is not automatically a reliable long-horizon forecast.

Busy Beaver is a discrete noncomputable extremal function, so it has no
analogous computable Taylor envelope. Indeed, any finite set of its known
values can be interpolated exactly by a polynomial, but that perfect analytic
fit says nothing about the next value. Every computably specified analytic,
polynomial, exponential, or Taylor-derived waiting schedule is eventually
exceeded. This comparison is with \emph{computably specified} analytic laws,
not with analyticity alone: an analytic formula, its coefficients, or its
boundary data may themselves contain noncomputable information.

This computability-theoretic obstruction goes beyond sensitive dependence in a
precise decision-theoretic sense: even with the generating rule and initial
data known exactly, no uniform terminating algorithm can certify the encoded
unbounded-future event. A finite contractive iterated-function system with
...
which no general decision algorithm exists. The categories can overlap: a
dynamical system capable of universal computation inherits the Halting
barrier. This is a worst-case statement across generator descriptions, not a
claim that typical financial markets display Busy-Beaver delays. The contrast
is therefore between sensitivity or visual complexity \emph{alone} and
computational universality, not between all chaotic systems and all computers
\cite{barnsley:88,moore,calude:037103}.

These computational claims are related but not identical. Gold's result has
the learner-relative form of Eq.~\eqref{eq:goldquantifiers} and concerns
eventual stabilization from observed history.
...
A large market record can lure the trader precisely because it is
pattern-rich. Ramsey theory gives this observation a rigorous combinatorial
meaning without assuming a probability law. For a finite set \(A\), write
\([A]^q\) for its \(q\)-element subsets and \([c]=\{1,\ldots,c\}\). The finite
Ramsey theorem states
\begin{equation}
 \begin{aligned}
  &\forall c\geq1,\ m\geq q\geq1\ \exists R_q(m;c),\\
  &\forall N\geq R_q(m;c),\\
  &\forall\,\chi\colon[N]^q\to[c]\ \exists H\in[N]^m,\\
  &\chi\!\upharpoonright_{[H]^q}\ \text{is constant}.
 \end{aligned}
 \label{eq:finiteramsey}
\end{equation}
Thus every red--blue coloring of the pairs among six objects contains a
monochromatic triangle. Van der Waerden's theorem gives a time-ordered
analogue: for every fixed number of colors and target length, every
sufficiently long colored initial segment of the integers contains a
monochromatic arithmetic progression of that length
\cite{Ramsey-GRS-90,GS-90}. These theorems justify the expository slogan that
complete disorder is impossible only in this precise Ramsey-theoretic sense.
Their thresholds can be enormous, but they are finite and computable; this is
not a Busy-Beaver obstruction.

The conclusion depends entirely on the chosen coding. In a discretized return
record, a monochromatic arithmetic progression consists of equal symbols at
equally spaced dates; those dates need not be consecutive, and the symbols
need not form a price trend. A Ramsey-homogeneous set may be sparse and found
only after the whole sample has been inspected. The theorem guarantees some
color, not a prespecified favorable one, and says nothing by itself about
effect size, statistical dependence, causation, persistence, transaction
costs, or profit.

There is no conflict here with Martin-L\"of randomness. Indeed, a fair-coin
Martin-L\"of-random sequence is Borel normal. For a nonempty binary word \(u\),
let
\[
 N_u(N)=\#\{0\leq t\leq N-|u|:
 x_{t+1}\cdots x_{t+|u|}=u\}.
\]
Then
\begin{equation}
 \begin{aligned}
  \lim_{N\to\infty}\frac{N_u(N)}{N}
    &=2^{-|u|},\\
  \lim_{N\to\infty}\frac{N_{u0}(N)}{N_u(N)}
    &=\frac12,\\
  \lim_{N\to\infty}\frac{N_{u1}(N)}{N_u(N)}
    &=\frac12 .
 \end{aligned}
 \label{eq:randomnormality}
\end{equation}
Consequently every prescribed finite sign word---including arbitrarily long
runs and any chart motif expressible in that binary coding---recurs infinitely
often, while its next bit is asymptotically
balanced~\cite{calude:02,Nies2009}. Normality alone is much weaker than
Martin-L\"of randomness; the point is that local regularity is compatible with
the stronger notion. Algorithmic randomness means escaping every effective
null test and lacking a global effective compression or sustained betting
certificate, not the absence of finite patterns.

Calude and Longo call regularities forced by data volume
\emph{Ramsey-type correlations}. Their accompanying incompressibility count
sharpens the contrast: under the uniform measure, almost all sufficiently long
binary strings are incompressible by any fixed positive fraction of their
length, yet every sufficiently long encoding still contains the appropriate
Ramsey regularities~\cite{Calude2016}. This is an epistemic warning, not a new
trading impossibility theorem. A Ramsey configuration is not itself a sample
correlation coefficient, and neither Ramsey theory nor incompressibility proves
that every observed financial correlation is spurious.

The trader's trap arises when pattern abundance is combined with selection.
Searching many assets, date ranges, indicators, transformations, lags, and
parameter values creates many opportunities to retain an exceptional in-sample
result. If \(M\) independent tests each reject with probability \(\alpha\)
under their true null hypotheses, the chance of at least one false rejection
is \(1-(1-\alpha)^M\); without independence the union bound gives at most
\(\min\{1,M\alpha\}\). This probabilistic multiple-testing statement is
distinct from Ramsey's deterministic existence theorem, but the practical
lesson is aligned: a discovered trend or cross-market relation is a candidate
hypothesis, not yet an edge. The full search universe must be reported, and the
candidate must survive selection correction, costs, a genuinely untouched
holdout or independent replication, and testing outside the selected regime.
...
seller accepts downside or volatility risk for a premium; and a lawfully
informed rule relies on costly analysis or information lawfully obtained and
not yet reflected in prices. A single realized profit may be luck. What
requires an articulated edge is a repeatable ex ante claim---
...
The finite-pattern trap of Section~\ref{sec:ramseytrap} becomes operational in
empirical strategy discovery. Trying many specifications and reporting the
best backtest makes its apparent Sharpe ratio an order statistic rather than
an unbiased performance estimate. The Deflated Sharpe Ratio corrects for
selection, non-normal returns, and the number of
trials~\cite{BaileyLopez2014}. Such corrections do not prove an edge; they make
a finite-sample FAPP claim more honestly falsifiable.
...
position. This is a direct strategy-relative diagonal construction:
\(\forall\sigma\,\exists\mathcal E_\sigma\). No physical market has to observe
the deployed trader. The always-flat trader earns zero, while every nonzero
position incurs a strict loss on its constructed countermarket.

The direct diagonal construction needs no Halting-Problem reduction. Gold's
result supplies the complementary history-based limit: no effective
learner stabilizes on a correct generating program for every computable binary
record, although appropriately restricted classes can be identified in the
limit. The sharper next-move diagonal makes a computable predictor wrong at
every date on its predictor-relative computable record. The separate halting
argument shows that eventual-profit certification and uniform certification of
encoded market events are undecidable even under promises of totality.
Busy Beaver maps a description-size budget to a possible event date beyond
every computable size-dependent waiting schedule. Particular low-complexity
cases may be settled by finite computation and proof---as the Calude
\(\Omega_U\) calculation illustrates---but those finite glimpses do not supply
a scalable continuation. A passive Martin-L\"of-random record blocks unbounded
...
measure. The computable halting-encoded family with Busy-Beaver-scale
transients and the incompressible random record are therefore different kinds
of opacity. No-arbitrage excludes the standard almost-sure zero-cost payoff,
and uniform binary averaging fixes expected directional accuracy at \(1/2\).
These are distinct claims with distinct quantifiers.

Ramsey regularity adds a finite-sample warning rather than another no-go
theorem: structured-looking subsequences are unavoidable in sufficiently rich
records, including random ones. A data-mined trend is therefore a hypothesis
requiring genuinely prospective support, not yet a trading edge.
...
remains possible. Real traders likewise proceed through explicit heuristics:
trend persistence, mean reversion, valuation correction, compensated risk
exposure, or some other restriction on the market side. They may also possess
a lawful informational advantage obtained through research, interpretation,
specialization, or speed. Trading on material nonpublic information where the
applicable law prohibits it is not a mathematical escape from the results; it
falls outside the lawful practical strategy class, and even superior
information is not a pathwise guarantee.

A rising nominal chart, moreover, need not show real value creation. For a
funded portfolio, if \(R^{\mathrm{nom}}\) is nominal total return and \(\pi\)
is inflation over the same interval, measured by a stated price index, then
\begin{equation}
  1+R^{\mathrm{real}}
  =\frac{1+R^{\mathrm{nom}}}{1+\pi}.
  \label{eq:real-return}
\end{equation}
Merely offsetting the associated loss of purchasing power is therefore the
zero-real-gain baseline. Applied scientific and technological progress can
raise productivity and real output~\cite{Solow1957}. If those gains persist
and reach listed firms as after-cost cash flows in a sufficiently stable
institutional regime, they may support a positive long-horizon tendency in the
real total return of diversified productive capital. This is a falsifiable
regime prior, not a universal law: expectations may already be capitalized,
discount rates and risk premia change, gains may accrue elsewhere, and
innovation can displace incumbent firms~\cite{Schumpeter1942}. A positive
long-run tendency may justify long exposure under that restricted hypothesis,
but it neither predicts each next move, establishes active alpha, nor
contradicts any no-go result above.

A large or public strategy may also change the market on which it was
estimated. Accordingly, any defensible claim of repeatable success must state
its market restriction, probability law, benchmark, financing and
admissibility conditions, horizon, data protocol, and deployment effects.
Time reversal is one simple robustness diagnostic among many, not a universal
test. The paper's central conclusion is therefore conditional: useful
strategies may exist, but no total deterministic computable self-financing rule
guarantees strict gain on every positive computable price path; under an
arbitrage-free stochastic model, no admissible zero-cost rule can deliver
nonnegative discounted gain almost surely and positive gain with positive
probability.
