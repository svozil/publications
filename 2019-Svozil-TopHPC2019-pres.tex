...
\title{\bf \color{white} {Some Quantum advantages}}
\subtitle{ \color{white} {\footnotesize http://tph.tuwien.ac.at/\textasciitilde{}svozil/publ/2019-Svozil-TopHPC2019-pres.pdf
%\\
...
\date{\textcolor{white!100}{TopHPC2019, Tehran, Iran, April 22-25, 2019}}
...
% }
 ...
 \frame{
 \frametitle{\color{orange!100}Possible quantum advantages: features not present in classical "paper machines"}
...
Scheme to exploit quantum parallelism supporting/rendering
...
by a Hadamard or quantum Fourier transformation;
\item
...
\end{itemize}
 ...
 \frame{
 \frametitle{\color{orange!100}
``Babylonian'' example 1: Deutsch algorithm (eg, Mermin DOI: 10.1017/CBO9780511813870) -- ``parity'' [aka (non)constancy]
of a Boolean function of a single bit}
...
$f_i(x)$&$x=0$&$x=1$\\
\hline
{\color{red!100} $f_0$}&0&0\\
{\color{green!100}$f_1$}&0&1\\
{\color{green!100}$f_2$}&1&0\\
{\color{red!100}$f_3$}&1&1\\
\hline
\end{tabular}
\end{wraptable}

The task is to find out {\color{orange!100} (not which function but)} whether or not this function is constant.
...
\large\}
$
of all such functions, which can be realized by {\color{red!100}one} quantum query.



{\color{red!100}However:} No generalization exist, as there is no exponential quantum speedup for parity (Farhi, Goldstone, Gutmann~\& Sipser DOI: 10.1103/PhysRevLett.81.5442).
...
\item
It processes this coherent superposition of all $i$ by computing $x^i \text{ mod } n$,
...
which then (with high probability) conveniently yields a classical information (in one register) about the period or order; that is,
