...
\begin{equation}
{\cal L}_t \psi = \frac{d^2}{dt^2} \psi = k^2  ,
\end{equation}
with $k  \in {\Bbb R}$.
...
\begin{equation}
G(t - t') = \int_{-\infty}^{\infty}  \frac{e^{-i\omega (t-t')}}{(-\omega^2)} d\omega.
\end{equation}
And the solution is obtained by integrating over the constant $k^2$;
that is,
\begin{equation}
%\begin{array}{l}
\psi (t)=\int_{-\infty}^{\infty}  G(t - t') k^2 dt'
=-\int_{-\infty}^{\infty} \left(\frac{k}{\omega}\right)^2  e^{-i\omega (t-t')} d\omega dt'
.
%\end{array}
\end{equation}
Note that if we are imposing boundary conditions; e.g.,
$\psi (0) = \psi ( L ) =0$,
representing a string ``fastened'' at positions $0$ and $L$,
the eigenfunctions change to
\begin{equation}
\psi_k (t) = \sin ( \omega_n t)= \sin \left( \frac{n \pi }{L} t\right),
\end{equation}
...
\begin{eqnarray*}
    G_1(t,t')&=&-{1\over2\pi}\int\limits_{-\infty}^{+\infty}
                 ...
                 & &\qquad\qquad\ {\rm Res}\left({e^{ik(t-t')}
                    \over(k+1)(k-1)};k=-1\right)\Biggr]H (t-t')
\end{eqnarray*}
...
\begin{eqnarray*}
   G_1(t,t')&=&-{i\over2}\left(e^{i(t-t')}-e^{-i(t-t')}\right)H (t-t')=\\
            &=&{e^{i(t-t')}-e^{-i(t-t')}\over2i}H (t-t')=
               \sin(t-t')H (t-t')\\
   G_1(0,t')&=&\sin(-t')H (-t')=0\qquad\mbox{  since  }\quad t'>0\\
   G_1'(t,t')&=&\cos(t-t')H (t-t')+\underbrace{\sin(t-t')\delta(t-t')}_
                {\mbox{$=0$}}\\
   G_1'(0,t')&=&\cos(-t')H (-t')=0\qquad\mbox{ as before.}
\end{eqnarray*}
