...
\title{The Quantum Resemblance: Context, Stochasticity, and Meaning in Large Language Models}
...
Modern LLMs built on the Transformer architecture resolve polysemy by generating dynamic, context-aware vector embeddings. This contrasts sharply with older models (e.g., Word2Vec) which assigned a single, static vector to each word, effectively averaging all its possible meanings into one representation.
...
The central vector $\bm{v}(\text{bank})$ is a shared element common to three distinct contexts (orthonormal bases). Each context connects $\bm{v}(\text{run})$ to a different semantic association: physical activity ($\bm{v}(\text{exercise})$), management ($\bm{v}(\text{business})$), and flow ($\bm{v}(\text{river})$).}
...
$\bm{e}_1= (1,0,0)^\intercal$, $\bm{e}_2=(0,1,0)^\intercal$, and $\bm{e}_3=(0,0,1)^\intercal$ as well as the orthogonal basis $\{\bm{f}_1, \bm{f}_2, \bm{f}_3\}$
with $\bm{f}_1=(1/\sqrt{2}, -1/\sqrt{2}, 0)^\intercal$, $\bm{f}_2=(1/\sqrt{2}, 1/\sqrt{2}, 0)^\intercal$, and $\bm{f}_3=(0,0,1)^\intercal$, obtained by rotating the first and second vectors
...
In contrast, a Transformer computes layer-wise updates for its hidden state representations.
