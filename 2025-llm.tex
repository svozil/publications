...
The theoretical underpinning for this phenomenon is the
 Johnson-Lindenstrauss lemma~\cite{johnson-1984,Dasgupta-2003,mahoney2009johnsonlindenstrauss,freksen2021ijohnsonlindenstraus}.
...
where $k = O(\varepsilon^{-2} \log n) = \varepsilon^{-2} O(\log n)$,
while preserving the pairwise distances  to within a factor of $(1 \pm \varepsilon)$.
Indeed, there is a tight bound~\cite[Section~9]{alon-2003}.
 This translates---through the polarization identity $\bm{u} \cdot \bm{v} = \frac{1}{2} \left( \|\bm{u}\|^2 + \|\bm{v}\|^2 - \|\bm{u} - \bm{v}\|^2 \right)$ applied to the norms of the projected vectors---into the corresponding $n$
 position vectors (from the origin to the points) also having their pairwise inner products preserved to within a small additive error,
which is the crucial property for establishing quasi-orthogonality.
