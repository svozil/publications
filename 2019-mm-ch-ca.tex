...
{\it ``Verwirrungen des Z\"ogling T\"orle\ss''}\sidenote[][-15mm]{German original \url{http://www.gutenberg.org/ebooks/34717}:
{\it
...
als ob sie ganz dast\"unde?
F\"ur mich hat so eine Rechnung etwas Schwindliges;
als ob es ein St\"uck des Weges wei\ss{} Gott wohin ginge.
Das eigentlich Unheimliche ist mir aber die Kraft,
die in solch einer Rechnung steckt und einen so festh\"alt,
...
\index{theory of complex functions}
will be presented.
For much more detailed introductions to complex analysis,
including proofs,
take, for instance, a ``classical'' introduction,\cite[-7mm]{Remmert-1991-tocf}%,jaenich-ft,salamon-ft},
...
Note that the function $\varphi \mapsto e^{i\varphi}$ in (\ref{2017-m-ca-cn}) is not injective. In particular,
...
%as $\sqrt[p]{z} = \sqrt[p]{\vert z\vert } \exp \left(i\varphi / n + 2\pi i k/n\right)$
with $k \in \mathbb{Z}$.
...
\begin{equation}
1 \equiv \begin{pmatrix}1,0\end{pmatrix},\text{ and }
i \equiv \begin{pmatrix}0,1\end{pmatrix}.
\end{equation}
Figure~\ref{2018-m-f-cp} depicts this schema,
including the location of the points corresponding to the real and imaginary units $1$ and $i$,
respectively.
\begin{marginfigure}
\begin{center}
...
\draw[line width=0.5pt,orange] (-3,-3) grid[xstep=1, ystep=1]  (3,3);

% Draw the axis

%\draw [color=black] (-3,0) -- (3,0);
%\draw [color=black] (0,-3) -- (0,3);
\draw[draw=gray!80,->] (0,-3) + (0,-0.5cm)  -- (0,3) -- +(0,0.5cm) node[above right] {$\Im z$};
\draw[draw=gray!80,->] (-3,0) +(-0.5cm,0) -- (3,0) -- +(0.5cm,0) node[below right] {$\Re z$};

% Draw half circles

\draw [line width=0.5pt,color=blue,dashed] (-1cm,0cm) arc [start angle=-180,end angle=180,x radius=1cm, y radius=1cm];

% Draw of points

\filldraw  (0,1) circle[radius=4pt] node[above right] {$i$};
\filldraw  (1,0) circle[radius=4pt] node[above right] {$1$};
\filldraw  (3,1) circle[radius=4pt] node[above right] {$3+i$};
\filldraw  (1,-2) circle[radius=4pt] node[below right] {$1-2i$};
\filldraw  (-2,2) circle[radius=4pt] node[above left] {$-2+2i$};
\filldraw  (-2.5,-3) circle[radius=4pt] node[below left] {$-\frac{5}{2}-3i$};
\filldraw  (0,0) circle[radius=4pt] node[below left,yshift=0.0ex,xshift=0.0ex] {$0$};

\end{tikzpicture}
\end{center}
\caption{Complex plane with dashed unit circle around origin and some points}
\label{2018-m-f-cp}
\end{marginfigure}

The addition and multiplication of two complex numbers represented by $\begin{pmatrix}x,y\end{pmatrix}$ and $\begin{pmatrix}u,v\end{pmatrix}$ with $x,y,u,v \in {\Bbb R}$
are then defined by
\begin{equation}
\begin{split}
...
Earlier we encountered problems with the square root function on complex numbers.
...
\begin{axis}[axis lines=middle, draw=gray!80,axis equal, enlargelimits=true ,
xtick={-1,0,1,2},
ytick={-1,0,1,2},
...
To give an example mentioned earlier: the square root function $\sqrt{z} =
...
%\draw [color=black] (-3,0) -- (3,0);
%\draw [color=black] (0,-3) -- (0,3);
\draw[draw=gray!80,->] (0,-5) + (0,-0.5cm)  -- (0,5) -- +(0,0.5cm) node[above right] {$\Im w$};
\draw[draw=gray!80,->] (-5,0) +(-0.5cm,0) -- (5.3,0) -- +(0.5cm,0) node[below right] {$\Re w$};

% Draw half circles

\draw [color=orange,->] (-4cm,0cm) arc [start angle=-180,end angle=177,x radius=4cm, y radius=4cm];
%\draw [color=blue,dashed,->] (0cm,2cm) arc [start angle=90,end angle=267,x radius=2cm, y radius=2cm];

% Draw of points

\filldraw  (-4,0) circle[radius=6pt] node[above right] {$b^2$};
\filldraw [draw=gray!90] (-4,0) circle[radius=3pt] node[below right] {$a^2$};
\filldraw [color=orange] (4,0) circle[radius=4pt] node[above right] {$c^2$};
%\filldraw [color=blue] (-2,0) circle[radius=4pt] node[above left] {$D$};
\end{tikzpicture}
&
\begin{tikzpicture}  [scale=0.25]
...
%\draw [color=orange,->] (-4cm,0cm) arc [start angle=-180,end angle=177,x radius=4cm, y radius=4cm];
\draw [color=blue,dashed,->] (-4cm,0cm) arc [start angle=-177,end angle=180,x radius=4cm, y radius=4cm];

% Draw of points

\filldraw  (-4,0) circle[radius=6pt] node[above right] {$a^2$};
\filldraw [draw=gray!90] (-4,0) circle[radius=3pt] node[below right] {$b^2$};
\filldraw [color=blue] (4,0) circle[radius=4pt] node[above right] {$d^2$};
%\filldraw [color=blue] (-2,0) circle[radius=4pt] node[above left] {$D$};
\end{tikzpicture}
\\
$z= r e^{  i \textrm{arg}(z)}$
&
$w(z)= z^2= r^2 e^{ 2 i \textrm{arg}(z)}$
&
$w(z)= z^2= r^2 e^{ 2 i \textrm{arg}(z)}$
\\
$-\pi < \textrm{arg}(z) \le +\pi$
...
\end{tabular}
\end{center}
\label{f-2018-m-ca-csrt}
\caption{Sketch of the Riemann surface of the square root function, requiring two sheets with the origin as branch point,
as argued from the inverse (in this case square) function.}
\end{figure}
%\end{fullwidth}
}
...
\eproof
 ...
 \section{Laurent and Taylor series}
 \index{Laurent series}
 \index{Taylor series}

%https://www.youtube.com/watch?v=BsDGcJN_1TU
...
Suppose that $f(z)$ is analytic at and in a region $G$ ``around'' $z_0$.
...
No proof is given here.
\marginnote[0mm]{For proofs and additional information see  Chapter~6 of~\bibentry{Brown-Churchill}.}
...
appear that $\sin z$ is nonconstant and bounded. However, it is only bounded on the real axis;
indeed,  $\sin iy = (1/2i)(e^{-y}-e^y) = i \sinh y$.
Likewise, $\cos iy =   \cosh y$.}
...
\end{split}
 \end{equation}
\marginnote[-20mm]{Note that, as
 $\overline{(uv)}= (\overline{u}) {\;} (\overline{v}) $, so is
 $\vert uv \vert^2 = uv \overline{(uv)}= u(\overline{u}) {\;} v(\overline{v}) =\vert u \vert^2 {\;} \vert v \vert^2$.}
...
\marginnote[-20mm]{For a discussion and proofs, see, for instance,
Chapter~19 of \bibentry{ziegler-aigner},
or Chapter~4 (by Remmert) of \bibentry{Numbers-Ebbinghaus}.}
...
\index{asymptotic development}
or, equivalently, an
{\em asymptotic representation}
\index{asymptotic representation} or
{\em asymptotic expansion}
\index{asymptotic expansion}
...
asymptoticity
\index{asymptoticity} can be defined as follows:\cite[-0mm]{Olver-1997,Bender-Orszag,Boyd99thedevil}
...
the Bachmann-Landau notation or asymptotic notation\sidenote{The symbol ``$O$'' stands for ``of the order of'' or ``absolutely bound by'' in the following way:
...
%\item
%the function $f(z) = z^\alpha$ with real $0 < \alpha < \infty$.
%In this case $a_0=0$, and $a_1=\lim_{z\rightarrow 0} c z^(\alpha - 1)
...
However, if we allow also functions $g(z)$ which are not necessarily analytic, then
...
such that (\ref{2019-mm-ch-ca-ritt}) holds; that is,\sidenote[][-0mm]{For proper definitions, proofs and further details
see~\bibentry{Pittnauer-73}, \bibentry{Remmert-1991-tocf} and \bibentry{Costin-2009}.}
...
Other (uniform with respect to the summation index $j$)
...
\marginnote[0mm]{Here we closely follow Section~88 of~\bibentry{Brown-Churchill}.}
...
Suppose $R>0$.
%Because of the reflection symmetry of the sine function with respect to the argument $\frac{\pi}{2}$, that is, $\sin \left( \frac{\pi}{2} + \theta \right) =  \sin \left( \frac{\pi}{2} - \theta \right) =  - \sin \left( \theta -  \frac{\pi}{2}\right)$
...
In the domain $0\le \theta \le \frac{\pi}{2}$ of this integration
$\sin \theta \ge \frac{2}{\pi} \theta$ holds.\sidenote{This can be made plausible by drawing the graphs of
$\sin \theta$ and $\frac{2}{\pi} \theta$ in the interval $\left[0,\frac{\pi}{2}\right]$; see also
formula~4.3.79, page~75 of~\bibentry{abramowitz:1964:hmf}}
%https://math.stackexchange.com/questions/1820362/proof-that-frac2-theta-pi-sin-theta-theta
...
%\draw [color=black] (-3,0) -- (3,0);
%\draw [color=black] (0,-3) -- (0,3);
\draw[draw=gray!80,->] (0,-5) + (0,-0.5cm)  -- (0,5) -- +(0,1cm) node[above right] {$\Im z$};
\draw[draw=gray!80,->] (-5,0) +(-0.5cm,0) -- (5,0) -- +(1cm,0) node[below right] {$\Re z$};


\draw[orange,->] (-5,0) -- (5,0);
\draw[blue,dashed,->] (-5,0) -- (4.8,0);

% Draw half circles

\draw [color=orange,->] (5,0) arc [start angle=0,end angle=180,x radius=5cm, y radius=5cm];
\node[orange] at (5.3,4.5) {$C_\infty \text{ for } k> 0$};
\draw [color=blue,dashed,->] ( 5,0) arc [start angle=360,end angle=180,x radius=5cm, y radius=5cm];
\node[blue] at (5.3,-4.5) {$C_\infty \text{ for } k < 0$};

%\filldraw [color=blue] (0,-2) circle (3pt) node[above right] {$-i$};

\end{tikzpicture}
\caption{Extension of the integral over the real $z$-axis by including the Jordan arcs $C_\infty$
in the upper ($k > 0$) and lower ($k < 0$)  imaginary half planes of $z$, respectively.}
\label{2021-mm-ch-ca-ja}
\end{center}
\end{marginfigure}
}
Let us, for the sake of an example,
compute the integral along the real line
$\int_{-\infty}^\infty e^{i k x} dx$.
We extend this integral by the Jordan arc  $C_\infty = \lim_{R\rightarrow \infty} C_R$
%, depicted in Figure~\cite{2021-mm-ch-ca-ja},
after
analytic continuation of the integral kernel $ e^{i k x} \rightarrow  e^{i k z}$
to the complex $z$-plane.

This contour integral has an analytic interior.
