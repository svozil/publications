...
\begin{column}{0.485\textwidth}
\begin{block}{Measurement dynamics --- violation of unitary confinement}
\begin{itemize}
  \setlength{\itemsep}{0.5em}
  \item \textbf{von Neumann Process 1 (violation of unitary state evolution):} projection/collapse --- a nonunitary
        state update.
  \item \textbf{Process 2 (unitary confinement):} unitary, reversible evolution preserving inner
...
\begin{exampleblock}{Additional postulate; exact conditional theorem}
Postulate that the outcomes are independent, identically distributed fair
bits.  Thus, for every $s\in\{0,1\}^n$,
$\Pr\bigl((X_1,\ldots,X_n)=s\bigr)=2^{-n}$.  This is Martin-L\"of's fair-coin
product measure $\mu$.  For a universal test $(U_m)$,
$\mu(U_m)\leq2^{-m}$ and the nonrandom set is $N=\bigcap_m U_m$; hence
$\mu(N)=0$ and $\mu(\mathrm{MLR})=1$.
\end{exampleblock}

\begin{alertblock}{Probability one $\neq$ logical necessity}
The measure-one conclusion is an exact theorem \emph{conditional on the process
model}.  Martin-L\"of proves it for the specified computable measure; he does
not prove that physical trials are independent, identically distributed fair
bits.  Null, nonrandom
realizations still exist, and no finite prefix can certify Martin-L\"of
randomness.
\end{alertblock}
