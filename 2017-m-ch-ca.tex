...
\sidenote[][-40mm]{German original (http://www.gutenberg.org/ebooks/34717):
{\it
...
als ob sie ganz dastünde?
Für mich hat so eine Rechnung etwas Schwindliges;
als ob es ein Stück des Weges wei\ss{} Gott wohin ginge.
...
\end{equation}
Fig.~\ref{2018-m-f-cp} depicts this schema,
...
We shall use this funtion as a sort of ``Rosetta stone'' for an understaing of conceivable ways of coping with nonunique functional values.
...
$-\pi < \textrm{arg} (z)\le +\pi$.
Thus the Riemann surface of inverse function of the square function, that is, the square function, has to have 2 sheets to be able to cover
...
states that if $f: {\Bbb C} \rightarrow {\Bbb C}$ is an entire function and if
for some real number $c$ and some positive integer $k$ it holds that
$\vert f(z)\vert \le c \vert z \vert^k$ for all $z$ with $\vert z \vert >1$,
then $f$ is a polynomial in $z$ of degree at most $k$.

No proof \cite{Greene} is presented here.
This theorem is needed for an investigation into the general form of
