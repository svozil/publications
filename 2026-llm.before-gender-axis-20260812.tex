...
\begin{abstract}
A chosen $d$-dimensional language-model representation has only $d$
independent coordinates, yet, at fixed angular tolerance, it can accommodate
exponentially many nearly orthogonal feature directions.  L\'evy concentration
and Johnson--Lindenstrauss embeddings anchor this claim, while simultaneous
readout remains limited by overlap: for uniformly random directions, a typical
pair has root-mean-square overlap $1/\sqrt d$.  We then propose a strictly
structural analogy for polysemy.  The same lexical anchor ray is retained
across different uses, just as one ray may be an element of several
orthonormal bases.  The surrounding text then controls a separate classical
residual map.  Thus the complete large language model representation depends
on linguistic context, whereas, for a fixed quantum state, the Born probability
of the shared projector does not depend on which compatible basis contains it.
\end{abstract}
...
semantic directions, and what limits their simultaneous recovery by a simple
correlation readout?
...
readout.  We hypothesize that learning spends its limited angular separation where
cross-talk is costly---between important features that often activate
together.  For lexical ambiguity, a reusable lexical ray may additionally be
shared across several context-dependent residual subspaces.
...
The coactivation prediction and the shared-lexical-ray model are empirical
...
sequence drives different contextual states.  In an ordinary Transformer,
every occurrence of the same token already begins with the same lookup vector;
the later hidden vectors are generally different because self-attention and
the remaining layers depend on the surrounding sequence.  Our proposal makes
that distinction into an explicit structural constraint.  A fixed unit vector
$r_t$ represents the reusable lexical anchor; several partial orthonormal
structures contain that same vector, and a classical context-dependent map supplies the remaining
sense information.  The nontrivial claim is therefore not that a shared input
embedding exists.  It is that retaining an identifiable shared ray at a chosen
representation layer, while conditioning the complementary directions and
readout on context, improves held-out prediction, interpretability, or sample
efficiency relative to equally expressive untied models.
...
Here ``quantum-inspired'' has one deliberately narrow meaning.  In quantum
theory, a unit vector determines a ray, equivalently a rank-one projector, and
that ray may occur as one outcome in more than one orthonormal basis.  The
bases are then interlocking measurement contexts with a common element.  In
dimension at least three they can be distinct while sharing only that element.
For a fixed quantum state $\rho$, the Born probability of the shared projector $P$ is
$\operatorname{tr}(\rho P)$ and is independent of which other orthogonal
projectors complete the basis.  Gleason's theorem, under its hypotheses in
dimension at least three, characterizes probability assignments of this
basis-independent form~\cite{Gleason,ramanathan-2024}.  Here $\rho$ is the
matrix representing the fixed quantum state, $P$ projects onto one possible
outcome ray, and $\operatorname{tr}$ means the trace, the sum of a matrix's
diagonal entries.  This limited context independence does not mean that the
probability is independent of the quantum state.

The proposed language-model encoding copies only the incidence pattern, not
the quantum probability rule.  The same lexical ray is placed in several
partial orthonormal structures, each completable to a basis and drawable as a
hyperedge sharing one vertex with the others, but the
semantic map is deliberately allowed to depend on the surrounding text.  A
hypergraph records which elements belong to which contexts; it does not by
itself prescribe probabilities, dynamics, or noncommutativity
\cite{svozil-2021-chroma}.  Thus the analogy is structural, whereas the LLM
readout remains explicitly context dependent.  The narrow statement about a
shared quantum projector should not be confused with claims about state
update, sequential measurements, or Kochen--Specker contextuality.  The
relation to quantum cognition and compositional distributional semantics is
reviewed in Sec.~\ref{sec:related-quantum}; no physical quantumness or semantic
Kochen--Specker theorem is inferred.
...
Secs.~\ref{sec:reference} and \ref{sec:attention} present the shared lexical
ray and its context-dependent router; and Sec.~\ref{sec:discussion} relates the
...
% ======================================================================
\section{A shared-anchor model of lexical ambiguity}
\label{sec:reference}
% ======================================================================
...
separate classical model with a deliberately narrow quantum-logic inspiration.
One fixed lexical ray is contained in several partial orthonormal structures,
each of which can be completed to an orthonormal basis.  The
surrounding text selects or mixes context-specific residual directions, so the
complete contextual representation generally changes even though its lexical
anchor does not.  The shared part can represent lexical identity, morphology,
or a token-specific baseline.  Tying it across contexts pools training
examples and reduces the number of separately estimated directions.  The
expected advantage is regularization, interpretability, or sample efficiency,
not greater expressivity.  It should be largest for rare or imbalanced uses and
can disappear or reverse when no stable shared component exists.
...
organizes that selection by sharing a token-associated ray while placing
context-specific content in perpendicular residual subspaces.  The global code
...
\subsection{The shared lexical-ray hypothesis}
...
map into the same layer carrier.  It is a hypothesized context-stable lexical
...
empirical hypothesis concerns whether this tied organization improves a
declared held-out task.
...
The center is the shared lexical projector $P_{\rm bank}$; the outer nodes
represent displayed residual sectors for the financial, river-edge, and
seat/bench contexts.  Each colored arm is only the visible fragment of a
context hyperedge; omitted mutually orthogonal vertices complete its basis.
When $m_s>1$, an outer $S_{t,s}$ node compresses several rank-one projector
vertices associated with an orthonormal basis of that residual subspace; it is
not itself one rank-one quantum outcome.
The concentric colors mark membership of the same projector in all three
contexts.  The diagram records shared incidence only, not a vector, map,
causal arrow, Born-rule measurement, or claim about what an existing
Transformer has learned.
}
\label{fig:reference-subspace}
\end{figure}

\subsection{Shared-anchor residual subspaces and classical filters}
...
\subsection{Projection energy and context-class readout}
\label{sec:gated}
...
surrounding sequence $x$.  It is therefore correct to say that a modern LLM
uses one initial token vector and generally different contextual occurrence
vectors.  The shared-anchor proposal is a constraint on how those two levels
are related, not an alternative hardware architecture.
...
h_t^+(x)=a_t r_t+(I-P_t)\bigl[h(x)+\eta U\bar h_t(x)\bigr],
\end{equation}
%
where $U\in\R^{d\times d}$ is a learned output map and $\eta$ is a scalar gate.
...
The shared-anchor residual-subspace proposal adds a separate inductive bias for lexical
ambiguity.  Reusing one lexical ray across context classes should reduce sample
complexity when the common component is real and per-use data are scarce.
...
The present model is a classical tied-subspace adapter: a router mixes
context-conditioned residual subspaces that share one lexical ray.  Its
...
source of the structural constraint, not the physical nature of the
computation.
...
mixes real low-rank residual subspaces tied through one real lexical ray.  Its
quantum-logic inspiration consists of one structural fact.  A projective
measurement context can be represented by an orthonormal basis, or by the
rank-one projectors associated with its basis vectors.  Distinct contexts may
contain the same projector.  Figure~\ref{fig:reference-subspace} copies that
shared-incidence pattern: several context hyperedges contain $P_t$.

The contrast is equally important.  For a fixed quantum state $\rho$, the Born
probability $\operatorname{tr}(\rho P_t)$ of that shared projector does not
depend on which compatible projectors complete the measurement context.  In
the proposed LLM model, the router probabilities, residual coordinates, and
complete representation depend deliberately on the linguistic context $x$.
The router is learned by ordinary optimization and obeys no Born or Gleason
rule.  Noncommuting matrices, order effects, complex phases, unitary dynamics,
and physical quantum states are not part of the proposal.
...
For lexical ambiguity, the proposed tied adapter preserves a fixed lexical
anchor while the surrounding text selects or mixes low-rank residual context
subspaces.  Sharing is predicted to improve sample efficiency when a genuine
common component exists and per-sense data are scarce.  The weak existence of
a token component is trivial; the substantive test is incremental held-out
value beyond token means, principal-component removal, and parameter-matched
untied adapters.

The minimum pilot in Sec.~\ref{sec:predictions} can reject the shared-anchor
claim.  The implementation is classical.  Its only quantum-logic-inspired
element is the incidence pattern of one shared projector in several
orthonormal contexts; no Born-rule, noncommutativity, or semantic
Kochen--Specker claim is made.
