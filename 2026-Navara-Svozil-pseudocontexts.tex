...
\begin{abstract}
We give a restrictive definition of a pseudocontext.  Two disjoint families
of propositions form a pseudocontext only when their equality of probability
sums is forced by finitely many context-normalization equations.  The
definition is therefore independent of a chosen coordinatization and of the
mechanism used to generate probabilities, provided mutually exclusive
propositions are additive and every context is complete.  For the incidence
matrix $M$ of a finite context hypergraph and
$d=\chi_A-\chi_B$, forcing is equivalent to the existence of a balanced
certificate $\lambda$ satisfying
$M^{\mathsf T}\lambda=d$ and $\one^{\mathsf T}\lambda=0$.
This supplies an exact rational algorithm and includes complementary context
coverings as integral certificates.  We apply the criterion to a
15-vertex real three-dimensional construction with pseudocontexts of size
three and to a 20-vertex complex three-dimensional construction with
pseudocontexts of size two.  The latter is the minimum possible size over
$\C$, whereas size three is minimal over $\R$.  Equal projector sums that
hold only for selected frames or coordinates are distinguished as
representation-dependent identities and are not pseudocontexts in the
present sense.
\end{abstract}
...
separately in Ref.~\cite{navara-svozil-2026-complex-hypergraphs}.
...
\begin{equation}
 M=M_\Gamma\in\{0,1\}^{\E\times V},\qquad
 M_{Ev}=
 \begin{cases}
  1,&v\in E,\\
  0,&v\notin E,
...
Therefore, for every solution of $Mp=\one_\E$,
\[
...
\label{thm:forcing}
Suppose the real affine system $Mp=\one_\E$ is nonempty and let
...
\end{equation}
for every admissible weight on $\Gamma$.
\end{theorem}

\begin{proof}
If 2 holds and $Mp=\one_\E$, then
\[
 d^{\mathsf T}p
 =\lambda^{\mathsf T}Mp
 =\lambda^{\mathsf T}\one_\E
...
Conversely, choose one solution $p_0$ of $Mp=\one_\E$.  Every vector
$x\in\ker M$ gives another solution $p_0+x$.  By 1,
$d^{\mathsf T}(p_0+x)=d^{\mathsf T}p_0=0$, hence $d$ annihilates
$\ker M$.  Finite-dimensional linear algebra gives
$d\in(\ker M)^\perp=\operatorname{im}M^{\mathsf T}$, so
$d=M^{\mathsf T}\lambda$ for some real $\lambda$.  Evaluating at $p_0$
yields
...
side becomes
$\sum_v(M^{\mathsf T}\lambda)_vP_{h(v)}$, while the right-hand side is
$(\one^{\mathsf T}\lambda)I_H=0$.  Equation~\eqref{eq:certificate} gives
...
row reduction of
\begin{equation}
 \begin{pmatrix}M^{\mathsf T}\\ \one_\E^{\mathsf T}\end{pmatrix}\lambda
 =
...
\begin{equation}
 \mathcal C_\Gamma
 =M^{\mathsf T}\{\lambda\in\mathbb Q^{\E}:
                   \one^{\mathsf T}\lambda=0\}
 \label{eq:consequence-space}
\end{equation}
and search it for sparse vectors with entries in $\{-1,0,1\}$.  The positive
...
\[
 \one_V^{\mathsf T}M^{\mathsf T}\lambda
...
$v_{01},v_{12},\ldots,v_{70}$ its corners, with indices modulo eight.  The
...
\end{equation}
Thus every admissible weight satisfies
\begin{equation}
...
Ref.~\cite{navara-svozil-2026-complex-hypergraphs}.  It is therefore a
...
\]
and a generic rotation of it yield distinct decompositions of the same
...
$M^{\mathsf T}\lambda=\chi_A-\chi_B$,
$\one^{\mathsf T}\lambda=0$ is necessary and sufficient for precisely this
...
All context lists and certificates needed for the results are displayed in
the text.  Exact coordinates and faithfulness certificates for the octagon
are given in Ref.~\cite{navara-svozil-2026-complex-hypergraphs}.  Lean~4
sources checking the finite coordinate-operator identities discussed in
Section~\ref{sec:excludes} are available from the corresponding authors upon
reasonable request.
