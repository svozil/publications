...
\begin{abstract}
A fixed-width large language model (LLM) can contain far more approximately
distinct feature directions than the number of independent coordinates in its
hidden space.  This geometric coexistence does not make all those directions
reliably readable at the same time.  L\'evy concentration,
Johnson--Lindenstrauss embeddings, random spherical coding, and the Welch bound
make that distinction precise: at a fixed angular tolerance the number of
available directions can grow exponentially with the usable rank $d$, while a
simple readout from $k$ random-like active features accumulates interference on
the scale $\sqrt{k/d}$.  These are established mathematical benchmarks, not a
theorem about semantic capacity.

The chapter advances two positive, testable hypotheses.  First, training should
allocate lower cross-talk to important features that often activate together.
Second, occurrences of a lexically ambiguous token may be described by a tied
low-rank model: all senses reuse a token-associated subspace, while
sense-specific experts model residual variation.  Sharing the common part is
predicted to improve sample efficiency for rare or imbalanced senses, not to
increase expressive power.  The nontrivial test is an advantage over token-mean
and principal-component removal, unconstrained low-rank adapters, and
parameter-matched mixture-of-experts models on held-out data.

This is an analytical and model-proposal chapter; it reports no new experiment.
It supplies explicit failure criteria and a small pilot protocol.  The model is
``quantum-inspired'' only in its use of context-indexed projectors and optional
order-sensitive operators.  The proposed model uses ordinary real matrices on
classical hardware; an optional geometric comparison uses complex vectors.
\end{abstract}
