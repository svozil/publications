...
final appendix specifies a separate finite-dimensional unitary--trace
attention model with complex amplitudes, learned unitary evolution, optional
partial tracing, and phase-sensitive projective Born-rule predictions.
...
comparisons.  Appendix~\ref{app:quantum-attention} gives a separate
unitary--trace attention construction as a deliberately nonstandard
...
deliberately nonstandard unitary--trace construction that meets the listed
requirements only by changing the model class; it is not a reinterpretation
of ordinary transformer attention.
...
\section{A deliberately quantum extension: unitary--trace attention}
\label{app:quantum-attention}

The requirements stated in Sec.~\ref{sec:polysemy} can be met with a much
smaller construction, but only by changing the model class.
As a constructive boundary case, consider \emph{unitary--trace attention}.
It uses a finite-dimensional complex Hilbert space, learned unitary matrices,
projective Born readout, and an optional partial trace.  It is an attention
analogue, not a reinterpretation of a standard transformer.

Only two spaces are needed.  For each target $i$, the address space
$\mathcal H_A^{(i)}\simeq\mathbb C^i$ has orthonormal basis
$\{|j\rangle_A:j=1,\ldots,i\}$, and the semantic or value space
$\mathcal H_S\simeq\mathbb C^d$ has a complete orthonormal vocabulary basis
$\{|y\rangle_S:y=1,\ldots,d\}$.  The latter is an explicit simplification of
this toy model.  A partial trace requires a distinction between retained and
discarded systems, so this single tensor product
$\mathcal H_A^{(i)}\otimes\mathcal H_S$ cannot be removed.
...
\end{equation}
This is the entire unitary-plus-partial-trace version of incoherent
attention.  It contains no interference between different addresses.  In
