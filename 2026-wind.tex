...
\begin{align}
&D_{\rm down} + D_{\rm up} =\frac{r_3}{r_3-r_2}
-\frac{r_2}{r_3-r_2}
=1,
 \nonumber \\
&\text{ or } D_{\rm up}=1-D_{\rm down}
.
...
and the same gear radii used in the two inverted orientations.

The condition for genuine upwind amplification is
\[
|D_{\rm up}|>1.
\]
Using Eq.~\eqref{eq:upwind-v-vw-ratio}, this becomes
\[
\frac{r_2}{r_3-r_2}>1,
\]
or
...
it moves upwind faster than the wind only in the narrower range
\(r_2<r_3<2r_2\).  Equivalently, by
Eq.~\eqref{eq:upwind-downwind-symmetry}, an upwind speed exceeding the wind
speed corresponds to a downwind speed ratio satisfying \(D_{\rm down}>2\).
