...
continues to operate as a wheel-driven fan and accelerates this air rearward,
...
Again, propulsion arises only because the ground provides an external reaction surface that allows the drivetrain to exchange momentum between the two media.
...
boat is the laboratory frame in which the far-field (undisturbed) water is at
rest (neglecting currents). The flow in the vicinity of the hull/foils/propulsor
is generally nonzero due to induced velocities and wake formation, that is, the
creation of a trailing region of disturbed flow behind the hull or propulsor.
Along the downwind axis, the true wind speed is $v_w$, the boat speed through
the water is $v_b$, and the apparent-wind velocity is
\(
v_a = v_w - v_b
\).
...
\(\dot m\), which in practice depends strongly on its effective area (size) and
loading.
...
\begin{tikzpicture}[line cap=round,line join=round,>=Stealth,font=\small]
  ...
  \node[above] at (3.58,\yTop+0.2) {$v_i=-v_1$};
...
&$\qquad$&
\begin{tikzpicture}[line cap=round,line join=round,>=Stealth,font=\small]
  %\useasboundingbox (-3.2,-0.55) rectangle (8.2,6.95);
   \useasboundingbox (-3.2,-0.55) rectangle (5.2,6.95);
  ...
  % Upper rack (kept fixed)
  \draw[thick] (-3.0,\yTop) -- (4.9,\yTop);
  \node[above] at (-2,\yTop+0.2) {upper rack};
  \draw[->,thick] (2.8,\yTop) -- (4.35,\yTop);
  \node[above] at (3.58,\yTop+0.2) {$v_i=-v_{\mathrm{id}}$};
  ...
  % Labels (shifted with gear train)
  ...
  %\node[below right] at ({1.4+\xG},0.7) {$v_3=r_3\omega_3=r_3\omega_2$};
  ...
  % vf double arrow to the left (kept fixed and redrawn uniformly)
