...
\begin{lemma}\label{lem:chromatic-completeness}
Let $H$ be an $n$-uniform hypergraph.  A faithful orthogonal representation
of $H$ is chromatically complete if and only if $H$ admits a strong
$n$-coloring.
\end{lemma}

\begin{proof}
Suppose first that $\rho$ is chromatically complete.  By definition, there
exists a set $\Sigma$ of $n$ distinct spectral labels and a map
$c:V\to\Sigma$ such that $c|_e$ is a bijection for every context $e$.
Identifying the elements of $\Sigma$ with $n$ colors gives a strong
$n$-coloring, because no two vertices in the same context can receive the
same spectral value.

Conversely, suppose that $H$ admits a strong $n$-coloring.  Since every
context contains exactly $n$ vertices, injectivity of $c|_e$ implies
bijectivity onto the set of $n$ colors.  Replacing the color names by any
fixed set $\Sigma=\{\lambda_1,\ldots,\lambda_n\}$ of distinct eigenvalues
gives precisely the spectral assignment required in
Definition~\ref{def:chromatic-completeness}.
\end{proof}
...
\begin{enumerate}
    ...
    faithful FOR in $\C^3$;
    \item[(ii)] there are hypergraphs with $\chi(H)>3$ that possess an FOR
    in $\R^3$, and hypergraphs with $\chi(H)>3$ that possess no faithful FOR
    in $\C^3$.
\end{enumerate}
\end{proposition}

\begin{proof}
For $\chi(H)\leq 3$ with an FOR, take the single context
\[
    E=\{\{1,2,3\}\}.
\]
It has $\chi(H)=3$ and is represented by the standard basis of $\C^3$.

For $\chi(H)\leq 3$ without a faithful FOR, take
\[
...
For $\chi(H)>3$ with an FOR, use the completed Yu--Oh hypergraph described
in Sec.~\ref{sec:yuo}.  It has $\chi=4$ and an explicit faithful
representation in $\R^3$.

For $\chi(H)>3$ without an FOR, use Greechie's $G_{32}$ hypergraph.  Its
