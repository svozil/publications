...
environment that observes the trader's position can construct an anti-aligned
counterpath; this elementary online-game result refutes unrestricted pathwise
guarantees, not a realistic model of price formation. If market rules can
emulate universal computation, deciding their future price events would solve
the Halting Problem. At the passive pole, a Martin-L\"of-random record defeats
every effective fair-game betting test. No-arbitrage supplies a separate
financial boundary. Any defensible claim of repeatable success must therefore
state a restriction, distribution, benchmark, or information asymmetry. These
mechanisms have different quantifiers and are not interchangeable.
\end{abstract}
...
may mean a realized path, a probability law over paths, or a responsive rule
that observes the trader's action. This organizing picture is not a claim that
every financial model is literally a two-person zero-sum game.
...
need not outperform its financing benchmark. The universal claim has the
quantifier order
\begin{equation}
...
\end{equation}
whereas the adaptive counterpath establishes
\begin{equation}
...
Under the responsive timing used below, the trader announces long, short, or
flat before the environment chooses the next return. The environment can then
anti-align that return with the announced position. This is first of all an
elementary perfect-information counterstrategy. It is diagonal-style only in
the operational sense that the trader's own output is transformed into its
opposite effect. A trader that never takes a position---that is, does not trade
at all---earns zero rather than a strict loss; zero already refutes a guarantee
of strictly positive gain.

Universal computation creates a different obstruction. If admissible market
generators can simulate arbitrary Turing machines, a future price event can
encode whether a computation halts. A program that decided every such event
would decide the Halting Problem. Gold's identification-in-the-limit framework
and later work on unsolvable inductive inference place this explicit reduction
in a broader learning-theoretic setting
\cite{go-67,ad-91,angluin:83,2016-AlexanderKarlSvozil-ml,moore}.

At the passive pole, a binary price record may be Martin-L\"of random relative
to a specified computable fair measure. It then passes every effective null
test and permits no lower-semicomputable nonnegative fair-game capital process
to become unbounded. This does not exclude isolated correct forecasts or
finite lucky profits. No-arbitrage supplies yet another boundary, formulated
almost surely under a pricing measure rather than through an adversarial
counterpath or an effective null test.
...
distributional assumption absent from the unrestricted market class. A trend
...
not yet fully reflected in prices.
...
& \textbf{Conclusion} \\
\colrule
Adaptive counterpath
& Observes the current position before choosing the next return
& Pathwise worst case:
\(\forall\sigma\,\exists P_\sigma\)
& No strict-profit guarantee in this responsive class \\
Halting reduction
...
& Passive binary record under a specified computable fair measure
& Each Martin-L\"of-random path; such paths have measure one
& No effective fair-game capital process becomes unbounded \\
No-arbitrage
...
that the adaptive response is a realistic microstructure model or a newly
discovered minimax theorem. The mathematical study of unpredictable prices
...
\ref{sec:computability} gives the adaptive counterpath, two Halting reductions,
and the Martin-L\"of, computable, and Schnorr randomness hierarchy.
...
\subsection{A Responsive Market: The Adaptive Counterpath}
...
\begin{remark}[Interpretive Scope of the Adaptive Counterpath]
\label{rem:counterpath-scope}
Theorem~\ref{thm:counterpath} is a worst-case online-game result, not a model of
competitive price formation. Its environment observes the trader's current
position before selecting the next return. This timing is stronger than
ordinary market impact and should not be read as an empirical claim that
markets systematically move against every trade. The theorem's logical role
is narrower: an unrestricted responsive-environment class cannot support a
strict pathwise-profit guarantee. Applications to actual markets require
separate assumptions about timing, information, liquidity, price formation,
and admissibility.
\end{remark}

The proof is primarily an anti-alignment counterstrategy. It is diagonal-style
in the limited operational sense that the market takes the strategy's own
current output and reverses the intended effect of every nonzero position:
long is answered by down, short by up, and neutrality by zero. Unlike a formal machine
diagonalization, no enumeration of all programs is required. Its quantifiers are
...
This section supplies a falsification tool, not a further impossibility
theorem on the level of the Halting reductions or no-arbitrage. The
trader--market viewpoint suggests holding the trader fixed while transforming
the market record. Among computably specified stress transformations, time
reversal is perhaps the simplest, particularly for detecting dependence on
...
Here ``insight'' means an explicit restriction rather than a claim about a
trader's state of mind. A strategy specifies a market class and a mechanism
expected to persist in it, and the resulting claim is falsifiable outside that
class. A lucky finite gain needs no such account; a defensible claim of
repeatable edge, positive conditional expectation, or sustained benchmark
outperformance must state one.
...
the environment. Feedback may also be amplified when many systems share
signals, funding constraints, or risk controls; this is an economic
control-and-interaction mechanism, not a consequence of undecidability.
...
Everything returns to the same pair: a trader and a market. Under the strong
responsive timing of Theorem~\ref{thm:counterpath}, the market observes the
current position and anti-aligns the next return. This elementary
perfect-information construction establishes
\(\forall\sigma\,\exists\mathcal E_\sigma\), not an empirical law of price
formation. The always-flat trader earns zero, while a nonzero position incurs
a strict loss on the constructed path.

Other specifications of the market side give different boundaries. Universal
computation makes general profit and market-event decision undecidable even
under promises of totality. A passive Martin-L\"of-random record blocks
unbounded success by effective fair-game capital processes relative to its
computable reference measure. No-arbitrage excludes the standard almost-sure
zero-cost payoff, and uniform binary averaging fixes expected directional
accuracy at \(1/2\). These are distinct claims with distinct quantifiers.
...
useful strategies may exist, but no insight-free, zero-cost rule guarantees
strict profit against every admissible market.
