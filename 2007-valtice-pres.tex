...
\addfootboxtemplate{\color{structure!65}}{\color{white}\tiny \hfill Communication Cost $\ldots$\hfill}
\addfootboxtemplate{\color{structure!50}}{\color{white}\tiny \hfill CEQIP, Valtice, June 26, 2007\hfill}
...
\title{\bf \textcolor{blue}{Communication Cost of Breaking the Bell Barrier}}
...
\subtitle{\textcolor{orange!60}{\small http://tph.tuwien.ac.at/$\sim$svozil/publ/2007-valtice-pres.pdf}}
\author{Karl Svozil}
...
\date{CEQIP, Valtice, June 26, 2007}
\maketitle
...
\item<1->
Solving decision problems by  spreading out, processing and extracting information
from multipartite configurations via equibalanced state partitions.

\item<1->
Against contextuality: about the impossibility for simultaneous counterfactual inference
of more than two noncommuting observables.
\end{itemize}
...
\section{Quantum computing by state identification}



\subsection{Strategy: spread \& detect}
\frame{
\frametitle{Spread of information ``across'' the quanta in multipartite states}
\begin{itemize}
\item<1->
...
\item<1->
There exist quantum state identification problems
based on the proper partitioning of mutually orthogonal sets of states yielding
``efficient'' computations which identify ``global, holistic'' properties
without evaluating the states of the individual quanta.
...
Can one always find an efficient quantum oracle for equibalanced partitions?

\end{itemize}
 }


\subsection{Example: generalized Deutsch Algorithm}
\frame{
\frametitle{Phases from the phase oracle $f_{ij}(x,y)=f_i(x)+f_j(y); \quad 0\le i,j\le 3$}
{\scriptsize
\centerline{
...
\frame{
\frametitle{Partitions corresponding to these decision problems}
\begin{itemize}
\item<1->D1:
$$
\left\{
...
\frame{
The strategy is to find the unitary transform
$$
...
\subsection{Counterexample: parity}

\frame{
\frametitle{Counterexample: parity}
\centerline{
...
$\;$\\
$\;$\\
Number of even or odd $f_i(x,y)=1$'s:
$$
\left\{
\left\{
     f_{  0} ,
