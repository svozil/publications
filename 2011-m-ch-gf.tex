...
This chapter marks the beginning of a series of chapters dealing with the solution to differential equations of theoretical physics.
\index{differential equation}
Very often, these differential equations are {\em linear}; that is, the ``sought after'' function $\Psi (x), y(x), \phi (t)$ {\it et cetera}
...
Green's function present a very elegant way of solving linear differential equations
of the form
\begin{equation}
\begin{array}  {l}
...
\label{2011-m-egfp1}
\end{equation}
This claim, as posted in Eq. (\ref{2011-m-egfp1}), can be verified by explicitly applying the differential operator  ${\cal L}_x$
to the solution $y(x)$,
\begin{equation}
\begin{array}  {l}
{\cal L}_x y(x)\\
...
\begin{eqnarray*}
   ...
   \left({d^2\over dx^2}-1\right)\Theta(x-x')\sinh(x-x')&
   \stackrel{?}{=}&\delta(x-x')
\end{eqnarray*}
...
$$
   ...
   +\Theta(x-x')\cosh(x-x')\right)-\Theta(x-x')\sinh(x-x')=
$$
$$
   \delta(x-x')\cosh(x-x')+\Theta(x-x')\sinh(x-x')-\Theta(x-x')\sinh(x-x')=
   \delta(x-x').
$$
\eexample
}
...
Note that the general solution to (\ref{2011-m-egfp}) can be found
...
differ only by a function which is a solution tho the homogenuous differential equation (\ref{2011-m-egfp0}), because
...
in Eq. (\ref{2011-m-egfp}) are constants, and thus {\em translational invariant}.
Then the entire {\it Ansatz} (\ref{2011-m-egfp01})
...
represents an excellent way of
...
G(x+\xi ,x'+\xi )= G(x,x').
\end{equation}
Now set $\xi = - x'$; then we can define a new green's functions which just depends on
...
What is important for applications is the possibility to adapt the solutions of some inhomogenuous differential equation
...
The sum in Eq. (\ref{2011-m-egfcr1}) stands for an integral in the case of contiuous spectrum of  ${\cal L}_x $.
...
\color{blue}
\bexample
Take, for instance the differential equation corresponding to the harmonic oscillator
\begin{equation}
{\cal L}_t = \frac{d^2}{dt^2} + k^2,
\end{equation}
with $\omega \in {\Bbb R}$.
Then the associated eigenfunctions are
\begin{equation}
\psi_k (t) = e^{-ik t},
\end{equation}
with $\infty \le k\le \infty$.
Taking the complex conjugate and integration over $k$ yields
\begin{equation}
\begin{array}{l}
\int_{-\infty}^{\infty} \psi_k (t)\overline{\psi_k (t')} dk \\
\quad =
\int_{-\infty}^{\infty}  e^{-ik t} e^{ ik t'} dk \\
\quad =
\int_{-\infty}^{\infty}  e^{-ik (t-t')} dk \\
\quad =
\delta (t-t').
\end{array}
\end{equation}
...
Then the Green's function of ${\cal L}_x$
can be written as the spectral sum of the absolute squares of the eigenfunctions,
devided by the eigenvalues $\lambda_j$; that is,
\begin{equation}
...
{\color{OliveGreen}
\bproof
For the sake of proof, apply  the differential operator  ${\cal L}_x$  to the Greens function {\em Ansatz}
...
then we can apply the following strategy using Fourier analysis to obtain the Green's function.
...
is just a constant; with our definition unity. Then, $\delta$ can be written as
\begin{equation}
\delta (x-x')=
\frac{1}{2\pi}
...
=
0,\\
\widetilde{G}(k) {\cal L}_k    -1 =0,\\
\widetilde{G}(k) = \left({\cal L}_k\right)^{-1},
\end{array}
...
by $ik$ in the former.
in that way, the Fourier transform  $\widetilde{G}(k) $ is obtained as a polynomial of degree $n$,
...
\put(924,733){\makebox(0,0)[l]{region $t-t'>0$}}
\put(924,122){\makebox(0,0)[l]{region $t-t'<0$}}
...
\begin{eqnarray*}
  \mbox{residuum theorem:}\ G_1(t,t')&=&0\qquad\mbox{for}
      \quad t>t'\\
   G_1(t,t')&=&-2\pi i\,{\rm Res}\,\left({1\over2\pi i}
      {e^{ik(t-t')}\over(k+i)};-i\right)=\\
   &=&-e^{t-t'}\quad\mbox{for}\ t<t'.
\end{eqnarray*}
Hence we obtain a Green's function for the inhomogenuous differential equation
$$
G_1(t,t')=-\Theta(t'-t)e^{t-t'}
$$
However, this Green's function and its associated (special) solution does
not obey the boundary conditions
$G_1(0,t')=-\Theta(t')e^{-t'}\ne0$ for $t'\in[0,\infty)$.

Therefore, we have to fit the Greens function by adding an appropriately weighted solution to the homogenuos differential equation.
...
with the {\em Ansatz}
$$
G(0,t')=G_1(0,t')+G_0(0,t';a)=-\Theta(t')e^{-t'}+ae^{-t'}  $$
...
that is,
$$
G(t,t')=\bigl[1-\Theta(t'-t)\bigr]    e^{t-t'}.
$$
Since $\Theta(-x)=1-\Theta(x)$, $G(t,t')$ can be rewritten as
$$
...
\begin{eqnarray*}
   y(t)&=&\int\limits_0^\infty \underbrace{\Theta(t-t')}_
...
ant thus
$$\tilde G(k)=
   {1\over(1-k^2)}={-1\over(k+1)(k-1)}
$$
The Fourier transformation is
\begin{eqnarray*}
    G_1(t,t')&=&-{1\over2\pi}\int\limits_{-\infty}^{+\infty}
                    {e^{ik(t-t')}\over(k+1)(k-1)}dk=\\
                 &=&-{1\over 2\pi}2\pi i\Biggl[\,{\rm Res}\left(
                    {e^{ik(t-t')}\over(k+1)(k-1)};k=1\right)+\\
                 & &\qquad\qquad\ {\rm Res}\left({e^{ik(t-t')}
                    \over(k+1)(k-1)};k=-1\right)\Biggr]\Theta(t-t')
\end{eqnarray*}
...
\end{marginfigure}
\begin{eqnarray*}
   G_1(t,t')&=&-{i\over2}\left(e^{i(t-t')}-e^{-i(t-t')}\right)\Theta(t-t')=\\
            &=&{e^{i(t-t')}-e^{-i(t-t')}\over2i}\Theta(t-t')=
               \sin(t-t')\Theta(t-t')\\
   G_1(0,t')&=&\sin(-t')\Theta(-t')=0\qquad\mbox{weil}\quad t'>0\\
   G_1'(t,t')&=&\cos(t-t')\Theta(t-t')+\underbrace{\sin(t-t')\delta(t-t')}_
                {\mbox{$=0$}}\\
   G_1'(0,t')&=&\cos(-t')\Theta(-t')=0\qquad\mbox{wie vorher}
\end{eqnarray*}
...
\begin{eqnarray*}
   y(t)&=&\int\limits_0^\infty G(t,t')f(t')dt'=
          \int\limits_0^\infty \sin(t-t')\underbrace{\Theta (t-t')}_
       ...
       &=&\sin t\int\limits_0^t(\cos t')^2dt'-\cos t\int\limits_0^t
          sin t'\cos t'dt'=\\
       &=&\sin t\left[{1\over2}(t'+\sin t'\cos t')\right]\Bigr|_0^t-
          \cos t\left[{\sin^2 t'\over2}\right]\Bigr|_0^t=\\
