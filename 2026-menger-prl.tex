...
% --- Substratum band (top strip, y in [1.0, 1.5]) -------------------------
\foreach \y in {1.1, 1.3, 1.5}{
  \draw[gray!65, thin, decorate,
        decoration={snake, amplitude=1.2pt, segment length=5pt}]
    (-3.9,\y) -- (3.8,\y);
}
% Flow arrows within the substratum band: left-to-right (toward M_+)
\foreach \x in {-2.8, -0.3, 2.2}{
  \draw[->, gray!75, thin] (\x,1.3) ++(-0.35,0) -- ++(0.7,0);
}
\node[gray!85, font=\scriptsize, anchor=west] at (-3.95, 1.75)
  {substratum $\rho_0$ (flow $\rightarrow$)};

% --- Hole ----------------------------------------------------------------
\draw[thick, dashed] (-2.2,0) circle (0.55);
\node[font=\scriptsize, anchor=north] at (-2.2,-0.65)
  {hole, $M_{\text{eff}}<0$};

% --- Positive mass -------------------------------------------------------
\fill (2.5,0) circle (0.10);
\node[font=\scriptsize, anchor=north] at (2.5,-0.18) {$M_+$};

% --- Separation indicator (thin dashed line with n-hat) ------------------
\draw[gray!70, thin, dashed] (-1.55,0) -- (2.35,0);
\node[black!75, font=\footnotesize] at (0.4, 0.25) {$\hat{\mathbf{n}}$};

% --- Acceleration arrows (bold, colored) ---------------------------------
% a_hole: points left (away from M_+), starts left-of-hole
\draw[->, very thick, red!70!black] (-2.85,0) -- (-3.95,0);
\node[red!70!black, font=\scriptsize, anchor=south]
  at (-3.40, 0.10) {$\mathbf{a}_{\text{hole}}$};

% a_{M+}: points right (away from hole), starts right-of-M+
\draw[->, very thick, blue!70!black] (2.65,0) -- (3.75,0);
\node[blue!70!black, font=\scriptsize, anchor=south]
  at (3.20, 0.10) {$\mathbf{a}_{M_+}$};
\end{tikzpicture}
\caption{Archimedean repulsion. The substratum (wavy band) flows gravitationally toward $M_+$ with $\hat{\mathbf{n}}$ the unit vector from hole to $M_+$. The hole, as a buoyant absence in this flow, is displaced in the $-\hat{\mathbf{n}}$ direction, $\mathbf{a}_{\text{hole}}$ [Eq.~\eqref{eq:ahole}]. Simultaneously, $M_+$ is gravitationally repelled along $+\hat{\mathbf{n}}$ by the hole's effective negative source, $\mathbf{a}_{M_+}$ [Eq.~\eqref{eq:aMplus}]. Both accelerations point away from the other body; the pair separates rather than running away in a common direction.}
