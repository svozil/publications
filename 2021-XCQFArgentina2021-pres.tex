...
\title{\bf \textcolor{blue}{Strong contextuality by \\non-faithful emeddability}}
\subtitle{\url{http://tph.tuwien.ac.at/~svozil/publ/2021-XCQFArgentina2021-pres.pdf}}
\author{Karl Svozil}
...
\date{Online presentation, Xth Conference on Quantum Foundations Wednesday, November 30, 2021}
\maketitle
...
\section{Preamble}
 ...
 \frame{
 \frametitle{Preamble}
{\color{blue}What I shall present here is based on}
\begin{itemize}
\item[Q]
{\color{purple}
quantum mechanical entities being formalized by entities of Hilbert space.
In particular, pure states are identified with unit vectors, maximal observables aka ``contexts'' are identified with orthonormal bases.}
\pause
\item[C]
{\color{purple}
In contradistinction, classical entities are based on Boolean algebras, and classical probabilities are based on convex combinations of
``extreme'' cases identified with two-valued states on them.}
\pause
\item[M]
{\color{purple}
Metaphysical conjecture/working hypothesis: Any measurement ``creates''---``carves out'' an ``emergent property'' that cannot be classically ``pre-existent''
relative to the presumtions (eg, non-contextuality).}
\end{itemize}
}

\section{Three main varieties of (quantum) contextuality}


 \frame{
 \frametitle{Three main varieties of (quantum) contextuality: {\#}1 \& {\#}2}


\begin{itemize}
\item[1.] {\color{purple} Bohr \& Heisenberg forms based on complementarity and measurement uncertainty/nesting; cf. Khrennikov, Jaeger, ...;}
\pause
\item[2.]  {\color{purple} nonclassical probability distributions such as of Born-Gleason-Lov{\'a}sz-type,
and forms derived from generalizations of Cauchy-type functional equations yielding violations of classical predictions;}
{\color{blue} these forms are both quasi--``empirical'' through counterfactuals \& stochastic (Specker 1960):}
\pause
\begin{itemize}
\item[2.1.] {\color{blue}  Boole-Bell type inequalities discussed by Bell, Froissart, Pitowsky, Tsirelson, CHSH, Suppes-Zanotti, Cabello, ... ;}
\pause
\item[2.2.] {\color{blue}  based on gadget graphs with input/output terminals---aka pre-/postselection of pure quantum states:
(Kochen-)Specker bug (1965, aka Hardy-type, cf Stigler's law of eponymy), Belinfante, Stairs, Cabello, ...;}
\end{itemize}
\end{itemize}
}

 \frame{
 \frametitle{Three main varieties of (quantum) contextuality: {\#}3}


\begin{itemize}
\item[3.] {\color{purple} nonempirical / logical / algebraic / theoretical / counterfactual  structure of observables with nonclassical interpretations:}
\pause
\begin{itemize}
\item[3.1.]\colorbox{yellow!40}{\begin{minipage}{.8\textwidth} \color{blue}
\bf \large nonfaithful embedding into Boolean algebras associated with inseparability,
nonunital value assignments, and other nonclassical properties;\end{minipage}
}
\pause
\item[3.2.]  {\color{blue}  nonexistence of any classical interpretation aka two-valued (even partial) states: Gleason, Specker,
Zierler-Schlessinger, Kamber, Kochen-Specker, Pitowsky, Hrushovski-Pitowsky, Cabello, Abbot-Calude-Svozil ...; }
\end{itemize}
\end{itemize}
}

\section{Note on nonclassical probability distributions}


\frame{
\frametitle{A brief note on nonclassical probability distributions}
\begin{itemize}
...
different probability distributions on fixed collections of (interwined)observables ...}}
\end{center}
...
\centerline{\Large {\color{applegreen}\decofourleft \hspace{.15cm} {\it interlude} \hspace{.15cm} \decofourright}}
                                                    ...
                                                    \vspace{1.15cm}
\begin{center}
{\large {\color{blue} ... now we shall be talking about \\``weird'' nonclassical collections of (interwined)observables ...}}
\end{center}
...
based on Erna Clavadetscher-Seeberger, Diss. ETH Z\"ürich (Specker)
