...
distinguishable. The useful resource for the query separations considered
here is consequently the structure of the oracle response---its phases,
spectrum, accessible registers, and possible environment---rather than simply
the formal number of branches in a superposition. Rewriting an oracle by
fixed basis changes preserves this discrimination, whereas changing its
response representation, aggregating
...
The relevant query resource is this rate of separation, not the number of
formal components in the probe superposition.

These query separations need not cross the stabilizer boundary. The standard
Boolean Deutsch and linear Bernstein--Vazirani protocols, including their
corresponding XOR oracles, use only Clifford operations; the witness coupling
in Eq.~\eqref{eq:witness-unitary} is likewise Clifford before the witness is
discarded. They are therefore efficiently simulable when a full classical
circuit description is supplied~\cite{GottesmanKnill,AaronsonGottesman2004}.
In the query model, however, such a description would reveal hidden oracle
data and is stronger than matched black-box value access. Stabilizer
simulability and classical oracle-query complexity are therefore distinct:
nonstabilizer ``magic'' is not necessary for a quantum query
advantage~\cite{Veitch2014}.
