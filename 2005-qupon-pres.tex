...
\title{\bf Contextuality, uniqueness property \& quantum computing}
...
\subtitle{http://tph.tuwien.ac.at/$\sim$svozil/publ/2005-qupon-pres.pdf}
\author{Karl Svozil}
...
K.S., JPA, in press; quant-ph/0401113; K.S., AIP Conference Proceedings 750,quant-ph/0406014
\begin{itemize}
\item<+->
...
(Kamber, Alda, Zierler\&Schlesinger, Bell-type, ...)
express the ``scarcity'' or even nonexistence
of (dispersion free) two-valued states interpretable as classical truth assignments
...
\frame[shrink=2]{
\frametitle{Simplest example: EPR-type setup}
A {\em context} is defined as a single (nondegenerate)
``maximal'' self-adjoint operator.
Take five observables $A,B,C,D,K$ with two systems
of {\em contexts}
$\{A,B,C\}$
and
$\{D,K,A\}$,
...
b) Greechie (orthogonality) diagram;
c) Tkadlec diagram.
\label{2004-qnc-f1}}
\end{figure}
 }
...
quant-ph/0206076\\
In ``simultaneous'' measurements, all single measurements
...
\item<+->
Example: take the two four spin--1/2 singlet states
\begin{eqnarray}
...
Question:
Is it possible
to encode equibalanced decision problems (e.g., Deutsch's problem and Parity)
...
\item<+->
Deutsch's problem OK, Parity not OK, Recursion \& interation (not OK?)
\end{itemize}
 }
