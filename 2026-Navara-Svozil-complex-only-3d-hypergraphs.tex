...
\begin{abstract}
We give two finite three-dimensional orthogonality hypergraphs that possess a
faithful orthogonal representation (FOR) over $\C^3$ but no FOR over $\R^3$.
The first has 20 vertices and 12 contexts.  A pair of complementary context
coverings forces equality of two rank-one-projector sums; an exact algebraic
complex representation realizes two distinct nonorthogonal decompositions of
that operator, whereas a real two-vector decomposition is rigid.  The second
hypergraph adds one vertex and two contexts.  It has an exact 21-ray complex
representation at the orthogonal limit, while a reduced real closure system
leads to a strictly positive expression that would have to vanish.  Exact
overlap certificates verify faithfulness of both complex representations.
Phase-adjusted realification also gives explicit faithful representations of
both hypergraphs in $\R^6$, where the three-ray contexts cease to be maximal.
The separation is therefore dimension-specific and does not rely on a
...
\end{align*}
Here, with $t=|m|$,
$2M-|m|=2t^2-t+2=2(t-1/4)^2+15/8>0$.
This contradicts
...
Choose the phases inductively.  Having fixed
$\theta_1,\ldots,\theta_{m-1}$, every nonorthogonal pair $(k,m)$ excludes at
most two values of $\theta_m$ modulo $2\pi$.  Their union is finite, so a
phase outside it exists.  Continuing through $m=N$ makes the real dot product
nonzero for every originally nonorthogonal pair, while all original
orthogonalities remain zero.
...
We thank Josef Tkadlec for computational checks of finite context
structures and Ad\'{a}n Cabello and Mladen Pavi\v{c}i\'{c} for helpful
communications.  This research was funded in part by the Austrian
