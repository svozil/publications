...
some particular (finite) collection of propositions under ``mild'' side conditions.
In particular, the theorem presupposes
\begin{itemize}
\item[(i)]
the physical existence of at least a particular finite set of observables --
due to complementarity, many of them may not be simultaneously co-measurable -- in the following way:
\item[(ii)]
with respect to truth assignments and statistics (e.g., probabilities and expectation values),
any subset of mutually  co-measurable (i.e., formally, mutually commuting) observables
{\em within} the entirety of observables considered in Kochen-Specker type arguments
-- and, in particular, any ``complete'' set of yes-no propositions
represented by mutually orthogonal projectors spanning the Hilbert space
(or, equivalently to a single maximal operator) --
should behave {\em quasi-classically} (an assumption which leads to Gleason's theorem;
i.e., to the derivation of the Born rule for computing quantum probabilities),
and
\item[(iii)]
...
This is often referred to as the ``non-contextuality assumption.''
\end{itemize}
...
In what follows we shall consider the actual
{\em location of breakdown of classicality} within the set of Kochen-Specker observables;
that is, we would like to identify the observables for which classicality
inadvertedly renders complete contradictions;
...
one of the presuppositions (i)--(iii) must be abandoned.

We shall also consider certain collections of observables which do not render Kochen-Specker contradictions.
Restricting ourselves to these very limited collections would allow maintenence of assumptions (i)--(iii)
about quantized systems; alas this would reduce the domain of conceivable observables dramatically.

The results presented can be interpreted as one natural consequence of
and advancement beyond the Kochen-Specker theorem.
They may be particular important if we would like to investigate the concrete ``underpinning'' of the Kochen-Specker theorem;
and exactly why and where a quantized system disobeys the classical rules.

Still, there is also a concrete application which profits from such quantum information theoretic findings.
Qontemporary quantum random number generators can no longer be based upon and certified by
...
\bibitem [{Note3()}]{Note3}%
  \BibitemOpen
  \bibinfo {note} {Observables are represented by circles, contexts by smooth
  line segments.}\BibitemShut {Stop}%
...
\bibitem [{\citenamefont {Abbott}\ and\ \citenamefont
  ...
  \bibnamefont {Calude}},\ }\href@noop {} {\ }\Eprint
  {http://arxiv.org/abs/{CDMTCS}-392} {{CDMTCS}-392} \BibitemShut {NoStop}%
\bibitem [{\citenamefont {Merali}(2010)}]{Merali:2014aa}%
  ...
  10.1038/news.2010.181},\ \bibinfo {note} {published online. URL:
  http://dx.doi.org/10.1038/news.2010.181}\BibitemShut {NoStop}%
