...
\title{\bf \textcolor{blue}{Present Situation in Quantum Mechanics}}
\subtitle{\textcolor{blue}Very Few Answers, Many Questions}
\subtitle{\textcolor{orange!60}{\small http://tph.tuwien.ac.at/$\sim$svozil/publ/2012-WTCS2012-pres.pdf
%\\
%http://arxiv.org/abs/quant-ph/0503229v4
}}
\author{Karl Svozil}
\institute{Institut for Theoretical Physics, University of Technology Vienna, \\
...
\date{Auckland, Feb. 23rd, 2012}
\maketitle



\begin{frame}[fragile]
\frametitle{Schrödinger, E. (1935a). Die gegenwärtige Situation in der Quantenmechanik.
Naturwissenschaften, 23, 807--812, 823--828,
844--849. English translation in Ref. Trimmer (1980) and in
Ref. (Wheeler \& Zurek, 1983, pp. 152--167). \\
...
\centerline{ }
\centerline{\Large  \color{blue} Are there quantum  measurements?}
\centerline{\Large  \color{blue} Can there be quantum irreversibility?}
\centerline{\Large  \color{blue} Can there be quantum acausality?}
...
\begin{frame}[fragile]
\frametitle{Max Born (1926) \\
http://dx.doi.org/10.1007/BF01397184}

\begin{columns}
\begin{column}{7cm}
{\color{blue}
{ \it   $\ldots$ the motion of [[individual]] particles conforms to the laws of probability,
but the probability itself is propagated in accordance
with the law of causality. [This means that knowledge of
a state in all points in a given time determines the distribution
of the state at all later times.]
}}

2-tier quantum evolution:   \\
*)  {\color{magenta} Everett Process 2: unitary, isometric, one-two-one evolution of the quantum wave function inbetween measurements;}\\
and \\
*) {\color{magenta} Everett Process 1: many-to-one state reduction (wave function collapse) upon measurement of single quanta.}
\end{column}
\begin{column}{4cm}
\begin{overprint}
\includegraphics[width=50mm]{2012-WTCS2012-pres-465px-Max_Born.jpg} \\
\end{overprint}
\end{column}
\end{columns}
...
\begin{frame}[fragile]
\frametitle{The young Schrödinger's uneasyness with Everett Process 2 and the resulting quantum coherence expressed in the aforementioned papers in 1935}

\begin{columns}
\begin{column}{7cm}
The  Cat Paradox \&
Schrödinger's uneasyness with the coherent superposition of classically mutually exclusive states:
how would you experience
\\
*)
{\color{magenta}
a ``macroscopic'' superposition between death \& life}; \\
or, alternatively,   \\
*)
{\color{magenta} what would you experience while going through a double slit?!}  \\
$\;$\\
The production \& maintainance of macroscopic superpositions can possibly be assumed to be impossible
 ``for all practical purposes'' (fapp, John Bell 1990),
but there is no fundamental principle ruling them out.
\end{column}
\begin{column}{4cm}
\begin{overprint}
\includegraphics[width=55.0mm]{2012-WTCS2012-pres-ski_interferenz.jpg}
\end{overprint}
\end{column}
\end{columns}
...
\begin{frame}[fragile]
\frametitle{Schrödinger's Legacy: the  1949-1955  Dublin seminars}

\begin{columns}
\begin{column}{7.5cm}
{\color{blue}
{ \it  The idea that  [the alternate measurement outcomes] be not alternatives but {\em all} really happening simultaneously
seems lunatic to [the quantum theorist], just {\em impossible.}
He thinks that if the laws of nature took {\em this} form {\color{magenta} [[Everett Process 2]]} for,
let me say,
...
jellification only by our perceiving or observing it {\color{magenta} [[Everett Process 1]]}.
}
}
\end{column}
\begin{column}{4cm}
\begin{overprint}
\includegraphics[width=50mm]{2012-WTCS2012-pres-Schroedinger2.jpg} \\
\end{overprint}
\end{column}
\end{columns}
...
\begin{frame}[fragile]
\frametitle{Hugh Everett (1957):  Can there be a Cartesian  cut?\\
http://dx.doi.org/10.1103/RevModPhys.29.454}

\begin{columns}
\begin{column}{7cm}
{\color{blue}
{ \it   Consider for example an
...
Process 2 [[unitary transformation, isometry, one-to-one]]?
If so, then it would appear that no discontinuous probabilistic process like Process 1  [[measurement, many-to-one]] can take
...
\begin{column}{4cm}
\begin{overprint}
\includegraphics[width=50mm]{2012-WTCS2012-pres-Everett-image-01-large.jpg} \\
\end{overprint}
\end{column}
\end{columns}
...
\begin{frame}[fragile]
\frametitle{New experimental input: quantum measurements can be undone, so they are reversible; i.e., one-to-one, and not irreversible, i.e., many-to-one }


\begin{itemize}

\item<+->   quantum erasure experiments
(Peres, 1980; Scully
\& Drühl, 1982;  Scully et al., 1991; Zajonc
et al., 1991; Kwiat et al., 1992; Pfau et al., 1994; Chapman et al.,
1995; Herzog et al., 1995; $\ldots$)

\item<+->   haunted measurements   (Greenberger \& YaSin, 1989)
...
\begin{frame}[fragile]
\frametitle{Consequences of reversibility for Quantum Random Number Generators}

\begin{itemize}

\item<+->
Quantum components of quantum random number generators -- including beam splitters --
are, essentially,  {\color{magenta}one-to-one}; i.e., reversible, and in this sense {\color{magenta}causal},
 with corresponding unitary operator representations.

For instance, two beam splitters in serial composition yield a Mach-Zender interferometer rendering
the initial state; that is, the identity evolution.

\item<+->
But then, how comes the randomness about?!

\item<+->
And exactly where and when does randomness emerge?

\item<+->
Can we trust quantum physical random number oracles based on reversible (state) evolution?

\item<+->
Cf also Wigner's Friend (1961).
\end{itemize}
\end{frame}

\begin{frame}[fragile]
\frametitle{One possible conceptual solution: Context translation principle}
 \begin{itemize}

\item<+->
The context translation principle introduces
stochasticity and noise through translation of the preparation into the measurement.

\item<+->
But this noise, although fapp irreversibly generated, again is subject to principal reversibility, so only epistemic and not ontic in nature.
\end{itemize}
...
\centerline{ }
\centerline{\Large  \color{blue} What constitutes a quantum state?}
...
\end{center}
 ...
 \begin{frame}[fragile]
\frametitle{Motivation from probability theory}

\begin{itemize}

\item<+->
Bell-type theorems relating to classical correlations polytopes suggest
that quantum probabilities are {\color{magenta}incompatible} with classical ones
--
in the sense that the quantum probabilities violate Boole's {\em condition of physical experience}.
They are, in a certain, weak, sense, empirically falsifyable, and have been tested.



\item<+->
Greenberger-Horne-Zeilinger theorems suggest that quantum predictions strictly (not only statistically)
{\color{magenta}violate classical predictions}. They are, in a certain, weak, sense, empirically falsifyable, and have been tested.

\item<+->
Kochen-Specher-type theorems imply that there {\color{magenta}does not exist any global truth assignment} ($=$ two-valued measures) on certain finite sets of quantum observables.
...
\end{frame}
 ...
 \begin{frame}[fragile]
\frametitle{What kind of probabilities for different nonboolean algebraic structures?}

Requirement:  The probabilities for quasi-classical subalgebras should be quasi-classical (Boolean).

\begin{itemize}

\item<+->
For Hilbert lattices of dimension two (``decaying, unconnected'' subalgebras) the problem is unsolved.



\item<+->
For finite dimensional Hilbert lattices of dimension three onwards (``interconnected'' subalgebras),
Gleason's theorem identifies the probability measures to be the  quantum probabilities governed by the Born rule axiom.

\item<+->
For finite algebraic structures (such as Wright's pentagram) there exist exotic probability measure which are neither classical nor quantum.

\end{itemize}

All connections to physics are merely hypothetical. The Born rule is falsifyable and has been tested.
...
\end{frame}
 ...
 \begin{frame}[fragile]
\frametitle{What constitues or defines a physical state?}

It is suggested here to define a physical state -- different from the usual quantum pure state identified with a (unit) vector or the subspace spanned by that vector or the associated projector --
{\it via} a single unique {\color{magenta}context}.

Contexts represent {\color{magenta}maximal sets of comeasurable} observables, corresponding to {\color{magenta}maximal observables} of the von Neumann (1931)  and Kochen-Specker (1967) type
[e.g.,Halmos, Finite-Dimensional Vector Spaces,
1974, sect. 84].
...
\begin{frame}[fragile]
\frametitle{Speculations \& conjectures}

\begin{itemize}

\item<+->
Physical state is defined by a particular
{\em \color{magenta} maximal} set of mutually orthogonal vectors $=$ a context.

Notice that, in twodimensional Hilbert spaces, fixing a vector fixes the context completely.
Not so in higher dimensions!

\item<+->
Mixed states are merely {\color{magenta}epistemic constructions},
marking our inability to characterize the physical situation completely.

\item<+->
Ontologically, at any time, the quantized system is in a particular, {\color{magenta}single} context.

Note that its specification in terms of location in Hilbert space may require
an infinite amount of (algorithmic) information.
...
\end{frame}
 ...
 \begin{frame}[fragile]
\frametitle{What if you force a physical system to answer questions it has not been prepared for?}
...
\item<+->
It may act like a typical Viennese asked by a toureist where the city centre and St. Stephen's Cathedral is:
it gives a random, arbitrary answer ;-)
...
\item<+->
In the case of the Viennese it may be internal confusion.

\item<+->
In the case of the quantum it may be external resources, such as the ``macroscopically many'' degrees of freedom of the measurement apparatus.
\end{itemize}
...
\centerline{ }
\centerline{\Large  \color{blue} Quantum Entanglement}
...
\item<+->
Why do the quantum correlations are not violating the classical bounds stronger?



\item<+->
How come there is peaceful existence (Shimony) between relativity and quantum theory despite these ``nonlocalities?''
...
\centerline{\Large Finally}
\centerline{ }
\centerline{\Large  \bf\color{red} Happy Birthday, Luminita!!!!}
\centerline{ }
\centerline{\Large  \color{blue} $\ldots$ in anticipation:}
\centerline{ }
\centerline{\Large  \bf\color{red} Happy Birthday, Cris!!!!}
