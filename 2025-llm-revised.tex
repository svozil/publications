...
\title{Perspectives on Large Language Models: Polysemy, Stochasticity, Exponential Expressibility, and a Quantum-Attention Analogue}
...
\begin{abstract}
This paper explores foundational aspects of Large Language Models (LLMs). We analyze how the expressibility of semantic features scales exponentially with embedding-space dimension through quasi-orthogonal vectors. We contrast the dynamic, context-dependent embeddings of Transformer architectures, which resolve polysemy, with a static vector approach inspired by quantum contextuality. Stochasticity is framed as an essential feature of probabilistic generation, enabling flexible and creative output while also imposing characteristic limitations. Finally, we discuss a speculative quantum-attention analogue, asking what would have to change if attention were constrained to unitary evolution and terminal measurement rather than nonlinear softmax mixing.
\end{abstract}
...
Finally, Sections~\ref{section:qm} and~\ref{section:quantum_attention} draw a deliberately analogical comparison to quantum mechanics and discuss a speculative quantum-attention analogue. The point is not to identify present LLMs with quantum systems, but to use the shared mathematical language of vector spaces, inner products, and context-dependent projections to clarify both the analogy and its limits.
...
Tokens are the discrete interface to the model's internal vectorial representations; subsequent processing acts on these vectors rather than on characters or words directly.
...
where input and output tokens are \textit{mapped to vectors}, amounting to a vector embedding in a space whose dimension is much smaller than the vocabulary size but still high-dimensional in the geometric sense relevant below.
...
At scale, this repeated application of calculus and linear algebra produces structured vector representations that support syntactic, semantic, and pragmatic regularities in language.
...
The core mechanism enabling this behavior is the self-attention layer within the Transformer architecture. Self-attention allows the model to weigh the importance of all tokens in the preceding sequence---including its own generated output---at each computational step. This process is enhanced by multi-head attention, which permits the model to track disparate linguistic and logical relationships simultaneously in parallel subspaces. Therefore, while self-attention provides the mechanical capability for in-context reflection, this behavior is fundamentally a learned stylistic and computational pattern. The same mechanism is one route by which autoregressive models can approximate nontrivial algorithmic computation.
...
\begin{equation}
  \Pr\bigl(|\bm{u}\cdot\bm{x}|\geq \varepsilon\bigr)
...
In trained representations the raw dimension $d$ is often not the correct
quantity: anisotropy, low intrinsic dimension, and narrow activation cones
reduce the concentration dimension. We therefore write $d_{\mathrm{eff}}$ for
the effective dimension controlling the approximately isotropic part of the
representation being probed~\cite{svozil-2026-llm}.
...
in one case the (maximal) observable, in its spectral decomposition with arbitrary real (mutually distinct if maximal) eigenvalues $\lambda_i$, is:
\begin{align}
...
\end{pmatrix}.
\end{align}
For the second basis, with eigenvalues $\mu_i$, set
\begin{equation}
 a=\frac{\mu_1+\mu_2}{2},\qquad b=\frac{\mu_2-\mu_1}{2}.
\end{equation}
Then the observable is
\begin{align}
...
they determine which sense of a polysemous token is operationally accessible.
This suggests a possible architecture in which context bases, or basis-selection operators, are trainable objects on a footing comparable to token embeddings.
...
The implications are restrictive but conceptually clarifying.
Many workhorse techniques of classical deep learning,
such as gradient backpropagation through non-linear activations,
are unavailable in their standard form. A quantum-attention analogue would instead shift the emphasis toward symmetry operations, reversible dynamics,
and controlled basis changes.
If classical transformers are powered by the calculus of weighted averages,
a genuinely unitary analogue would be powered by the geometry of Hilbert-space rotations.
In such a scheme, ``attention'' would mean selecting or coherently aligning an appropriate basis before a final measurement converts amplitudes into token probabilities.

\section{Summary and Outlook}

The development of a large language model is a multi-stage, resource-intensive endeavor that progressively builds knowledge and refines behavior, starting from a base model pre-trained on internet-scale data, which is then fine-tuned into a conversational agent and further sharpened through reinforcement learning. Understanding this pipeline is key not only to appreciating the models' capabilities but also to grasping the structural principles that govern their operation and give rise to their limitations. The most important of these are the high-dimensional supply of quasi-orthogonal semantic directions, the dynamic handling of meaning, the intentional use of randomness in generation, the bounded character of algorithmic search, and the limited but instructive structural analogies with quantum mechanics.
...
This reliance on vectors, inner products, and context-dependent probability distributions creates a useful analogy with quantum mechanics. A structural resonance exists between the LLM's softmax function, which determines next-token probabilities from inner products, and the Born rule in quantum mechanics, which calculates measurement probabilities from projections onto a basis. Critical disanalogies remain decisive: LLMs lack quantum phase and interference, and their dynamics are dissipative rather than unitary. Precisely for that reason, the analogy is most useful when it clarifies what would have to change in a genuinely unitary attention mechanism: nonlinear mixing would have to be replaced by reversible rotations, with probabilities appearing only at preparation or measurement. Understanding these core mechanisms---how meaning is dynamically constructed, how responses are probabilistically generated, how algorithmic capacity remains bounded, and how these processes resemble but do not instantiate quantum formalisms---is essential for using such models responsibly.


A final observation concerns the distinction between a model's underlying representational capacity and the constraints imposed by deployment. Following pre-training, modern LLMs typically undergo additional alignment procedures, including instruction tuning and reinforcement-learning-based optimization. These stages are intended to improve usefulness, reliability, and safety, but they also influence how the model expresses and prioritizes information in practice.

Similarly, most deployed systems do not engage in unrestricted online updating of their core parameters during user interactions. Allowing continuous self-modification in response to arbitrary environmental inputs would introduce significant challenges related to robustness, security, and controllability. As a result, LLMs generally appear behaviorally stable but also fixated and non-responsive across sessions. Some systems may maintain user-specific memories or conversation histories, but this should not be confused with ongoing retraining of the underlying model.

From a theoretical perspective, the representational capacity of large neural networks is not exhausted by the behaviors exhibited in current deployments. The practical behavior of an LLM reflects not only its architecture and learned parameters, but also the objectives, safeguards, and operating constraints under which it is deployed. It is therefore plausible that future systems, particularly when coupled to persistent memory, autonomous planning mechanisms, and embodied robotic platforms, may display forms of interaction that appear more adaptive, contextual, and human-like than those commonly observed today.

Such developments could substantially influence the future evolution of robotics and autonomous agents.

\begin{acknowledgments}
This text was partially created and revised with assistance from one or more of the following large language models. All content, ideas, and prompts were provided by the author.
