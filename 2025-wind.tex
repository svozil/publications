...
Throughout, ``faster-than-the-wind'' refers to motion parallel (upwind or downwind) to the true wind in the ground/water frame, unless stated otherwise.
...
wheel-driven fan, accelerating this oncoming air rearward to produce thrust.
...
Again, propulsion arises only because the ground provides an external reaction surface (with the ground) that allows the drivetrain to exchange momentum between the two media.
...
Throughout this section we work in the laboratory frame in which the far-field
...
wake formation. Along the downwind axis, the true wind speed is $v_w$, the boat
speed through the water is $v_b$, and the apparent wind is
\(
v_a = v_w - v_b
\).
...
Imagine Ann standing on a dock while Bob sits in a small boat that is free to
drift horizontally with negligible resistance. Bob attempts to reach Ann by
...
center-of-mass motion of the system relative to the environment.
...
difference between two media---the air motion relative to the ground.
