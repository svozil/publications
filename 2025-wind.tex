...
This idea is formalized in control theory~\cite{bechhoefer-2021} with the ``inerter,'' a mechanical element where the force between its two ends is proportional to their relative acceleration: $F = b(\ddot{x}_1 - \ddot{x}_2)$, with $b$ being the inertance~\cite{Smith2002}. This concept generalizes mass to a two-terminal network. This naturally leads to a thought-provoking question: what if the inertance, $b$, could be negative?
...
However, the {\it behavior} of negative effective mass can be achieved by an {\it active} system. Such a system uses sensors, a controller, and an actuator (like a motor) to inject energy from an external source and generate forces that oppose what an ordinary mass would do. This emulation does not violate any physical laws because it is powered. An ``anti-inerter'' would create a counter-intuitive effect: pushing on one end would cause the other to accelerate {\it away} from the first, rather than following it.
