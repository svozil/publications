...
structural and deliberately limited.  As a constructive boundary case, a
final appendix specifies a separate finite-dimensional quantum-instrument
attention model
with complex amplitudes, tensor composition, entangling unitary evolution, and
phase-sensitive Born-rule predictions.  Further mathematical details are
...
comparisons.  Appendix~\ref{app:quantum-attention} gives the separate
quantum-instrument construction as a deliberately nonstandard
boundary case.  Additional formal statements and derivations appear in the
...
Appendix~\ref{app:polysemy}.  Appendix~\ref{app:quantum-attention} gives a
deliberately nonstandard construction that meets the listed requirements only
by changing the model class; it is not a reinterpretation of ordinary
...
% ======================================================================
\section{A deliberately quantum extension: entangled instrument attention}
\label{app:quantum-attention}

The requirements stated in Sec.~\ref{sec:polysemy} can be met, but only by
changing the model class.
As a constructive boundary case, consider \emph{entangled quantum-instrument
attention} (EQIA).  It implements the finite-dimensional operational quantum
framework rather than merely decorating ordinary transformer arithmetic with
complex notation.  It is therefore a proposed attention architecture, not a
reinterpretation of a standard transformer.

For target position $i$, declare the data register
\begin{equation}
\begin{aligned}
 \mathcal R_i={}&\mathcal H_{Q_i}\otimes\mathcal H_A
 \otimes\bigotimes_{j\leq i}
 \left(
 \mathcal H_{S_j}\otimes\mathcal H_{K_j}\otimes\mathcal H_{V_j}
 \right),\\
 \mathcal H_i={}&\mathcal R_i\otimes\mathcal H_O
 \otimes\mathcal H_M\otimes\mathcal H_E .
\end{aligned}
\label{eq:eqia-space}
\end{equation}
Here $Q_i$ is a query register, $A$ a coherent source-address register,
$S_j$, $K_j$, and $V_j$ are the sense, key, and value registers of source
position $j$, $O$ is the output, $M$ is a meter or measurement record, and
$E$ is an environment.  This tensor factorization is part of the model; it is
not inferred from the width of an ordinary residual stream.  Autoregressive
causality is imposed by including no source coupling with $j>i$.

For token type $t_j$, declare an orthonormal sense basis
$\{|s\rangle_{S_j}:s=1,\ldots,S_{t_j}\}$.  A coherent preparation of one
occurrence may be
\begin{equation}
\begin{aligned}
 |\psi_j\rangle
 &=\sum_{s=1}^{S_{t_j}}a_{js}
 |s\rangle_{S_j}|k_{js}\rangle_{K_j}|v_{js}\rangle_{V_j},\\
 a_{js}&=|a_{js}|e^{i\phi_{js}},
 &\sum_s|a_{js}|^2&=1 .
\end{aligned}
\label{eq:eqia-preparation}
\end{equation}
All displayed key and value kets are normalized.  Orthogonality of the sense
kets then normalizes Eq.~\eqref{eq:eqia-preparation} even when key or value
branches overlap.  The numbers $|a_{js}|^2$ are sense-basis populations and
$\phi_{js}-\phi_{jr}$ are relative phases.  A common phase has no observable
effect.  The sense basis and phase reference acquire operational meaning only
through declared preparation gates and measurement settings; a consistent
rephasing of basis states and operators changes no prediction.

Classical uncertainty over preparations is represented by a density matrix
\begin{equation}
 \rho_{\mathcal R_i}
 =\sum_z w_z|\Psi_z\rangle\langle\Psi_z|,
 \qquad w_z\geq0,
 \quad\sum_z w_z=1 .
\label{eq:eqia-density}
\end{equation}
A mixed state need not be diagonal in the sense basis.  Diagonal entries give
populations, while off-diagonal entries retain coherence relative to that
basis.

For example, prepare the address register as
$|a_i^{(0)}\rangle_A=i^{-1/2}\sum_{j\leq i}|j\rangle_A$ and, with
$\hbar=1$, evolve the closed enlarged system according to
\begin{equation}
 U_i=\mathcal T\exp\left[-i\int_0^\tau H_i(t)\,dt\right].
\label{eq:eqia-unitary}
\end{equation}
One admissible causal Hamiltonian has the form
\begin{equation}
\begin{aligned}
 H_i(t)={}&H_A(t)
 +\sum_{j\leq i}|j\rangle\langle j|_A
       \otimes H_{Q_iK_j}(t)\\
 &+\sum_{j<k\leq i}H_{S_jS_k}(t)
 +H_{\mathcal R_iE}(t),
\end{aligned}
\label{eq:eqia-hamiltonian}
\end{equation}
where every term is self-adjoint.  For instance, $H_A$ may contain conjugate
pairs $\kappa_{jk}|j\rangle\langle k|+\kappa_{jk}^*|k\rangle\langle j|$,
and $H_{Q_iK_j}$ may be a real linear combination of tensor products of
self-adjoint query and key observables.  The controlled query--key interaction
imprints source-dependent phases and correlations; the address Hamiltonian
then recombines these paths.  The terms $H_{S_jS_k}$ permit Schmidt rank
greater than one across different sense registers and hence entanglement,
rather than merely correlated classical choices.  Because the terms need not
commute, their time order is part of the attention rule.

The final term gives decoherence its standard system--environment meaning.
For a simplified sense register, suppose unitary evolution produces
\begin{equation}
 \sum_s a_s|s\rangle|0\rangle_E
 \longmapsto
 \sum_s a_s|s\rangle|E_s\rangle_E .
\label{eq:eqia-environment}
\end{equation}
Discarding the environment gives
\begin{equation}
 \rho_S'
 =\operatorname{tr}_E|\Psi\rangle\langle\Psi|
 =\sum_{s,r}a_sa_r^*\langle E_r|E_s\rangle
 |s\rangle\langle r| .
\label{eq:eqia-partial-trace}
\end{equation}
The joint state still evolves unitarily, while the reduced state is generally
mixed.  This is the explicit system--environment coupling and partial trace
that ordinary transformer processing lacks; it is the mechanism summarized
in Appendix~\ref{app:dirac}, not merely small Euclidean cross-talk.

It remains to turn the evolved state into attention.  Let $O$, $M$, and $E$
begin in declared blank states and write
\begin{equation}
\begin{aligned}
 \rho_i^0
 &=\rho_{\mathcal R_i}\otimes
 |0\rangle\langle0|_{OME},\\
 \Sigma_i
 &=W_iU_i\rho_i^0U_i^\dagger W_i^\dagger .
\end{aligned}
\label{eq:eqia-meter-state}
\end{equation}
Here the unitary $W_i$ coherently routes the addressed value into $O$ and
correlates the attention result with $M$.  A controlled swap can move an
unknown value state when the register dimensions agree, but it cannot leave a
second perfect copy behind.  Coherent address garbage must be uncomputed or
included among the branch records before interference is read out.

Let $\{P_\beta^{(M)}\}$ be a projective measurement of the meter and put
$\Pi_\beta=I_{\mathcal R_iO}\otimes P_\beta^{(M)}\otimes I_E$.  The
unnormalized conditional output state is
\begin{equation}
 \widetilde\rho_{O,\beta}
 =\operatorname{tr}_{\mathcal R_iME}
 \left(\Pi_\beta\Sigma_i\Pi_\beta\right).
\label{eq:eqia-instrument}
\end{equation}
These maps form a quantum instrument.  In particular,
\begin{equation}
 p_i(\beta)=\operatorname{tr}\widetilde\rho_{O,\beta},
 \qquad
 \rho_{O|\beta}
 =\frac{\widetilde\rho_{O,\beta}}{p_i(\beta)}
\label{eq:eqia-born-update}
\end{equation}
are the Born attention probability and its selective, outcome-conditioned
state update.  If the result is discarded, $\sum_\beta
\widetilde\rho_{O,\beta}$ is the output of a completely positive
trace-preserving map.  A vocabulary POVM obeying
$\Lambda_y\succeq0$ and $\sum_y\Lambda_y=I_O$ supplies a second Born readout,
\begin{equation}
\begin{aligned}
 p(y,\beta\mid x)
 &=\operatorname{tr}
   \left(\Lambda_y\widetilde\rho_{O,\beta}\right),\\
 p(y\mid x)&=\sum_\beta p(y,\beta\mid x).
\end{aligned}
\label{eq:eqia-vocabulary}
\end{equation}
There is no attention softmax after Eq.~\eqref{eq:eqia-born-update};
normalization follows from the complete measurement.  Preparation,
Hamiltonian, and POVM parameters can be trained by minimizing
$-\log p(y_{\rm true}\mid x)$.

The essential phase test is a two-path ``bank'' interferometer.  Include all
unobserved key, value, meter, and environmental information in branch-record
states $|r_{\rm F}\rangle$ and $|r_{\rm R}\rangle$, and prepare
\begin{equation}
\begin{aligned}
 |\psi_\varphi\rangle
 &=\frac{|{\rm F}\rangle|r_{\rm F}\rangle
 +e^{i\varphi}|{\rm R}\rangle|r_{\rm R}\rangle}{\sqrt{2}},\\
 \gamma&=\langle r_{\rm F}|r_{\rm R}\rangle,
 &|\mathord{\pm}\rangle
 &=\frac{|{\rm F}\rangle\pm|{\rm R}\rangle}{\sqrt{2}} .
\end{aligned}
\label{eq:eqia-bank-state}
\end{equation}
Recombine and measure the sense register in the incompatible
$\{|\mathord{+}\rangle,|\mathord{-}\rangle\}$ basis.  The Born rule predicts
\begin{equation}
 p_\pm(\varphi)
 =\frac12\left[1\pm
 \operatorname{Re}\!\left(e^{i\varphi}\gamma\right)\right].
\label{eq:eqia-bank-fringe}
\end{equation}
For indistinguishable branch records, $\gamma=1$ and hence
$p_+=\cos^2(\varphi/2)$ and $p_-=\sin^2(\varphi/2)$.  Changing $\varphi$
from zero to $\pi$ reverses the result although both direct sense-basis
populations remain $1/2$.  By contrast, the incoherent state
$\rho_{\rm mix}=(|{\rm F}\rangle\langle{\rm F}|
+|{\rm R}\rangle\langle{\rm R}|)/2$ always gives $p_+=p_-=1/2$.
Distinguishable which-sense records reduce the fringe visibility to
$|\gamma|$; dephasing or a which-sense measurement removes it, while coherent
erasure or joint recombination can restore it.  A global phase changes
nothing.  Coupling the two output ports to different value or vocabulary
outcomes therefore supplies the required phase-sensitive intervention in which
phase, with all sense-basis populations held fixed, changes next-token
predictions.

A physical implementation must also respect no-cloning
\cite{wo-zu,nielsen-book10}.  Arbitrary unknown nonorthogonal semantic states
cannot be copied into independent attention heads.  Several head ancillas may
interact with one state as parts of a single global unitary, or heads may act
sequentially, act on disjoint factors, or use fresh states independently
prepared from classical input.  They cannot each receive a perfect copy of an
unknown state.  Sequential noncommuting head measurements generally disturb
one another, and one system cannot be maximally entangled with two independent
systems.  Thus complementarity, measurement disturbance, no-cloning, and
monogamy become constraints of the architecture rather than metaphors.

On a classical computer, EQIA is an exact or approximate simulation of
complex states, tensor products, quantum channels, and Born sampling.  It is
quantum-formal but not physically quantum, and a general state-vector or
density-matrix simulation has exponential cost in the number of tensor
factors.  On quantum hardware the same specification can instead use physical
state preparation, entangling gates, environment couplings, and measurement.
Neither realization shows that language is intrinsically quantum, and no
computational advantage follows without a separate complexity and resource
analysis.  The conditional state update is operational and does not select an
interpretation of physical collapse.
