...
\paragraph{Process randomness and randomness of a realized sequence}

At the level of an individual trial, randomness concerns the mechanism
producing an outcome or its conditional uncertainty given specified information.
We use \emph{process randomness} for this probabilistic description of a binary
process \((X_n)_{n\geq1}\). Its joint law specifies how the trials depend on
their history. The realized symbol \(0\) or \(1\) does not itself certify a
random origin. Martin-L\"of randomness instead concerns an individual
\emph{infinite} record \(x=x_1x_2\cdots\), tested through prefixes of
unbounded length relative to a specified law. Neither a single outcome nor a
completed finite sample settles this property.

Fair one-bit marginals alone do not suffice. Toss one fair bit \(Y\) and set
\(X_n=Y\) for every \(n\). Each \(X_n\) has probability \(1/2\) of either
value, but the record is \(000\cdots\) or \(111\cdots\), neither random for
the fair-coin law \(\lambda\). The repetitions are predictable after the first
observation. Independent fair trials, equivalently conditional fairness after
every finite past, supply the missing hypothesis:
\begin{equation}
  \mathbb P(X_{n+1}=1\mid X_1,\ldots,X_n)=\tfrac12
  \quad\text{a.s.},\qquad n\geq0.
  \label{eq:conditional-fairness}
\end{equation}
Here \(n=0\) includes fairness of the first trial. The resulting joint law is
\(\lambda\), with \(\lambda([s])=2^{-|s|}\) for every cylinder \([s]\) of
sequences beginning with the finite word \(s\). For a computable biased or
dependent law \(\mu\), randomness must instead be assessed relative to
\(\mu\). In particular, both constant records above are random for their own
two-atom law: a positive-probability atom cannot lie in a null test. Thus
measure-relative randomness does not by itself assert independence or fresh
uncertainty at each trial.
...
test~\cite{MartinLof1966}. For a computable law \(\mu\), replace the bounds
on \(\lambda(U_k)\) by bounds on \(\mu(U_k)\); write
\(\mathrm{MLR}_\mu\) for the resulting set of random records. Computability
here means that the cylinder probabilities \(\mu([s])\) can be approximated
uniformly to any requested rational accuracy.

\begin{proposition}[From a Computable Process Law to Random Paths]
  \label{prop:process-ml}
  If a binary process has computable joint law \(\mu\), then
  \begin{equation}
    \mathbb P\bigl((X_1,X_2,\ldots)\in\mathrm{MLR}_\mu\bigr)
    =\mu(\mathrm{MLR}_\mu)=1.
    \label{eq:process-ml}
  \end{equation}
  In particular, independent fair trials produce a fair-coin Martin-L\"of-random
  infinite sequence almost surely.
\end{proposition}

\begin{proof}
For each effective test, \(\mu(\bigcap_k U_k)\leq2^{-k}\) for every \(k\),
so its failure set is null. There are only countably many effective tests.
Their union is therefore null, leaving \(\mathrm{MLR}_\mu\) with measure one.
\end{proof}

This is already part of Martin-L\"of's 1966 treatment of computable
distributions and universal sequential tests, rather than a new result of the
present paper~\cite[Sec.~IV, p.~614]{MartinLof1966}. His subsequent Bernoulli
discussion also connects sampling with computable bias to a computing machine
supplied with a random-number table. The bridge is \emph{almost sure}, not a
guarantee for every point of the sample space or a finite stopping rule that
certifies an observed record. A computable probability law must also be
distinguished from a deterministic program producing its sample path.

A sequence is \emph{computably random} if no total
...
\paragraph{Computable transformations and later results}

The process--path link also has a pointwise formulation for computable
sampling. Let \(F\) be an almost-everywhere computable map from binary
records with computable law \(\mu\) to records with computable law \(\nu\),
preserving these measures: \(\nu(A)=\mu(F^{-1}(A))\). Randomness conservation
and the complementary ``no randomness from nothing'' theorem give
\begin{equation}
  F(\mathrm{MLR}_\mu)=\mathrm{MLR}_\nu.
  \label{eq:randomness-conservation}
\end{equation}
Every random input has a random output for the \emph{output} law, and every
random output has some random preimage. The latter is an existence statement,
not an effective inversion procedure. It also does not say that every preimage
is random. Rute's 2016 analysis proves corresponding results and separations
for weaker randomness notions and characterizes Martin-L\"of randomness
through the joint conservation and preimage requirements~\cite{Rute2016}.
Computable processing can therefore transfer randomness between specified
laws without preserving fair-coin randomness when it changes the law.

Two recent developments extend this distinction beyond independent bit streams.
Huang, J.~H. Lutz, N. Lutz, and Migunov's 2026 journal treatment defines
randomness for individual continuous-time Markov-chain trajectories, including
their states and sojourn times, and establishes equivalent martingale,
effective-measure, and complexity characterizations. For stochastic chemical
reaction networks, every random trajectory with bounded molecular counts is
non-Zeno: infinitely many reactions cannot occur in finite
time~\cite{HuangLutzLutzMigunov2026}. This is a concrete passage from an
almost-everywhere process statement to a property of every random trajectory.

Huttegger, Walsh, and Zaffora Blando (2026) characterize Martin-L\"of and
Schnorr randomness through the merging of probabilistic
forecasts~\cite{HutteggerWalshZafforaBlando2026}. For computable laws \(\nu,\mu\)
with full support, let \(D_n(\nu\Vert\mu)\) be the relative entropy of their
length-\(n\) distributions. A record is \(\nu\)-Martin-L\"of random precisely
when its one-step conditional Kullback--Leibler divergences from \(\nu\) to
\(\mu\) are summable for every computable comparison law \(\mu\) satisfying
\(\sup_n D_n(\nu\Vert\mu)<\infty\). Their Schnorr counterpart additionally
requires this supremum to be computable. Summability implies that the
forecast discrepancies tend to zero. These are conditional results about
agreement of forecasts, not certainty of individual outcomes or financial
profit guarantees.

For the trading claims, Proposition~\ref{prop:randomized} uses a process law:
conditional fairness and integrability imply zero expected gain at each fixed
finite horizon. Theorem~\ref{thm:effective} instead fixes an infinite random
record and excludes unbounded effective nonnegative test capital on that
record. Proposition~\ref{prop:process-ml} connects the two levels without
identifying their different quantifiers, capital constraints, or horizons.
