...
In what follows the following notation will be adopted by the definition: $a^n = \underbrace{a \circ \cdots \circ a}_{n \text{ times}}$.
...
(13)&(14)&(15)&(16)\\
\end{tabular}
\caption{Different mappings; only (7) und (10) satisfy exhaustion (i) and uniqueness (ii);
they represent  permutations which induce associativity. Therefore only (7) und (10) represent group composition tables, with identity elements 0 and 1, respectively.\label{2017-m-ch-gt-t-gc1t}}
\end{center}
\end{table}


\subsection {Group of order 3}
...
$I$ or $b$. Yet $t_{22}$ cannot be $I$ because this would require $t_{23}=t_{32}=b$, but $b$ already occurs in the third row and column.
Therefore, $t_{22} = b$, implying  $t_{23}=t_{32}=I$, and in the next step, $t_{33} = a$.
The third table (3) represents the composition table in terms of multiples of the generator $a$ with the relations $b=a^2$ and $a^3=I$.
...
Table~\ref{2017-m-ch-gt-t-gc1t6}. The second group is non-Abelian; that is, the group  composition is not equal its transpose.
\begin{table}
\begin{center}
\begin{tabular}{cccccccccccccccccccccccccccccccc}
%\begin{tabular}{c|ccccccccccccccccccccccccccccccc}
...
\\(2)\\
\end{tabular}
\caption{The two groups with 6 elements; the latter one being non-Abelian.
...
Properties (i) and (ii) -- exhaustion and uniqueness -- is a translation into the equivalent properties of bijectivity; together with the coinciding (co-)domains this is just saying
