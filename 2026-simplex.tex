...
\end{equation}
Their absence gives the Kochen--Specker obstruction.  Their scarcity gives the meager cases considered below: nonunital, nonseparating, and nonfull two-valued-state spaces.  Their abundance, especially a separating family, permits a concrete set representation and hence a partition-logic model~\cite{svozil-2001-eua}.
...
\end{equation}
commute and therefore possess a common eigenbasis.  That eigensystem is one orthonormal basis in $\complex^8$; as a projector logic it is a single Boolean block with eight atoms and therefore has eight dispersion-free states~\cite{svozil-2024-convert-pra-externalfigures}.  There is no Kochen--Specker contradiction at the level of that isolated context.  The contradiction enters only when the value of each global product observable is assumed to factor into values of the local Pauli components $X_i$ and $Y_i$.  Then each local factor appears twice in the product of the four assigned values, forcing $+1$ classically, whereas the corresponding operator product is $-\id$.  The failure is therefore not the absence of a valuation on one context, but the impossibility of a product-preserving valuation on the operator algebra being implicitly invoked.

The Peres--Mermin square~\cite{peres111,mermin90b,peres-91} is different but related.  At the operator level it is a parity proof.  When the masked contexts are extracted by simultaneous diagonalization or by matrix pencils, the operator argument can be transcribed into a projective Kochen--Specker structure, in particular the Peres 24-ray completion containing the Cabello 18--9 set~\cite{svozil-2024-convert-pra-externalfigures}.  Thus some operator-functional contradictions descend to combinatorial projector contextuality after unmasking, while others, such as the single-context GHZ eigensystem, do not.  This is why operator-functional contextuality deserves to be kept as a third layer rather than absorbed into either colorability or simplex embeddability.
...
\end{align}
Let $I_\Omega:S_\Omega\hookrightarrow \operatorname{Herm}(\HH)$ and $I_E:S_E\hookrightarrow \operatorname{Herm}(\HH)$ denote the inclusion maps expressed in chosen orthonormal bases.  The simplex-embeddability LP asks whether there exists a nonnegative matrix $\sigma\in\reals^{m\times n}$ such that
\begin{equation}
...
Second, simplex embeddability is weaker than simpliciality.  A generalized probabilistic theory or an accessible fragment may embed into a classical simplex even though its own state space is not itself a simplex.  This distinction is central to the generalized-probabilistic-theory analysis of Schmid \emph{et al.}~\cite{schmid-2021-simplex}.  For accessible fragments, cone equivalence is especially important: cone-equivalent fragments are either both classically explainable or both not, because the embedding problem depends on the generated cones~\cite{selby-2023-fragments}.
...
Peres--Mermin & unmasked joint eigenspaces & nontrivial multi-context test & parity plus Kochen--Specker/projector structure\\
...
\end{equation}
It is useful to distinguish several ways in which $\TVS(H)$ can be deficient.  Tkadlec called attention to Greechie diagrams whose state spaces are empty, not unital, not separating, or not full~\cite{tkadlec-96}; these notions are standard in the quantum-logic literature~\cite{pulmannova-91,schaller-92,svozil-tkadlec}.
...
These propositions have a limited scope.  They do not say that every meager hypergraph automatically fails every simplex test.  The simplex LP is fragment-dependent.  If a nonunital atom is never prepared or measured, if a nonseparated pair is never operationally distinguished, or if the TIFS endpoint effects are absent, the corresponding logical defect may not be visible.  Conversely, once the relevant effects and eigenstate preparations are included, the deterministic valuation obstruction becomes a concrete operational obstruction.

\section{Relation to generalized noncontextuality}

The deterministic two-valued-state analysis is closest to the traditional Kochen--Specker setting.  Generalized noncontextuality is broader: it does not assume determinism from the outset.  Nevertheless, determinism for sharp projective measurements is recovered on the support of eigenstate preparations when the operational equivalences include the relevant projective measurements and mixture decompositions.  This is the familiar route from operational noncontextuality to Kochen--Specker-style constraints, developed in modern form by Kunjwal and Spekkens~\cite{kunjwal-spekkens-2015}.
