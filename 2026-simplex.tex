...
\begin{abstract}
This paper separates three classicality tests that are often conflated in discussions of contextuality: combinatorial tests based on valuations, colorings, and partition-logic embeddability; simplex tests for operational probability tables; and operator-functional tests requiring spectral-functional and multiplicative consistency of observable values.  The distinction is relative to the structure retained in the fragment---incidence data, probability data, or operator algebra.  Calibration examples and restricted projector-cone computations show that these tests often coexist in familiar quantum examples, but none uniformly replaces the others.  The numerical thresholds reported here concern the deliberately restricted projector-cone prepare-and-measure fragments specified in the paper; they are not claimed to be full operational noncontextuality robustnesses unless the corresponding operational equivalences, coarse grainings, and unit effects are explicitly included.  Rows certified only by exact primal feasibility should be read as rational feasible upper bounds matching numerical optima, not as independently certified exact optima.
\end{abstract}
...
This taxonomy is meant to complement, not replace, existing classifications.  The graph-theoretic approach of Cabello, Severini, and Winter relates exclusivity graphs to invariants such as independence numbers and the Lov\'asz theta function~\cite{cabello-severini-winter-2014}.  The hypergraph approach of Ac\'in, Fritz, Leverrier, and Sainz formulates contextuality scenarios directly as compatibility/exclusivity structures with probabilistic models~\cite{acin-fritz-leverrier-sainz-2015}.  The sheaf-theoretic framework of Abramsky and Brandenburger classifies empirical models by the obstruction to gluing local sections, distinguishing probabilistic, logical, and strong forms of contextuality~\cite{abramsky-brandenburger-2011}.  The present distinction is cross-cutting: it asks which part of the Hilbert-space description is being retained before the classicality test is applied---incidence data, convex-operational probability data, or operator-algebraic product data.  The safest organizing principle is therefore not a single ladder from classical to quantum, but a grid: one axis records the combinatorial assignment structure, another records the probability model placed on it, and a third records algebraic constraints among operator-valued observables.

Table~\ref{tab:taxonomy-grid} makes the grid explicit.  The rows are not
levels of a hierarchy.  They specify which data have been retained before a
classicality test is applied.

\begin{table*}[t]
\caption{A concrete version of the three-axis taxonomy.  The same physical
example may occupy different rows depending on which operational and algebraic
data are retained.}
\label{tab:taxonomy-grid}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llll}
\toprule
Axis retained & Input data & Classical object tested & Typical obstruction\\
\midrule
Combinatorial assignment structure
& Orthogonality hypergraph, contexts, atoms
& Two-valued states, strong colorings, partition logics
& KS uncolorability; nonunital, nonseparating, or nonfull state space\\
Convex-operational probability structure
& Preparations, effects, probabilities, operational equivalences
& Simplex embedding of the specified GPT fragment
& No positive cone/simplex factorization without noise\\
Operator-algebraic structure
& Commuting observables, spectral functions, product identities
& Functional and multiplicative value assignment
& GHZ/Mermin parity contradiction\\
\bottomrule
\end{tabular}%
}
\end{table*}

In the Abramsky--Brandenburger terminology, the combinatorial row is closest
to the existence or nonexistence of global sections, while the simplex row
concerns a specified probabilistic empirical model after the operational
identifications have been fixed.  In the Cabello--Severini--Winter framework,
graph invariants such as independence numbers and Lov\'asz-theta-type quantities
benchmark exclusivity structures and their quantum representations.  The
present projector-cone thresholds are different objects: they depend on the
chosen preparation and effect cones, not only on the exclusivity graph.
...
The operator-functional layer is included because it behaves differently from both the projector-hypergraph and probability-table layers.  In GHZ- or Mermin-type parity arguments, the contradiction is not always located at the level of a single resolved Boolean context.  For the closed collective GHZ measurement, the commuting global product observables have a common eigenbasis, and their joint spectral projectors form one Boolean algebra.  The contradiction appears only when one additionally requires the assigned value of a global product, such as $X_1Y_2Y_3$, to factor into pre-existing local values of the observables $X_i$ and $Y_i$.  The classical product of the four assigned global values is then forced to be $+1$, whereas the corresponding operator product is $-\id$.  Thus the relevant issues are the assignment of values to individual observables and the product-preservation assumption, rather than the incidence structure of the resulting joint spectral projectors.  This does not place operator-valued parity proofs outside simplex analysis in general: the simplex verdict depends on the chosen operational fragment.  A closed collective single-context GHZ fragment is simplex-trivial, whereas an enlarged or spectrally unmasked fragment, including the relevant local factors, refinements, and operational equivalences, may enter a simplex test as a nontrivial fragment.  Peres--Mermin provides the standard case in which the operator parity proof descends, after unmasking, to a genuine multi-context projector configuration~\cite{mermin90b,svozil-2024-convert-pra-externalfigures}.
...
\end{equation}
Their absence gives the KS obstruction.  Their scarcity gives the meager cases considered below: nonunital, nonseparating, and nonfull two-valued-state spaces.  The term \emph{meager} is used here only as a convenient umbrella term for these Tkadlec-style finite-state-space pathologies; it is not meant in the Baire-category sense.  Their abundance, especially a separating family, permits a concrete set representation and hence a partition-logic model~\cite{svozil-2001-eua}.
...
Operator-functional contextuality concerns observables as algebraic quantities, not merely as names or labels for their spectral projectors.  Two requirements should be distinguished.
...
Three qualifications fix the scope of the restricted tests used below.
...
The relation between the restricted LP used here and the full SSWKS framework
is therefore best described as follows.  For a fixed accessible GPT fragment
with all operational equivalences already quotient\-ed out, the factorization
Eq.~\eqref{eq:simplex-lp} is the corresponding cone form of the simplex
embedding problem.  The projector-cone computations in this paper are obtained
by choosing a much smaller fragment: the listed projectors are used as the
preparations and effects, and no additional equivalence constraints are imposed
unless they are generated by that cone data.  Hence the resulting \(r\)-values
are not lower or upper bounds on a separately specified full operational
robustness in any automatic sense.  They are robustnesses of this particular
fragment.  A full SSWKS implementation for the same physical scenario would
require an explicit operational specification of preparations, measurements,
coarse grainings, and equivalences before the two numbers could be compared.
...
\begin{table*}[t]
\caption{Calibration table for the restricted projector-cone prepare-and-measure fragments used in this paper.  The listed rays are used both as rank-one preparations and as rank-one effects.  The noise is the state-side depolarizing map $\rho\mapsto \tr(\rho)\id_d/d$.  These numbers are not universal contextuality rankings and are not full operational noncontextuality robustnesses unless the missing operational data are explicitly added.  ``Exact optimum'' means exact primal and exact dual certificates.  ``Exact primal'' means an exact feasible rational certificate matching the numerical optimum, but with no exact dual witness yet recovered.  ``Numerical'' means floating-point evidence only.}
\label{tab:results}
\resizebox{\textwidth}{!}{%
...
Cabello--Estebaranz--Garc\'ia-Alcaine 18--9 & 4 & 18 & 10 & 146 & $1/3$ & exact primal; dual open\\
...
Only rows marked ``exact optimum'' in Table~\ref{tab:results} are certified optima by rational primal and dual witnesses.  Rows marked ``exact primal; dual open'' are certified feasible rational upper bounds that match numerical optima, but the displayed rational value is not independently certified as optimal until a dual witness is supplied.
...
The computational table should therefore not be read as a ranking of one scalar called contextuality.  The Specker bug, $\Gamma_3$, and Tkadlec's nonunital configuration realize different forms of meagerness.  Yu--Oh 13 and Yu--Oh 25 are related but generate different operational cones.  Cabello 18--9 and Peres--Mermin/Peres 24 show how adding the orthogonal completion can increase simplex robustness.  The LP thresholds quantify nonclassicality for the selected restricted projector-cone fragments; they should not be read as full operational noncontextuality robustnesses unless the corresponding unit effects, coarse grainings, and operational equivalences have also been included.  They do not replace either the combinatorial classification or the operator-functional parity analysis.
...
The computations in Table~\ref{tab:results} rely on ray coordinates, incidence files, accessible-fragment reductions, rational facet data, and primal/dual certificate checks.  These data and scripts should be deposited as supplementary material or in a public repository before submission.  Until that deposit is made, the rows marked ``exact optimum'' are reproducible from the rational certificates described in Appendix~\ref{app:audit}, while rows marked ``exact primal; dual open'' should be treated as certified feasible upper bounds matching numerical optima rather than as independently certified exact robustness values.
...
\begin{table*}[t]
\caption{Audit status of the simplex computations.  Here $k_\Omega$ and $k_E$ are the accessible state and effect dimensions after Stage-1 reduction.  ``Dual open'' means that no rational dual certificate is presently available; the row is therefore not an independently certified exact optimum.}
...
Yu--Oh 25 completion & 25 & 6 & 6 & 96 & $21/44$ & yes & no & dual open\\
Tkadlec Fig.~2, nonunital & 37 & 6 & 6 & 276 & $35/64$ & yes & no & dual open\\
Cabello 18--9 & 18 & 10 & 10 & 146 & $1/3$ & yes & no & dual open\\
Peres--Mermin/Peres 24 & 24 & 10 & 10 & 120 & $4/9$ & yes & no & dual open\\
