...
philosophical question, and fluent behavior will not be used as evidence for the emergence of meaning or even
...
The calculation begins by encoding text as vectors.  A tokenizer maps a
string to token identifiers, and an embedding table maps each identifier to an
initial vector.  Transformer layers then use the surrounding sequence to
produce a context-dependent vector for each token occurrence.
This vector encoding immediately suggest some proximity to quantum mechanics, where vectors are used to encode pure states.

In pursuing LLMs further, two issues provide a manageable route into that Hilbert space geometry.
