...
As this shows, formal sums of the Grandy type (\ref{2009-fiftyfifty-1}) require specifications
...
\index{oracle}
\marginnote{Never mind such fallacious methods as the ones collected (in German) at URL \url{http://kamelopedia.net/wiki/Beweis}.
My personal favorites are
{\em ``proof by complete destruction,''}
{\em ``proof by authority,''}  with its variant
{\em ``proof by intimidation,''}
{\em ``proof through irritation,''} and, never to dismiss, the
{\em ``proof by pause: {\em `I shall prove this sentence after the break.'  $\ldots$ pause $\ldots$  `As I have demonstrated before the break $\ldots$'}.''}
}
...
\newthought{Some proof methods}, among others, which are used today, are:
\begin{itemize}
\item
...
\newthought{Let us consider some concrete examples} and instances of the perplexing varieties of proof methods used today.
...
\marginnote{Ramsey theory can be interpreted as expressing that {\em there is no true randomness,}
irrespective of the method used to produce it;
...
\newthought{Mathematical induction} presents a way to ascertain certain identities, or relations, or estimations
...
\index{basis of induction}, and then (ii) by verifying the {\em inductive step}.
...
Thus, a clear distinction should be made between {\em determinism} (such as {\em computable evolution laws})  on the one hand,
...
\index{busy beaver function} of $n$
...
Actually Leibnitz (among others like Bacon before him) argues that just two number,
say, ``$0$'' and ``$1$,'' are enough to creat all of other numbers, and thus
all of the Universe \cite{svozil-2005-cu}.
...
One of the most challenging number schemes is that of the real numbers \cite{drobot,Hlawka-zz}.
...
Cantor's theorem can be generalized as follows:
