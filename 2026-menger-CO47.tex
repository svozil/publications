...
\label{eq:Teff_def}
\end{equation}
vanishes outside the hole and equals $-T^{(0)}_{\mu\nu}$ inside. It is $T^{\mathrm{eff}}_{\mu\nu}$, not $T_{\mu\nu}$, that sources the \emph{local} gravitational field produced by the defect relative to the background; the absolute $T^{(0)}_{\mu\nu}$ enters only through the global cosmological geometry. The implications of this distinction for the energy conditions are taken up in Sec.~\ref{sec:exotic}.
...
The physical content of Theorem~\ref{thm:universality} and Corollary~\ref{cor:universality} is the following. \emph{In the regime where the linearized analysis applies, the shape of a hole is invisible at leading order in its exterior gravitational field.} A spherical cavity, a cubical cavity, a Swiss-cheese arrangement of small cavities, a random foam, and a Menger sponge~\cite{Menger1926a_reprint,Edgar_ClassicsOnFractals} of equal total absent volume all produce the same leading $1/r$ repulsive potential outside their common bounding sphere. The last example is instructive: at iteration $n$ the Menger-sponge absence in a cube of side $\mathcal{R}$ has volume $V_n = \mathcal{R}^3[1-(20/27)^n]$, and in the limit $n\to\infty$ the entire cube becomes the absence, producing the same leading $G\rho_0\mathcal{R}^3/r$ potential as a spherical cavity of the same volume---despite Hausdorff dimension $D_H = \ln 20/\ln 3 \approx 2.73$ and vanishing Lebesgue measure of the retained substratum in the limit. Shape information enters only through the quadrupole and higher multipoles, which decay as $r^{-(\ell+1)}$ and become negligible at distances $r \gg \mathcal{R}$.

The corollary explains, retroactively, why earlier formulations of this circle of ideas appear ad hoc: elaborate parametrizations of the absence in terms of fractal dimensions and defect-strength parameters do no work that the volume of the absence does not already do.
...
In the substratum framework, the resolution of this paradox is not philosophical but mechanical, and it follows from elementary fluid mechanics. Consider a positive mass $M_+$ placed at finite distance $r$ from a hole of effective mass $M_{\mathrm{eff}} = -|M_{\mathrm{eff}}| < 0$, and let $\hat{\mathbf{n}}$ denote the unit vector pointing from the hole's centroid toward $M_+$. Two physical effects operate simultaneously.

\emph{(i) Gravitational force on the positive mass due to the hole.} Treating the hole as a static negative effective source, the positive mass experiences a gravitational acceleration directed \emph{away} from the hole, i.e., along $+\hat{\mathbf{n}}$,
\begin{equation}
\mathbf{a}_{M_+} = +\frac{G|M_{\mathrm{eff}}|}{r^2}\, \hat{\mathbf{n}},
\label{eq:aMplus}
\end{equation}
with the $+\hat{\mathbf{n}}$ sign inherited from $M_{\mathrm{eff}} < 0$ in the standard Newtonian or weak-field acceleration $\mathbf{a} = -G M_{\mathrm{eff}} \hat{\mathbf{n}}/r^2$.

\emph{(ii) Buoyant force on the hole due to the positive mass.} The substratum surrounding the hole is gravitationally attracted toward $M_+$, acquiring at the hole's location an acceleration $\mathbf{g}_{\mathrm{at\ hole}}(M_+) = +(G M_+/r^2)\hat{\mathbf{n}}$. The hole, as the \emph{absence} of substratum within a flowing medium, is Archimedean: its acceleration is opposite to that of the displaced fluid, by the same mechanism that causes a bubble in water to rise against gravity. Hence
\begin{equation}
\mathbf{a}_{\mathrm{hole}} = -\mathbf{g}_{\mathrm{at\ hole}}(M_+) = -\frac{G M_+}{r^2}\, \hat{\mathbf{n}}.
\label{eq:archimedes}
\end{equation}
The $-\hat{\mathbf{n}}$ sign expresses that the hole accelerates \emph{away} from $M_+$.

Equations~\eqref{eq:aMplus} and~\eqref{eq:archimedes} are oppositely signed with respect to $\hat{\mathbf{n}}$, with $M_+$ accelerating along $+\hat{\mathbf{n}}$ and the hole along $-\hat{\mathbf{n}}$. Since $M_+$ sits at $+\hat{\mathbf{n}}$ relative to the hole by construction, both objects accelerate \emph{away from each other}. This is not the Bondi runaway, in which the two objects accelerate in the same direction with the negative-mass partner ``chasing'' the positive one indefinitely. It is, instead, ordinary mutual repulsion, of the same sign as electrostatic repulsion between like charges, with the momentum balance supplied by the substratum's own gravitational response to $M_+$: the medium, not the hole, is the reservoir.
...
We have argued that gravitational repulsion is a generic and unexotic phenomenon once space is modeled as a background substratum, with two qualifications relative to the most naive form of the claim. First, the exact relativistic geometry depends on the substratum's equation of state: a Lorentz-scalar substratum has Schwarzschild--de Sitter exterior with a static boundary requiring an Israel thin shell, while a frame-fixing dust-like substratum has FLRW exterior with a Lema\^itre--Tolman--Bondi hole boundary that evolves smoothly. Second, the universality theorem---that the shape of the hole is invisible at leading order---is exact only in the Newtonian/linearized regime; in the nonlinear regime it degrades to a statement about the dominant $1/r$ Newtonian piece, which remains shape-independent. With these qualifications the principal claim stands: a hole in a positive-energy substratum produces a repulsive $1/r$ potential whose magnitude is fixed by the absent volume and whose sign is fixed by background subtraction; the energy conditions of the underlying substratum are nowhere violated; and the Bondi runaway is replaced, via Archimedean buoyancy, by ordinary mutual repulsion.

The universality result has a methodological consequence worth stating explicitly. Earlier formulations of this circle of ideas attached interpretive weight to the fractal character of the absent region: Hausdorff dimensions, defect-strength parameters, and microscopic length scales were invoked to fix the magnitude of the effective mass. Theorem~\ref{thm:universality} shows that in the linearized regime this apparatus is unnecessary. The leading exterior field of any bounded hole is fixed by its volume alone; fractal or otherwise, the detailed shape is gravitationally inert at the dominant order.
