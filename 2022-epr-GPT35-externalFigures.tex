...
In order to fully grasp the distinction between quantum entanglement and classical correlations, we will construct classical states that exhibit certain characteristics of quantum entangled states. These classical states possess indefinite values for individual outcomes, yet they are correlated or relationally encoded in such a way that measurement of a particular outcome on one side guarantees the outcome of the same experiment on the other side with certainty. The key difference between quantum and classical systems lies in the ontology of the microstates. While classical physical states are statistical and have completely specified individual value-definite properties, capable of giving rise to an infinite number of value-definite individual outcomes in principle, such value definiteness in quantum mechanics is postulated to hold true only for a specific context or a ``star-shaped'' multiplicity of contexts (as proposed in~\cite{2012-incomput-proofsCJ}), corresponding to the particular pure state in which the particle was prepared (preselected).


\section{Enhancing understanding of EPR-Type configurations with classical shares}

In the subsequent section, we investigate classical models of singlet states.
Surprisingly, our study shows that the correlations in such models can be perfect, just like in the quantum case.
However, similar to the quantum scenario, the classical constraints on correlations as a function of the differences between microstates may or may not be linear.

The stochastic behavior of micro-states versus macro-states in the system under study is remarkably distinct. While a deterministic outcome is obtained by considering individual micro-states, bundling these micro-states effectively leads to value indefiniteness in the macro-states.


The distinction between micro- and macro-states, rooted in the philosophical concepts of epistemology and ontology, respectively, finds parallels in classical statistical mechanics~\cite{Myrvold2011237}.
...
To establish a concrete classical simulacrum, we begin by reviewing the explanatory model introduced by Peres in~\cite{peres222}. Peres proposed a system consisting of a bomb that is initially at rest, with zero angular momentum, which subsequently explodes into two fragments carrying opposite angular momenta.
...
Due to the inherent and subjective nature of each experimenter $\mathbf{A}$ and $\mathbf{B}$, as well as the prevailing circumstances, the observed outcome $\mathbf{r}{\boldsymbol{\alpha}}$ or $\mathbf{r}{\boldsymbol{\beta}}$ of any single observation appears to be uncontrollable and unpredictable.
...
These outcomes seem to be spontaneously and continuously created,
as if by a mysterious process, in relation to the measurement apparatus and the environment of sides $\mathbf{A}$ and $\mathbf{B}$, which may be spatially separated.
...
For pairs of outcomes associated with two fragments of the same bomb, it is always observed that when both experimenters $\mathbf{A}$ and $\mathbf{B}$ measure the same observable $\mathbf{r}{\boldsymbol{\alpha}}$, i.e., $\boldsymbol{\beta}=\boldsymbol{\alpha}$, they end up with exactly inverse events. Specifically, if $\mathbf{A}$ obtains outcome $\mathbf{r}{\boldsymbol{\alpha}} = \pm 1$, then $\mathbf{B}$ obtains outcome $-\mathbf{r}_{\boldsymbol{\alpha}} = \mp 1$ along the same direction $\boldsymbol{\alpha}$, and vice versa. This phenomenon arises from the configuration of the original angular momentum being zero and the conservation of momentum, where if $\mathbf{A}$ measures $\pm \mathbf{J}$, then $\mathbf{B}$ measures $\mp \mathbf{J}$,
and vice versa.
...
One may also consider that the interface or Heisenberg cut is located at the share, i.e., the individual fragments themselves.
...
and in the discrete generalized urn model described earlier, it is the random selection of particular elements from the set $\mathbf{H}$ of ball pairs.
...
When a state specification is shifted or rescrambled to relational properties, it results in a tradeoff or a zero-sum game of information:
...
albeit potentially hidden, information that corresponds to or determines the single outcomes encoded in the quantum share.
...
Moreover, in such cases, the temporal succession of events or outcomes becomes a matter of conventions determined by observational means, as it depends on the reference frame,
...
I purposely used the term 'isolated' in quotations, because the concept of isolation is a fallacy, an illusion, a fantasy. In the realm of quantum theory,
...
Einstein proposed a solution to the challenges of quantum entanglement by rejecting the idea of wave function completeness and embracing the concept of hidden parameters. For example, a wave function such as $\vert \Psi \rangle = \frac{1}{2}\left(\vert +- \rangle - \vert -+ \rangle \right)$ could correspond to a macrostate that groups or bundles together $\vert +- \rangle$ and $\vert -+ \rangle$. The microstate, for a specific measurement direction $\boldsymbol{\alpha}$, could then be either $\vert +- \rangle$ or $\vert -+ \rangle$.
...
An alternative approach to consider is the hypothesis of discarding the notion of spatio-temporal distinctness among the elements of an entangled state, which would challenge the principle of Einstein locality. This paradigm shift would necessitate a comprehensive reevaluation of how space-time categories are formed. We plan to address this intriguing question in a forthcoming publication, where we will delve deeper into this concept.
...
and satisfies criteria value indefiniteness and relational encoding mentioned earlier.
...
\begin{center}
\begin{tabular}{c}
\includegraphics[width=0.3\textwidth]{2022-epr-GPT35-figure0.eps}
\\
(a)
\\
\\
\includegraphics[width=0.3\textwidth]{2022-epr-GPT35-figure1.eps}
\\
(b)
\end{tabular}
\end{center}
...
The probability that the breaking point will be observed as lying between $\mathbf{J}_+$ and the projection of $\mathbf{A}$ onto $\mathbf{J}$,
denoted as $\mathbf{A}\mathbf{J}$, can be calculated as the length of the line segment $\overline{\mathbf{J}_+ \mathbf{A}\mathbf{J}}$,
...
\begin{figure}
\begin{center}
\includegraphics[width=0.5\textwidth]{2022-epr-GPT35-figure2.eps}
\end{center}
...
The elastic band model, when analyzed using a specific protocol, exhibits a correlation pattern that is similar to quantum mechanics, as shown by the expectation function~(\ref{2022-epr-efebm6}). This suggests that, like quantum mechanics, the elastic band model may also allow for violations of the CHSH inequality. However, this hypothesis is challenged by Peres' argument~\cite{peres222}, who thoroughly examined all classically possible configurations and demonstrated that the convex sum of their entries never violates the CHSH inequality.
This raises the question of how violations of the CHSH inequality could occur in the elastic band model, despite Peres' findings.
...
\begin{center}
\begin{tabular}{ccc}
\includegraphics[width=0.2\textwidth]{2022-epr-GPT35-figure3.eps}
\\
(a)
\\
\\
\includegraphics[width=0.3\textwidth]{2022-epr-GPT35-figure4.eps}
\\
(b)
\end{tabular}
\end{center}
...
The general goal of this paper was twofold. First, to point out that it is not sufficient to recover, by classical means, a quantum-type correlation function or some non-linear, trigonometric form (such as cosine) of probability for elementary propositions in order to fully reconstruct quantum predictions. And second, and more importantly, it has been argued that the mind-boggling features of quantum mechanics reveal themselves only through delayed choice measurements under strict Einstein locality conditions.
...
To gain a deeper understanding of these phenomena, we constructed a classical model based on Aerts' elastic band model, with certain modifications.
