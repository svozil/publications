...
\chapter{Crash retrievals and UFO materials}
...
conversation between James Iandoli~\index{Iandoli, James} and  Luis Elizondo\index{Elizondo, Luis}
...
Several individuals attempted to locate Robert Sarbacher,\index{Sarbacher, Robert Irving} who was living in Florida at the time~\cite{DolanDisclosure2018Oct}. UFO investigator William Steinman was successful in reaching Sarbacher and had several phone conversations with him. As a result, Sarbacher wrote a two-page, single-spaced typed letter detailing his meeting with Smith. In this letter, dated November 29, 1983, Sarbacher confirmed the following~\cite{Sarbacher-83}:
\begin{svgraybox}
\begin{enumerate}
...
John von Neuman\index{von Neumann, John} was definitely involved. Dr. Vannevar Bush\index{Bush, Vannevar}
...
The complaint to ICIG also mentioned a previous confidential complaint by Grusch in July 2021,
...
Currently, such material and information, if administered by the DOD, by the Intelligence Community or by the DOE
...
%A first attack on such a database would be to identify its hardware. Suppose for a moment we had retrieved a (partially) intact flying saucer from a crash. How should we go about identifying potential storage facilities in such a craft? I suggest that, as any such storage can be assumed to be ``very advanced,'' its material layer can be expected to be ``very complex''---that is, the information density stored per unit amount of whatever it is made of will be high. That in turn means that such a memory device, if analyzed on an atomic level, should appear to be highly irregular, randomly aligned, akin to white noise. There is an almost paradoxical feature of highly organized, dense coding: the ciphertext appears random in the very precise sense of algorithmic information theory (where randomness means roughly algorithmic incompressibility). So, one criterion to identify storage on such a recovered craft would be that it looks disorganized and ``amorphous.''
...
One argument against the conventional search for extraterrestrial intelligence (SETI) is that any signals emitted by a civilization that are identifiable through conventional analysis might only be detectable for a relatively short period, perhaps just one or two centuries. This is due to the fact that as technology progresses and broadcasts become digitized, the emitted signals tend to approach white noise, making them much more difficult to analyze and decipher. Therefore, traditional SETI approaches that rely solely on detecting narrowband radio signals may
