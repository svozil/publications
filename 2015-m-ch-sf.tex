...
\marginnote{This chapter follows several approaches:
...
resulting from the assumption of merely regular singular poles, can be estimated as follows.
...
$
does not grow faster than $\vert x \vert^{k-1}$ (meaning that it is bounded by some constant times $\vert x \vert^{k-1}$),
and
$
P_2 (x) = p_2(x)\prod_{j=1}^k (x-x_j)^2
$
%\label{2012-m-ch-sf-eforp12}
does not grow faster than $\vert x \vert^{2k-2}$ (meaning that it is bounded by some constant times $\vert x \vert^{k-2}$).
...
the following equations, which are helpful in determining the characteristc exponent $\sigma$:
\begin{equation}
\begin{split}
...
P_0(x) =1,\\
P_1(x) =x,\\
P_2(x) =  \left. \left( x^2 - \frac{1 }{ 3}\right)\middle/\frac{2 }{ 3}  \right.
