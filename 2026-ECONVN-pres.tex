...
% Self-contained; compile twice with pdflatex 2026-trade-pres.tex.
\newcommand{\doiref}[2]{%
...
% Slide 2.
\begin{frame}{``Message'': Finite profits are possible, but not programmable}
\small
...
\item[Wait:] hold no shares and earn zero trading gain.
\end{description}
\end{exampleblock}
...
Positions are whole shares or fractions of integers (\emph{rational} amounts).
\end{alertblock}
\end{frame}

% Slide 4.
\begin{frame}[shrink=8]{Context: Cantor's diagonal idea, also used by G\"odel, Turing et al}
\small
Proof by contradiction: Suppose there exist a \emph{complete} list claimed to contain \emph{all} infinite sequences of zeros and
...
For every deterministic trading program that returns a whole or fractional
share position on every finite history, there is a fixed, positive,
computable price path on which its total trading gain is never positive (price strictly above zero)
at any finite date.
\end{block}
...
section \enquote{A Computable Countermarket for Every Trader} (2026)}.}
\end{frame}
...
\textbf{We construct a price history that makes this rule lose.}
\end{block}
...
\item The trader holds no shares.\newline
We leave the price at $99$. Its trading gain is zero.
\end{enumerate}
\end{exampleblock}
\begin{alertblock}{Why this is a counterexample}
The proposed rule claims a strictly positive gain on every price history.
The history we have constructed instead produces a loss.
Therefore, this rule cannot satisfy its claim of universal profitability.
\end{alertblock}
\end{frame}
...
The trader returns its position in finite time. Whole shares and fractions
of integers can be compared with zero exactly. Each next price is therefore
computable.
\end{exampleblock}
...
There need not be one path defeating all traders simultaneously.
\end{exampleblock}
\begin{alertblock}{The precise scope of the construction}
The countermarket need not be typical, likely, or an economic equilibrium.
Its role is to refute a claim about \emph{every} positive computable price path.
The argument uses direct diagonal opposition and needs no Halting-Problem
reduction. The following results address separate questions.
\end{alertblock}
\end{frame}
...
% Slide 15.
\begin{frame}[shrink=8]{Turing: will the price ever rise?}
\small

\begin{block}{An artificial market controlled by a program}
Take any program with a fixed input. Start the price at $1$.
Execute one instruction of the program per day.
\begin{itemize}
\item While the program keeps running, the price stays at $1$.
\item If the program stops, the price becomes $2$ and stays there.
\end{itemize}
The prices simply indicate whether the program has stopped.
\end{block}

\begin{exampleblock}{We can calculate the price on any specified day}
\centering
...
We never need to wait indefinitely.
\end{exampleblock}

\begin{alertblock}{But will the price \emph{ever} rise?}
This asks whether the program will \emph{ever} stop:
the \emph{Halting Problem}.
Turing proved that no algorithm can always answer this correctly
for every program and input.
\par\smallskip
Therefore, even with the price generator's code, no algorithm can
decide for every such market whether its price will ever rise.
\end{alertblock}
...
Fix a programming language. Among programs of at most $n$ bits that
eventually stop, take the longest running time. This is the runtime
...
Use a binary record with independent, equally probable zeros and ones.
A \emph{Martin-L\"of-random} infinite record escapes every algorithmically
specified statistical test whose exceptional sets have probabilities
shrinking effectively to zero.
\end{block}
\begin{alertblock}{The betting consequence}
In the corresponding fair betting game, no computable strategy that keeps
its capital nonnegative can make that capital unbounded on such a record.
