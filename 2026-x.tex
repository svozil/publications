...
A stronger objection must be kept separate from the merely sociological one. It
says not only that present models lack taste or embodiment, but that humans have
access to a kind of mathematical or ontological insight unavailable to language
models: a direct relation to truth, perhaps Platonistic, perhaps religious or
dualistic, perhaps an encounter with the ineffable. I do not adopt that
metaphysics here. But it is important enough to treat explicitly, because it is
one of the few objections that, if true, would draw a principled rather than
merely technological boundary between artificial formalization and human
understanding.
...
\subsection{Intuition, Platonism, and the alleged access to the ineffable}
\label{sec:intuition-ineffability}

A more radical defense of the human prerogative does not appeal to social
history but to ontology. It holds that humans, unlike statistical machines, can
encounter truth directly: through mathematical intuition, through a Platonistic
faculty aimed at an objective realm, or through some interface to what ordinary
language cannot express. The contemporary literature on ineffability is relevant
precisely because it clarifies what such claims would have to mean. Jonas, for
example, analyzes the metaphysics of the unspeakable in art, religion, and
philosophy and argues that many strong versions of ineffable content are not
coherent objects of assertion~\cite{Jonas-ineffability}. That does not settle the
question of mathematical discovery, but it prevents an easy move: one cannot
secure human superiority merely by placing the decisive insight beyond
articulation and then using it to win an articulated argument.

G\"odel is the canonical modern case. His philosophical writings defend a form
of mathematical realism or Platonism, according to which mathematical truth is
objective and mathematical intuition is not reducible to formal syntax; this has
been carefully reconstructed by Parsons and others~\cite{godel1964cantor,parsons1995platonism}. His 1949 rotating-universe solution of Einstein's field
equations is a useful emblem, though not proof, of the point: G\"odel presented a
striking exact solution with profound conceptual consequences for time and
closed timelike curves~\cite{goedel1949cosmological}. Anecdotes about how
``suddenly'' such a solution was found should be treated cautiously unless tied
to a documented source. What is secure is weaker and more interesting: the
mathematical act looks from the outside like an abrupt compression of long
implicit search into a compact formal object.

Ramanujan supplies an even sharper example of the distinction between insight
and proof. He often recorded results with little or no derivation, and Hardy's
collaboration was partly an effort to translate extraordinary formulaic
intuition into proofs acceptable to the mathematical community~\cite{hardy1940ramanujan}. Yet Ramanujan is not evidence for an infallible
oracular faculty. Later editorial work on the notebooks found a small number of
incorrect assertions among thousands of results; Berndt reports that, in the
earlier notebooks, among more than 3200 results he counted no more than about a
half-dozen incorrect ones~\cite{berndt1998ramanujan,berndt2020living}. The lesson
is therefore not that intuition bypasses proof, but that intuition may generate
high-value conjectural structure far faster than proof can domesticate it.

Other classical testimonies point in the same direction without requiring
mysticism. Poincar\'e's famous descriptions of mathematical invention emphasize
unconscious recombination followed by conscious verification; Hadamard collected
similar reports from mathematicians on imagery, sudden illumination, and delayed
formal control~\cite{poincare1908science,hadamard1945psychology}. These cases are
important because they are compatible with the central claim of this paper. They
show that discovery may begin in non-discursive, imagistic, or apparently
pre-formal states. They do not show that the resulting mathematics is licensed
without public formalization. The private flash becomes science only when it
enters the shared regime of derivation, criticism, and repair.

Thus the strongest humanist objection should be stated precisely. It is not that
LLMs currently fail at one-shot proof, since that may be a temporary engineering
fact. It is that there might exist modes of ontological or mathematical access
whose contents are not recoverable from linguistic, statistical, or formal
operations. This paper leaves that metaphysical possibility open, but gives it no
free epistemic privilege. If an insight cannot be formalized, tested, or made to
bear consequences, it remains private. If it can, then the artificial
\emph{Rechner} re-enters: the issue becomes not where the image came from, but
whether it can be made accountable.
...
\subsection{Boring prose and homogenized style}
\label{sec:boring-prose}

A further objection is stylistic: model-assisted prose often sounds competent but
boring. This is partly true, but imprecise. The defect is less boredom than
regression toward a safe, high-probability register: balanced clauses, hedged
transitions, symmetric paragraphs, and a mild institutional tone. Recent work on
LLM-assisted writing reports measurable homogenization effects in creative and
expressive tasks, while other studies qualify how strong or domain-dependent
those effects are~\cite{padmakumar2024homogenization,anderson2024homogenization,ghods2024against}. The practical lesson is not to reject machine drafting, but to
treat style as another object of adversarial revision. The author must restore
asymmetry, compression, anger where warranted, rhythm, local risk, and the
unusual image---features that are often precisely what a generic model smooths
away.

This stylistic limitation also bears on the ontology question of
Sec.~\ref{sec:intuition-ineffability}. If human insight enters first as an image,
a tension, or a taste for an abnormal formulation, then prose homogenization is
not merely cosmetic. A boring sentence can be an epistemic failure when it
normalizes a thought whose force lies in its deformation of ordinary language.
The remedy is again not mysticism but responsibility: the model may supply the
first formal or rhetorical closure, but the author must decide where closure has
become premature smoothing.
...
deskilling, compute inequality, homogenization of scientific taste and prose,
and the conversion of machine objection lists into unreasonable burdens on
authors. The claim is narrower: embedded in audited, adversarial,
tool-grounded loops,
...
formalized, criticized, and made responsible. Claims about ineffable or
Platonistic access to truth may mark a possible metaphysical difference between
human and machine cognition, but they do not by themselves exempt an insight
from public formalization. Even this division of labour is contingent on systems
that may yet learn to supply the images too.
