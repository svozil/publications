...
into a hypergeometric equation.
...
it generalizes the factorial to real or complex arguments (different from the negative integers and from zero); that is,
\begin{equation}
...
\end{equation}
and thus, because for very large $n$ and constant $z$ (i.e., $z\ll n$),
$(z+n)\approx n$,
\begin{equation}
\begin{split}
\Gamma ( z  )
...
\end{split}
\end{equation}
$\Gamma (z+1)$ has thus been redefined in terms of $z!$ in Eq. (\ref{2011-m-ch-sfsf1}),
which, by comparing Eqs.
(\ref{2011-m-ch-sfsfdga})
and
(\ref{2011-m-ch-sfsfdga234}),
also implies that
\begin{equation}
\Gamma ( z + 1)
=  z
...
of $\Gamma(z)$
by partial integration; that is,
\begin{equation}
\begin{split}
\Gamma ( z+1 )
=
...
We also mention without proof following the formul\ae:
\begin{equation}
...
\end{equation}
\begin{equation}
\textrm{Euler's reflection formula } \Gamma ( x)\Gamma ( 1-x) =\frac{\pi}{\sin (\pi x)} .
\end{equation}
...
The simplest case is if $a_0(x)$ has a {\em simple zero} at $x_0$;
...
if
\begin{equation}
\lim_{x\rightarrow x_0} (x-x_0)\frac{ a_1(x)}{a_2(x)}\textrm{, as well as }
\lim_{x\rightarrow x_0} (x-x_0)^2 \frac{a_0(x)}{a_2(x)}
\end{equation}
both exist.
If any one of these limits does not exist, the singular point is
an
...
\end{equation}
This equation is Fuchsian if $0$ is a ordinary point,
...
Let us hint on the functional form of
the coefficients $p_1(x)$ and $p_2(x)$,
resulting from the assumption of regular singular poles.

First, let us start with poles at finite complex numbers.
Suppose there are $k$ finite poles (the behavior of
$p_1(x)$ and $p_2(x)$ at
infinity will be treated later).
Hence, in Eq. (\ref{2011-m-ch-sf-fc2}),
...
\end{equation}
where the $x_1,\ldots ,x_k$ are $k$ the points of the (regular singular) poles,
and $P_1(x)$ and $P_2(x)$ {\em entire functions;}
that is, they are analytic (or, by another wording, holomorphic)
over the whole complex plane formed by $\{x \mid x \in {\Bbb C} \}$.



Second, consider possible poles at infinity.
Note that,
as has been shown earlier,
because of the requirement that infinity is a regular singular point,
$p_1(x)x$
as well as
$p_2(x)x^2$,
as $x\rightarrow \infty$ approaches infinity,
...
$
does not grow faster than $\vert x \vert^{k-1}$,
and
$
...
both
$P_1(x)$ and $P_2(x)$ must be rational functions
-- that is, polynomials of the form $\frac{P(x)}{Q(x)}$ --
...
\cite{KristenssonC3}
of the rational functions in terms of their pole factors $(x-x_j)$,
...
\subsection{Frobenius method by power series}
...
\left[
({l + \sigma})({l + \sigma-1})   w_l \right. \qquad \qquad \\
...
\end{split}
\end{equation}
as the greater critical exponent $\sigma_1$   is a solution
of Eq. (\ref{2011-m-ch-sf-pss2sigma}) and thus vanishes, leaving us with a vanishing denominator.
...
+\left[ \frac{d  }{dx  }y_1(x)\right]    v(x)
+y_1(x)  \left[ \frac{d  }{dx  } v(x) \right]   \qquad  \\
...
+   p_2 (x)  y_1(x)\int_x  v(s) ds   \qquad    \\
+   p_1  (x)   y_1(x)   v(x)
+\left[ \frac{d  }{dx  }y_1(x)\right]    v(x)
+\left[ \frac{d  }{dx  }y_1(x)\right]    v(x)
...
+   p_2 (x)  y_1(x)\right\}}_{=0}\int_x  v(s) ds    \qquad \qquad   \\
+y_1(x)  \left[ \frac{d  }{dx  } v(x) \right]
...
the following equations, which are helpful in tetermining the characteristc exponent $\sigma$:
\begin{equation}
\begin{split}
...
$\alpha_n \stackrel{{\tiny \textrm{ def }}}{=} \tilde a_{n-1}$; then
$$ p_1(z)
...
$$
   p_1(z)= \frac{a_1(z)}{(z-z_0)a_2(z) }={(z-z_0) p_1(z)\over(z-z_0)}
$$
and
$$
   \alpha_0={1\over2\pi i}\oint{p_1(s)(s-z_0)\over(s-z_0)}ds,
$$
where $(s-z_0)p_1(s)$ is analytic around $z_0$; hence we can apply Cauchy's integral formula:
$$
...
$$\beta_0={1\over2\pi i}
\oint p_2(s)(s-z_0)ds.$$
Because the equation is Fuchsian,
$p_2(z)$ has only a pole of the order of two at $z_0$; and $p_2(z)$ is of the form
$$
   p_2(z)= \frac{a_2(z)}{(z-z_0)^2 a_2(z)}={(z-z_0)^2 p_2(z)\over(z-z_0)^2}
$$
...
\end{equation}
where the quotients $\frac{c_{j+1}}{c_j}$ are {\em rational functions} (that is, the quotient of two polynomials) of $j$, so that they can be factorized by
\index{rational function}
\begin{equation}
\begin{split}
\frac{c_{j+1}}{c_j}
...
\end{equation}
The factor $j+1$ in the denominator has been chosen to define the particular factor $j!$
in a definition given later and below; if
it does not arise ``naturally''
we may just obtain it by compensating it with a factor $j+1$ in the numerator.
...
The system of polynomial functions $\{1,x,x^2,\ldots ,x^k,\ldots \}$
...
Because, as pointed out earlier in the contect of hypergeometric functions,
they could be alternatively defined as the eigenfunctions and solutions of certain differential equation,
such as, for instance, the Schrödinger equation, which may be subjected to a separation of variables.
...
-- an equation
(note that  the quantum mechanical ``momentum operator'' ${\cal P}$ is identified with  $- i \hbar \nabla$)
\begin{equation}
\begin{split}
...
\textrm{  or, with } V=-\frac{e^2}{4\pi \epsilon_0 r}, \\
 -\left[ \frac{\hbar^2}{2\mu }\Delta +  \frac{e^2}{4\pi \epsilon_0r} \right]\psi ({\bf x})= E\psi, \\
 \textrm{ or }\left[ \Delta + \frac{2\mu }{\hbar^2} \left(\frac{e^2}{4\pi \epsilon_0r} + E \right)\right]\psi ({\bf x})=0,
\end{split}
...
Schr\"odinger might also have hinted that $\mu$, $e$, and $\epsilon_0$ stand for some (reduced) mass, charge, and the  permittivity of the vacuum, respectively,
...
essentially ``decays into'' (i.e. consists additively of)
...
\end{equation}
so that, after division by $\psi (r, \theta ,\varphi  )= R(r)\Theta(\theta)\Phi(\varphi)$
...
\end{equation}
As $\Phi$ must obey the periodic boundary conditions $\Phi(\varphi)=\Phi(\varphi  +2\pi)$,
...
\qquad =
\left. -\frac{i e^{i(m-n)\varphi}} { 2(m-n)\pi  } \right|_{\varphi =0}^{\varphi = 2\pi}\\
\qquad = 0,
\end{split}
...
Because  if the sum would be infinite, the terms $a_k$, for large $k$, would be dominated by $a_{k-2} O( k^2/k^2)= a_{k-2} O(1)$,
and thus
...
-
2
\frac{\mu e^2}{4\pi \epsilon_0\hbar^2} r
...
More explicitly,
$r=ya_0= y  (4\pi \epsilon_0 \hbar^2)/(m_e e^2)$,
or $y= r/a_0= r  (m_e e^2)/(4\pi \epsilon_0 \hbar^2)$.
Furthermore, let us define $\varepsilon = E \frac{2\mu a_0^2}{\hbar^2}$.
...
(It will later be argued that $\varepsilon$ must be dicrete;  with $n\in {\Bbb N}-0$.)
This yields
\begin{equation}
\begin{split}
...
and thus  $\hat{R}(  \xi )$ would roughly behave as $e^\xi$
--
the series solution
...
And since $k\ge 0$, $n$ must be at least $l+1$, or
\begin{equation}
l\le n-1 .
