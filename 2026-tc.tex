...
The classical resource is fragile, with its mutual information degrading
as $\Theta(\delta\theta\ln(1/\delta\theta))$ under a
misalignment~$\delta\theta$, whereas the quantum resource is robust,
degrading only as $\Theta(\delta\theta^{2}\ln(1/\delta\theta))$.
...
\blue{Sagawa and Ueda~\cite{SagawaUeda2008,sagawa2010} established
...
\blue{Section~\ref{sec:protocol} describes the operational
...
\blue{Section~\ref{sec:CHSH_work} defines and evaluates an aggregate
...
\label{eq:Itheta}
\end{equation}
This result depends on the correlation $E(\theta)$ \emph{only through
...
Note that Eq.~\eqref{eq:Itheta} is \emph{model-independent}: it holds for any bipartite device
...
\begin{enumerate}
  \item \textbf{Classical (linear / ``triangle'' model):}
  \[
  ...
  attains the local bound $\mathcal{S}_{\mathrm{CHSH}}^{(c)}=2$.\footnote{%
...
$\Theta(\delta\theta\ln(1/\delta\theta))$\blue{---worse than linear}:
...
\begin{table*}[t]
\blue{\caption{\label{tab:summary}
Summary of the three correlation classes.
...
\blue{For binary outcomes, the mutual information is bounded by
...
depends only on the \blue{leading-order scaling of $1 + E(\theta)$
...
The work bound~\eqref{eq:work_from_I} is an asymptotic (many-copy, or
...
\blue{(in nats, consistent with the natural-logarithm convention used
...
$E(\theta)$.
However, the quantitative values of the \blue{aggregate robustness}
parameter
...
\blue{Zurek~\cite{Zurek2003} connected quantum discord to the work
...
\blue{Funo \emph{et al.}~\cite{funo2013} showed that entanglement can
...
\blue{Gallego \emph{et al.}~\cite{gallego2010} studied operational
...
hierarchy depends only on the \blue{leading-order scaling} of
$E(\theta)$ at its extrema and is therefore generic within each theory.
\blue{We emphasize that the robustness comparison is made within the
