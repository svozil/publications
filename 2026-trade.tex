...
orders, and adaptation by other participants~\cite{Lo2004}. This falsifiable restriction,
...
testing (reversing or otherwise reordering a record), and the conditions under
which strategies may work ``for all practical purposes'' (FAPP, following
Bell~\cite{bell-a}).
...
The countermarket construction uses insider knowledge of the trader's
algorithmic strategy, namely access to its code. Without this knowledge, an
antagonistic market that must anticipate the next position from past trades
faces the same rule-inference limits as the trader facing an unknown market.
Even traders whose buy-or-sell decisions depend only on time can generate
every computable binary sequence, so Gold's limit (Section~\ref{sec:learning}) applies to identifying their
rules from history~\cite{go-65,go-67}. This concerns rule identification, not
the failure of every antagonistic action. Unbounded questions about a trader,
such as whether it will ever take a nonzero position, can also encode the
Halting Problem and remain undecidable even when its code is supplied
(Section~\ref{sec:halting}). If the market instead observes the current
position before setting the next price, it can oppose that position directly
without knowing or inferring the strategy.
\end{remark}
...
These limits leave room for FAPP profitability under specified market
conditions, lawful information, or compensation for risk. Such claims remain
conditional and must be evaluated after costs and financing, against an
explicit benchmark. The selection controls in Section~\ref{sec:ramseytrap}
and the possibility that deployment changes the market remain essential;
neither establishes a universal profit guarantee.
...
(FWF), Grant DOI: \href{https://doi.org/10.55776/PIN5424624}{10.55776/PIN5424624}. For open access purposes, the author
