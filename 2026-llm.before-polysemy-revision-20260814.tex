...
structural and deliberately limited.  A final appendix, \emph{A minimal
quantum attention model}, presents a finite-dimensional construction with
complex amplitudes, learned unitary evolution, optional partial tracing, and
phase-sensitive projective Born-rule predictions.
...
attention model}, presents a quantum model in its own right rather than a
reinterpretation of ordinary transformer attention.  Additional formal
...
\subsection{A sense is not a single feature} [[what does that mean? Incomprehensible]]
...
[[please introduce sense first, then polysemy]]
A \emph{sense} is a contextually appropriate meaning of a lexical item.
[[Take a simple example, and consider the wort (maybe token? ``bank'':
There exist at least three contextual meanings of the same word (token?) ``bank'':]]
[[(i)]] financial, [[(ii)]] river-edge, and [[(iii)]] seat meanings of ``bank'' are different senses.
The banking or tilting of an aircraft supplies another[[, forth, sense]].

A semantic feature is
narrower: it may mark animacy, tense, number, sentiment, or one fitted gender
contrast.  [[I dont understand this...what is the exact difference between sense and semantic feature?]]

 Several features can contribute to one sense, and one feature can
recur in many words and senses.

Polysemy is therefore not solved merely by
finding one direction for every dictionary definition. [[aka sense or what? This is blabla-blurbing]]
...
vectors for its occurrences.  These are related but not identical problems. [[what does that mean?---again blabla-blurbing?]]

\subsection{A static vector as a corpus-level mixture}

In a static system such as word2vec,[[quotation]] the same stored type vector is used in
every occurrence.  This may appear poorly suited to polysemy.

[[Presently, polysemy is handled differently:]] Under[[?]] a
particular generative model,  Arora and collaborators establish an
...
\begin{aligned}
 &v_{\rm bank}\approx
 p_{\rm fin}v_{\rm fin}  [[please write out {\rm finance} instead, everywhere it occurs]]
...
\subsection{Why this is not quantum coherence}
...
transformer processing supplies no Born rule and no physical collapse.
The
explicit pure-state and density-matrix comparison is given in
Appendix~\ref{app:polysemy}.
[[Is it, really?]]
Appendix~\ref{app:quantum-attention}, a \emph{minimal quantum attention model}, supplies such a specification, including a
...
[[Also relevant here: orthogonality in the LLM context means INDEPENDENCE,
whereas in quantum mechanics it means: belonging to the SAME CONTEXT--- this is almost dia... different! ]]
...
untied contextual models.
[[Blablabla-blurb?: The analogy is structural, not a claim that
polysemy is a quantum measurement. ]]
Appendix~\ref{app:polysemy} gives the
shared-projector construction [[really?]]and explains why quantum probability rules do
...
[[So: Whereas in the current scheme polysemy is handled as a convex real combination of vectors, in the new,, quantum-logic inspired scheme,
polysemy is modelled by common vectors to intertwining contexts aka senses(?) or semantics(?)]]
% ======================================================================
\section{Conclusion}
...
\section{A minimal quantum attention model}
\label{app:quantum-attention}

This appendix defines a \emph{minimal quantum attention model}, formulated in
a finite-dimensional complex Hilbert space.  Its ingredients are
...
amplitudes are first obtained from Eq.~\eqref{eq:mqa-scores} and $W_i$ is then
...
contrast, Eq.~\eqref{eq:mqa-mixture} gives the phase-insensitive state
...
Eq.~\eqref{eq:mqa-unitary}, are freshly prepared from classically specified
