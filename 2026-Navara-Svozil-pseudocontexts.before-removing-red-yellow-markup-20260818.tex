...
\corr{An admissible weight whose range is contained in} \corrmath{\{0,1\}}
\corr{is called a two-valued state.  Classical deterministic models use
two-valued states and their convex combinations.}  A quantum realization assigns rays to the vertices and uses
...
The forcing proof contains no coordinates \corr{and is independent of the
Hilbert-space scalar field}.  Realizability
...
\corr{The following threshold is relative to Definition}~\ref{def:pseudocontext},
\corr{which stipulates} \corrmath{k\ge2}\corr{.}

\begin{proposition}[No size-one Hilbert-space pseudocontext]
\label{prop:no-k1}
\corr{Even if Definition}~\ref{def:pseudocontext} \corr{were provisionally
extended to} \corrmath{k=1}\corr{, no triple satisfying its conditions could admit a
FOR in a real or complex Hilbert space.}
\end{proposition}

\begin{proof}
\corr{Suppose such a triple had} \corrmath{A=\{a\}}\corr{,}
\corrmath{B=\{b\}}\corr{, a~balanced certificate} \corrmath{\lambda}\corr{,
and a FOR} \corrmath{h}\corr{.  Multiplying the projector equations for the
contexts by the corresponding coefficients} \corrmath{\lambda_E}\corr{,
summing, and using the certificate equations gives}
\[
 \corrmath{P_{h(a)}-P_{h(b)}
 = (\one_\E^{\mathsf T}\lambda)I_H=0.}
\]
\corr{Equality of the rank-one projectors means equality of the rays,
contradicting injectivity of} \corrmath{h} \corr{because} \corrmath{a\ne b}\corr{.}
\end{proof}
...
\begin{proof}
\corr{Suppose a FOR supplies unit representatives}
$a_1,a_2,b_1,b_2$ in a real Hilbert space satisfying
\[
 P_{a_1}+P_{a_2}=P_{b_1}+P_{b_2}=:T.
\]
The range of $T$ is the common two-dimensional span of both pairs.
\corr{Internal nonorthogonality gives}
\corrmath{\langle a_1,a_2\rangle\ne0}\corr{.  Change the sign of}
\corrmath{a_2} \corr{if necessary and set}
\corrmath{c=\langle a_1,a_2\rangle>0}\corr{.  Faithfulness makes the rays}
\corrmath{[a_1]} \corr{and} \corrmath{[a_2]} \corr{distinct, so}
\corrmath{c<1}\corr{.  Hence} \corrmath{0<c<1}\corr{.}
In an eigenbasis of $T$ its eigenvalues are
$1+c$ and $1-c$, which are distinct.  Write the two unit vectors of any
decomposition in this eigenbasis as
$(\cos\phi,\sin\phi)$ and $(\cos\psi,\sin\psi)$.  \corr{The diagonal and
off-diagonal entries give}
\[
 \corrmath{\cos^2\phi+\cos^2\psi=1+c},\qquad
 \corrmath{\sin^2\phi+\sin^2\psi=1-c},
\]
\[
 \corrmath{\sin\phi\cos\phi+\sin\psi\cos\psi=0.}
\]
\corr{Subtracting the first two equations and applying the doubled-angle
identities yields}
\[
 \corrmath{\cos(2\phi)+\cos(2\psi)=2c},\qquad
 \corrmath{\sin(2\phi)+\sin(2\psi)=0.}
\]
\corr{Equivalently,}
\[
 \corrmath{e^{2i\phi}+e^{2i\psi}=2c.}
\]
Two points on the unit circle with this sum are
...
Thus no allowed real size-two pair exists.
\end{proof}
...
\label{thm:threshold}
\corr{Under Definition}~\ref{def:pseudocontext}, \corr{the minimum permitted
target size} \corrmath{k=2} \corr{is attained by a pseudocontext admitting a FOR
over} \corrmath{\C}\corr{.  The minimum size over} \corrmath{\R} \corr{is}
\corrmath{3}\corr{.}
\end{theorem}

\begin{proof}
\corr{The lower bound} \corrmath{k\ge2} \corr{over} \corrmath{\C}
\corr{is part of Definition}~\ref{def:pseudocontext}, \corr{and the octagon
with a FOR from Section}~\ref{sec:octagon} \corr{attains it over}
\corrmath{\C^3}\corr{.
Proposition}~\ref{prop:real-k2} \corr{excludes size two over} \corrmath{\R}\corr{,
while the construction of Section}~\ref{sec:mayet} \corr{attains size three in}
\corrmath{\R^3}\corr{.}
\end{proof}
