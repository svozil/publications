...
\title{Correlation-Powered Work: Equivalence in Peak Yield, Differences in Robustness}
...
\end{equation}
The first law for system A is $\Delta E_A = Q_E + W$, where $W$ is the work done \emph{on} A and $Q_E$ is the heat it absorbs from the bath. The standard non-equilibrium  (Helmholtz) free energy---the energy available in a system to do work at a constant temperature $T$---of A is $F_T(\rho_A) := \operatorname{Tr}[H_A \rho_A] - k_{\mathrm{B}} T S(\rho_A)$. For an ideal bath, the heat transfer is related to its entropy change by $Q_E = -k_{\mathrm{B}}T \Delta S(\rho_E)$.
...
&= \left[-k_{\mathrm{B}}T \Delta S(\rho_E) + W\right] - k_{\mathrm{B}} T \Delta S(\rho_A) \nonumber \\
&= W - k_{\mathrm{B}}T\left[\Delta S(\rho_A) + \Delta S(\rho_E)\right].
\end{align}
...
For the sake of differentiating various types of correlations (classical, quantum, stronger-than-quantum) we define the ``optimal'' Clauser-Horne-Shimony-Holt (CHSH) parameter
$\mathcal{S}_{\mathrm{CHSH}} := |E(\mathbf{a},\mathbf{b}) + E(\mathbf{a},\mathbf{b}') + E(\mathbf{a}',\mathbf{b}) - E(\mathbf{a}',\mathbf{b}')|$
derived from hull calculations of a classical correlation polytope~\cite{froissart-81,pitowsky-86}. Local realism demands $\mathcal{S}_{\mathrm{CHSH}} \le 2$, quantum mechanics permits $\mathcal{S}_{\mathrm{CHSH}} \le 2\sqrt{2}$ (the Tsirelson bound)~\cite{cirelson:80}, while the algebraic limit is 4. For any $E(\theta) \in [-1, 1]$, a valid no-signaling probability distribution with uniform marginals is $P(x,y|\theta) = \frac{1}{4}[1 + xy E(\theta)]$, and $x, y \in \{\pm1\}$ are the measurement outcomes for the two parties.
...
A simple local-hidden-variable model~\cite{peres222} gives the linear correlation $E_c(\theta) = -1 + 2\theta/\pi$. To test this against the CHSH inequality, we adopt the standard angles used to find the quantum bound: $\phi_a=0$, $\phi_{a'}=\pi/2$, $\phi_b=\pi/4$, and $\phi_{b'}=-\pi/4$. This choice results in three instances of the relative angle $\theta_1 = \pi/4$ and one instance of $\theta_2 = 3\pi/4$.

The correlations are $E_c(\pi/4) = -1 + 2(\pi/4)/\pi = -0.5$ and $E_c(3\pi/4) = -1 + 2(3\pi/4)/\pi = 0.5$. Substituting these values into the CHSH parameter yields:
$\mathcal{S}_{\mathrm{CHSH}}^{(c)} = |3 E_c(\pi/4) - E_c(3\pi/4)| = |3(-0.5) - 0.5| = |-2| = 2$.
This result correctly saturates the classical bound predicted by local realism.

\subsection{Quantum (Cosine) Correlation}
For a two-qubit singlet state, measurements yield $E_q(\theta) = -\cos\theta$. With the standard CHSH angles $\phi_a=0$, $\phi_{a'}=\pi/2$, $\phi_b=\pi/4$, and $\phi_{b'}=-\pi/4$, the relative angles are $\pi/4$ and $3\pi/4$. The correlations are $E_q(\pi/4)=-\sqrt{2}/2$ and $E_q(3\pi/4)=\sqrt{2}/2$, giving $\mathcal{S}_{\mathrm{CHSH}}^{(q)} = 2\sqrt{2}$.

This value, $2\sqrt{2}$, is the maximal allowed in quantum theory. To see this, define the CHSH operator $\mathcal{B} = A\otimes(B+B') + A'\otimes(B-B')$, where $A, A', B, B'$ are Hermitian operators with eigenvalues $\pm1$. Since $A^2=I$, etc., the square of the operator is $\mathcal{B}^2 = 4I\otimes I + [A,A']\otimes[B,B']$. The operator norm is bounded by $\|[X,Y]\| \le 2\|X\|\|Y\|$, so $\|[A,A']\| \le 2$ and $\|[B,B']\| \le 2$. Thus, $\|\mathcal{B}^2\| \le 4+4=8$. It follows that $\|\mathcal{B}\| \le  2\sqrt{2}$, which bounds its expectation value $\mathcal{S}_{\mathrm{CHSH}}=|\langle\mathcal{B}\rangle|$ for any quantum state.
...
\end{equation}
Using the same set of relative angles as before, $\theta_1 = \pi/4 < \pi/2$ and $\theta_2 = 3\pi/4 > \pi/2$, we find the correlations to be $E_s(\pi/4)=-1$ and $E_s(3\pi/4)=+1$. The CHSH parameter thus becomes:
$\mathcal{S}_{\mathrm{CHSH}}^{(s)} = |3(-1) - (+1)| = 4$.
This value saturates the algebraic maximum of 4, representing the strongest possible correlation consistent with the no-signaling principle.
...
\section{An Energetic Formulation of the CHSH Parameter}
\label{sec:energetic_chsh}

The analysis in Sec.~\ref{sec:advantages} established an operational hierarchy based on the robustness of different correlation laws against misalignment. While the CHSH parameter $\mathcal{S}_{\mathrm{CHSH}}$ quantifies this from the perspective of non-locality, we can construct an analogous "energetic" parameter that captures the same hierarchy from a thermodynamic resource perspective. This is achieved by substituting the correlation function $E(\mathbf{a},\mathbf{b})$ in the CHSH expression with the corresponding extractable work potential, $W(\mathbf{a},\mathbf{b}) = k_{\mathrm{B}}T I(A:E)_{\mathbf{a},\mathbf{b}}$.

We thus define the energetic CHSH parameter, $\mathcal{S}_W$, as:
\begin{equation}
\mathcal{S}_W := |W(\mathbf{a},\mathbf{b}) + W(\mathbf{a},\mathbf{b}') + W(\mathbf{a}',\mathbf{b}) - W(\mathbf{a}',\mathbf{b}')|.
\end{equation}
By factoring out the constant term $k_{\mathrm{B}}T$, this becomes a direct combination of the mutual information values accessible across the four measurement settings of a Bell test:
\begin{equation}
\mathcal{S}_W = k_{\mathrm{B}}T |I_{\mathbf{a},\mathbf{b}} + I_{\mathbf{a},\mathbf{b}'} + I_{\mathbf{a}',\mathbf{b}} - I_{\mathbf{a}',\mathbf{b}'}|.
\end{equation}
This parameter does not represent the work from a single process, but rather quantifies the total thermodynamic resource available across the specific set of operations that constitute a CHSH test. A larger value of $\mathcal{S}_W$ signifies a greater and more resilient thermodynamic potential when subjected to the misaligned measurements inherent to demonstrating non-locality.

To compare the three correlation laws, we again adopt the standard CHSH angles that maximize the quantum value: $\phi_a=0$, $\phi_{a'}=\pi/2$, $\phi_b=\pi/4$, and $\phi_{b'}=-\pi/4$. This choice results in three instances of the relative angle $\theta_1 = \pi/4$ and one instance of $\theta_2 = 3\pi/4$. The energetic parameter for this specific configuration simplifies to:
\begin{equation}
\mathcal{S}_W = k_{\mathrm{B}}T |3I(\pi/4) - I(3\pi/4)|.
\end{equation}
We now evaluate this expression for our three models using their analytic forms.

\subsection{Classical Energetic CHSH Term}
The classical mutual information is $I_c(\theta) = \ln 2 - h_2(\theta/\pi)$.
The two required values are $I_c(\pi/4) = \ln 2 - h_2(1/4)$ and $I_c(3\pi/4) = \ln 2 - h_2(3/4)$.
Given the symmetry of the binary entropy function, $h_2(p) = h_2(1-p)$, it is clear that $h_2(1/4) = h_2(3/4)$, and thus $I_c(\pi/4) = I_c(3\pi/4)$.
Substituting this into the expression for $\mathcal{S}_W$ yields:
\begin{align}
\mathcal{S}_W^{(c)} &= k_{\mathrm{B}}T |3I_c(\pi/4) - I_c(\pi/4)| = 2 k_{\mathrm{B}}T I_c(\pi/4) \nonumber \\
&= 2 k_{\mathrm{B}}T \left[\ln 2 - h_2(1/4)\right].
\end{align}

\subsection{Quantum Energetic CHSH Term}
The quantum mutual information is $I_q(\theta) = \ln 2 - h_2(\sin^2(\theta/2))$.
The two values are $I_q(\pi/4) = \ln 2 - h_2(\sin^2(\pi/8))$ and $I_q(3\pi/4) = \ln 2 - h_2(\sin^2(3\pi/8))$.
Using the identity $\sin^2(x) + \cos^2(x) = 1$ and noting that $\sin(3\pi/8) = \cos(\pi/8)$, the arguments of the entropy function are $p = \sin^2(\pi/8)$ and $1-p = \cos^2(\pi/8) = \sin^2(3\pi/8)$. The symmetry of $h_2(p)$ again implies that $I_q(\pi/4) = I_q(3\pi/4)$.
The energetic parameter is therefore:
\begin{align}
\mathcal{S}_W^{(q)} &= k_{\mathrm{B}}T |3I_q(\pi/4) - I_q(\pi/4)| = 2 k_{\mathrm{B}}T I_q(\pi/4) \nonumber \\
&= 2 k_{\mathrm{B}}T \left[\ln 2 - h_2(\sin^2(\pi/8))\right].
\end{align}

\subsection{Stronger-than-Quantum Energetic CHSH Term}
The stronger-than-quantum mutual information is $I_s(\theta) = \ln 2$ for all $\theta \neq \pi/2$. As both $\pi/4$ and $3\pi/4$ satisfy this condition, we have $I_s(\pi/4) = I_s(3\pi/4) = \ln 2$.
The calculation is straightforward:
\begin{equation}
\mathcal{S}_W^{(s)} = k_{\mathrm{B}}T |3\ln 2 - \ln 2| = 2 k_{\mathrm{B}}T \ln 2.
\end{equation}

\subsection{Comparison}
To compare the three energetic CHSH values, we must compare the magnitude of the binary entropy terms being subtracted from $\ln 2$. The binary entropy $h_2(p)$ is a monotonically increasing function for $p \in [0, 1/2]$. We compare the arguments for the classical and quantum cases:
The classical argument is $p_c = 1/4$, whereas the quantum argument is $p_q = \sin^2(\pi/8) = \left[1 - \cos(\pi/4)\right]/2 = \left(2-\sqrt{2}\right)/4$.
Since $\sqrt{2} > 1$, we have $2-\sqrt{2} < 1$, which implies that $\left(2-\sqrt{2}\right)/4 <  {1}/{4}$.
Therefore, $p_q < p_c$. Because both arguments are less than $1/2$, it follows that $h_2(p_q) < h_2(p_c)$. This establishes a clear hierarchy for the energetic CHSH parameters:
\begin{equation}
\mathcal{S}_W^{(c)} < \mathcal{S}_W^{(q)} < \mathcal{S}_W^{(s)}.
\end{equation}
This result demonstrates that the operational advantage of robustness is captured quantitatively by the energetic CHSH parameter. The hierarchy of classical, quantum, and stronger-than-quantum is mirrored perfectly by a hierarchy of the collective thermodynamic potential accessible in a Bell-test scenario. Stronger non-locality translates directly into a more valuable and resilient thermodynamic resource under operational constraints.
