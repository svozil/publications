...
occurrence vectors, quantum coherence, and one lexical projector shared by
several sense-labelled quantum contexts.  For ordinary transformers the
quantum comparisons are structural and deliberately limited.  A final appendix, \emph{A minimal
...
one lexical direction shared by several sense-labelled orthonormal bases.
...
comparison explicitly.  Appendix~\ref{app:quantum-attention} instead defines
a separate quantum model and a phase-sensitive ``bank'' calculation.

\subsection{A projector shared by quantum contexts}

A different quantum-logical analogy uses incidence rather than addition.  A
...
to many bases, the proposal has empirical content only if the shared lexical
component is approximately stable across held-out contexts or improves
held-out sense prediction.
% ======================================================================
\section{Conclusion}
...
optional shared-context model places one lexical projector in a
...
nonzero $a_t$ for every occurrence of type $t$; an empirical version would
test approximate stability.  This shared component is the substantive
hypothesis, while the remaining coordinates may change with the occurrence.
...
\end{equation}
For this calculation choose two vocabulary basis vectors as the incompatible
