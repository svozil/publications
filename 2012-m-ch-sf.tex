...
We also mention the formul\ae
\begin{equation}
\begin{split}
\Gamma ( \frac{1}{2})=\sqrt{\pi } ,\\
\textrm{ or, more generally, }\Gamma ( \frac{n}{2})=\sqrt{\pi }\frac{( n-2)!!}{2^{(n-1)/2}} \textrm{, for }n>0 \textrm {,  and }\\
...
Althogh we have considered an arbitrary finite number of poles,
...
\sum_{l=0}^\infty ({l + \sigma})({l + \sigma-1}) w_l (x-x_0)^{l + \sigma -2}\\
\qquad + \left[  \sum_{l=0}^\infty ({l + \sigma}) w_l (x-x_0)^{l + \sigma -1}\right]  \sum_{j=0}^\infty \alpha_j (x-x_0)^{j-1}
\\
...
({l + \sigma})({l + \sigma-1})   w_l (x-x_0)^{l}\right. \\
...
$$
l+j =m, \; j= m-l
\textrm{, since } l\ge 0 \textrm{ and } j\ge 0 \textrm{; thus } 0\le l\le m
$$
...
({m + \sigma})({m + \sigma-1})   w_m (x-x_0)^{m} \right. \\
...
+   p_2 (x)  y_1(x)\right\}\int_x  v(s) ds      \\ \qquad
+y_1(x)  \left[ \frac{d  }{dx  } v(x) \right]
...
\end{equation}
where the quotients $\frac{c_{j+1}}{c_j}$ are {\em rational functions} of $j$, so that they can be factorized by
\begin{equation}
\begin{split}
\frac{c_{j+1}}{c_j}
=
...
(It will later be argued that $\varepsilon$ will be dicrete;  with $n\in {\Bbb N}-0$.)
\begin{equation}
\begin{split}
...
\end{equation}
Because of convergence of $\hat{R}$ and thus of $R$, the series solution
