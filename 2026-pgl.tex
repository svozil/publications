...
\label{eq:colormap}
\end{align}
The separator \(br\) is rendered as a black square. In the row-wise version below, blank white cells are used only for alignment; they are not part of the logic.
...
\listingtitle{lst:trianglecode}{Structural layer: the triangle logic stored as symbolic rows. The same rendering predicates can be reused with a four-state palette.}
\begin{codeblock}
% order of rows in the final artifact
triangle_rows([a,b,c,d,e,f]).

% structural layer: symbolic rows
triangle_row(a, [s1,sep,s2,s3,s4]).
triangle_row(b, [s2,s3,sep,s1,s4]).
triangle_row(c, [s4,sep,s1,s2,s3]).
triangle_row(d, [s1,s2,sep,s3,s4]).
triangle_row(e, [s3,sep,s1,s2,s4]).
triangle_row(f, [s2,s4,sep,s1,s3]).

triangle_output :-
    triangle_rows(Rows),
    forall(member(R, Rows), (
        triangle_row(R, Symbols),
        forall(member(S, Symbols), render_symbol(S)),
        nl
    )).
\end{codeblock}
...
Generatively, these probabilities can be used as a second layer placed on top of the logical skeleton. For example,
\begin{align}
