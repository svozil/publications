...
Figure~\ref{fig:vlogic} presents three views of the same source structure: panel~(a) shows the underlying V-logic as a Greechie-style hypergraph, panel~(b) shows the corresponding atom-state incidence schema, and panel~(c) shows the grammar-generated output we call the \emph{Quantum Square}.
...
(a) & (b) & (c)
\end{tabular}
\caption{Three representations of the V-logic \(L_{12}\). (a) Greechie-style hypergraph. (b) Atom-state incidence schema, with colored cells indicating \(v_i(x)=1\) and gray cells indicating \(v_i(x)=0\). (c) The grammar-generated output, the \emph{Quantum Square}. In each row, the states assigning value \(1\) to the atom are listed first, then a black separator, and then the states assigning value \(0\).}
\label{fig:vlogic}
\end{figure*}
