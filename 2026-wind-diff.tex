...
\usepackage{tikz}
\usetikzlibrary{calc,arrows.meta,decorations.pathmorphing,decorations.pathmorphing,positioning}
...
Throughout, ``faster-than-the-wind'' refers to motion parallel (upwind or downwind) to the true wind in the ground/water \textcolor{blue}{(laboratory)} frame \textcolor{blue}{\(G\)}, unless stated otherwise.
...
In the usual downwind-faster-than-the-wind configuration, \textcolor{blue}{let the cart speed in
...
\(C\).} Once rolling, the wheels drive the propeller, which acts as a fan \textcolor{blue}{and
accelerates air rearward.} \textcolor{blue}{At the crossover \(v=v_w\), the apparent-wind
...
continues to \textcolor{blue}{operate} as a
wheel-driven fan\textcolor{blue}{ and accelerates this air rearward,
...
\textcolor{green}{As emphasized above, propulsion requires the external reaction supplied by the second medium.}
...
\textcolor{green}{Because a free-flying aircraft lacks a second medium that can provide an
...
Throughout this section we \textcolor{blue}{return to the ground/water frame \(G\), which for a
...
wake formation\textcolor{blue}{, that is, the
...
speed through the water is $v_b$, and the \textcolor{blue}{apparent-wind velocity} is
\(
v_a = v_w - v_b
\).
{\color{blue}Figure~\ref{fig:water-frame} summarizes the kinematic conventions used in this
...
The key point is a simple force--speed tradeoff. {\color{blue}In a one-dimensional
...
This \textcolor{blue}{force--energy} intuition must be read
with an important caveat: the achievable forces depend on how much fluid each
device can interact with per unit time, \textcolor{blue}{that is, on the mass flow rate
...
\textcolor{blue}{transfer} of kinetic energy to the wake represents an unavoidable \textcolor{blue}{energy} loss.
...
\textcolor{green}{To deepen this understanding, we employ a series of mechanical analogies that
...
{\color{blue}To keep the frame changes explicit, we use the vehicle frame only in the next
...
Thus,} for the \textcolor{blue}{immediate} kinematic relations below, we work in the instantaneous
rest frame of the gearbox (``vehicle''), so that all gear centers are at rest.
In this frame the upper rack (``wind'') moves with speed $v_i$ relative to the
gearbox, and the lower rack (``ground'') moves backward with speed $v_f$; \textcolor{blue}{the
...
\end{tabular}
\caption{Schematic of the gearbox analogy in the laboratory frame \textcolor{blue}{\(G\)}. \textcolor{blue}{The
...
defined in the text.} (a) Positive transmission (downwind): \textcolor{blue}{the} motion of the
upper rack (wind)
is transmitted through the gear \textcolor{blue}{train} to \textcolor{blue}{the lower contact} in
the same \textcolor{blue}{sense, yielding} velocity \textcolor{blue}{amplification}.
(b) Negative transmission (upwind): \textcolor{blue}{an} idler gear reverses the \textcolor{blue}{transmitted sense} of motion between the
\textcolor{blue}{upper} and \textcolor{blue}{lower contacts} while preserving the same \textcolor{blue}{no-slip, ground-constrained} coupling at the lower contact.}
\label{fig:gearboxes}
\end{figure*}
...
{\color{blue}We now switch back to the ground frame \(G\), because the no-slip condition at
...
between the ``wind'' and the vehicle over the ground, are \textcolor{blue}{therefore} obtained by
writing the no-slip conditions in \textcolor{blue}{\(G\)}.
...
\caption{\textcolor{blue}{Dimensionless plots of two key relations used in the text.
...
\textcolor{blue}{The dimensionless behavior of these relations is shown in Fig.~\ref{fig:dimensionless_plots}(a): as the idealized speed amplification grows, the transmitted force decreases proportionally.}
...
\textcolor{green}{As emphasized above, this mechanism works only because the chassis is externally
...
In the instantaneous \textcolor{blue}{vehicle} frame \textcolor{blue}{$C$}, the presence of the idler adds one more gear mesh between the upper rack and the main gear
...
reversed. In the \textcolor{blue}{ground frame $G$} we therefore have
\begin{equation}
...
\textcolor{green}{In the physical vehicle, the wind (upper rack) supplies input power to the
...
through \textcolor{blue}{lever geometry and a} fixed \textcolor{blue}{external} support. Here geometry replaces
...
However, it must be noted that a physical lever allows only transient faster-than-the-wind motion, limited by the length of its stroke. The wind-driven vehicle, however, operates as a \emph{continuous} rotary lever, maintaining this \textcolor{blue}{kinematic transformation} indefinitely as it rolls.
...
\textcolor{blue}{In the ideal lossless limit, the lever does not save energy or power; rather,
...
F_d = F_w \frac{d_s}{L},
\end{equation}
where $F_w$ is the wind force on the sail and $F_d$ is the force at the lever tip. The pivot acts as an external constraint (a fixed support) that supplies the required reaction forces and torque, enabling momentum \textcolor{blue}{exchange} between the wind-driven part of the lever and the ground.
...
\textcolor{green}{The lever analogy restates these same principles geometrically: the lever-arm
...
\textcolor{green}{The preceding analogies show that sustained propulsion requires an external
...
Imagine \textcolor{blue}{Alice} standing on a dock while Bob sits in a small boat that is free to
drift horizontally with negligible resistance. Bob attempts to reach \textcolor{blue}{Alice} by
...
\end{equation}
The negative sign indicates the boat moves backward. Bob's resulting progress toward \textcolor{blue}{Alice}, measured relative to the water, is therefore
\begin{equation}
...
\textcolor{green}{Thus, without an external horizontal impulse, internal motions merely
...
\textcolor{blue}{(a) Initial configuration.
...
Bob may be able to reduce the separation to \textcolor{blue}{Alice} by exploiting effects that are
...
\item Introductory undergraduate physics/engineering: \textcolor{blue}{Rates of mechanical energy transfer (power)}, efficiency, drag scaling, turbines vs fans, idealizations vs nonidealities.
...
\item Explain how static friction can transmit force without doing work in the ground frame; identify where \textcolor{blue}{mechanical energy} enters and leaves the system \textcolor{blue}{and the corresponding rates of transfer (power)}.
\item Describe the vehicle as a mechanical transformer between two media; draw qualitative \textcolor{blue}{rate-of-energy-transfer (power-flow)} diagrams for downwind and upwind operation.
...
\item Frame confusion: mixing air-frame and ground-frame velocities \textcolor{blue}{and rates of energy transfer (powers)}.
...
\item Short quiz: Multiple-choice on frames and signs of power; one free-response drawing a \textcolor{blue}{rate-of-energy-transfer (power-flow)} diagram.
...
The manuscript employs a constraint-based Newtonian framework to deliver a succinct, physically coherent, and pedagogically valuable analysis of superluminal motion. The frame analysis is quite detailed, the derivations are very strict, and the explanation of misunderstandings is very clear and helpful.

Before the manuscript is published, I suggest the following changes to make it even better:

• Minimize Repetition: The "external reaction surface" and the traction constraint are talked about in different parts of the text. This topic is important, however a little bit of consolidation would make the flow better and cut down on repetition without making things less clear.
• Enhance the Fluid-Dynamics Section: While the discourse on waterborne implementations is conceptually robust, it remains predominantly qualitative. For readers at a higher level, it might be better to give a short quantitative example, like quoting actuator-disk reasoning or fundamental efficiency scaling.
• Figures and Presentation: In the final version, make sure that every schematic figure has a clear caption and that the sign conventions and frame definitions are the same for all of them.


Overall, the text is a useful instructional tool that does a decent job of clarifying a sometimes misunderstood subject. I will recommend it with the slight modifications mentioned above.


point-by point response:

I thank the Referee for this very positive assessment of the manuscript and for
the helpful suggestions for further improvement. I am pleased that the
constraint-based Newtonian treatment, the frame analysis, and the pedagogical
explanations were found to be clear and useful.

I have addressed the three points raised in the report as follows:

* Minimize repetition concerning the external reaction surface and traction constraint.

I agree that this theme, while central to the paper, was repeated somewhat too
often in similar language. In the revised manuscript I therefore added a short
consolidating paragraph early in the paper that states this principle once in a
more global form: sustained faster-than-the-wind motion requires a coupling
between two media in relative motion together with an external reaction
constraint. I also shortened several later reiterations so that subsequent
sections can refer back to the main idea without restating the full argument
each time. The respective sections are marked green.

* Enhance the fluid-dynamics section with a short quantitative example.

I agree that a compact quantitative complement improves the waterborne
discussion. In the revised manuscript I therefore added a short actuator-disk
scaling argument in the section on mechanically coupled air and water
propellers/turbines. This gives an explicit relation between thrust, power,
upstream speed, and slipstream increment, and makes clear why, for a given
shaft power, a slower stream can in principle yield a larger thrust, while also
showing how wake kinetic-energy loss and efficiency depend on loading and
effective area.

* Ensure that all schematic figures have clear captions and consistent sign conventions and frame definitions.

I agree. I performed a full consistency check of all schematic figures and their
captions in the revised manuscript. In particular, I standardized the frame
definitions and velocity labels across the vehicle, gearbox, water-frame, lever,
and Popescu-glide figures. I also revised several captions so that the frame
used, the meaning of the velocity arrows, and the relevant sign conventions are
stated explicitly and consistently.


I again thank the Referee for the careful and constructive report. These
suggestions have helped improve both the flow and the pedagogical value of the
manuscript.

Reviewer 2 Comments:

This is an interesting example of modeling physical systems. I have several general comments as
well as more detailed suggestions that may help improve clarity and presentation.
General Suggestions
• Improve the diagrams using appropriate drawing software to enhance clarity and
readability.
• If available, include a photograph of a prototype of the wind-powered vehicle.
• You provided suggestions for classroom activities. Could students construct a physical
model that would allow them to test the concept experimentally?
• Several frames of reference are considered when quantifying the motion. This may be
confusing for the reader; consider simplifying this section or clarifying the transitions
between frames of reference.
• On page 8, line 46, you state that mechanical advantage saves power. Mechanical
advantage does not actually save energy or power. Instead, it reduces the force required by
increasing the distance over which the force is applied, as in the case of a lever arm.
• Consider incorporating Bernoulli’s principle into the explanation if it is relevant to the
airflow and energy transfer processes discussed.
• Could you illustrate the behavior of some of the derived formulas graphically by plotting
them as functions in the appropriate coordinate systems
More Detailed Suggestions
• In Section IV, include a diagram that illustrates the laboratory frame of reference you
introduce.
• On page 8, line 47, you use the term “apparent wind.” Do you mean the velocity of the
apparent wind? Please clarify.
• On page 8, line 45, explain what you mean by “wake formation.”
• On page 4, line 2, you state that “these devices exchange both change of momentum
(experienced as a force) and energy (experienced as power).” Consider clarifying this
statement. The exchange of momentum is associated with the impulse of a force, while the
rate of change of energy is perceived as power. Including equations and diagrams would
help support and clarify this explanation.
• Replace the phrase “kinetic energy grows” with “kinetic energy increases.”
• On page 4, line 13, replace “power loss” with “energy loss.”
• Reposition Figure 2 so that it appears closer to the relevant discussion in the text.
• If possible, support Figure 2 with a photograph or illustration of a physical model.
• You refer to the “instantaneous rest frame of the gearbox.” Please clarify why the term
instantaneous is used.
• The font size in Figure 2 is not consistent and should be standardized.
• Improve Figure 3 and clearly label the respective velocities discussed in that section.
• Is there a simulation of the phenomenon you describe? Including one could greatly aid
reader understanding.
• On page 10, under “Power Flow,” did you mean “rate of energy flow”? Please clarify the
terminology.
• On page 10, line 29, you write “where power enters and leaves the system.” Do you mean
when the fluid enters and leaves the system?
• On page 11, regarding the lever analogy, consider including an image or photograph of a
prototype that illustrates this concept.


point-by point response:

I thank the Referee sincerely for the careful reading of the manuscript and for the many constructive suggestions. The report has been very helpful in improving the clarity, precision, and pedagogical usefulness of the paper.

In the revised manuscript, all corresponding changes have been marked in blue. Because several new figures were added and all schematic drawings were redrawn, some figure numbers have changed in the revised version. In the responses below, I therefore refer mainly to the relevant sections and figure content rather than to the original numbering.

General Suggestions

1. Improve the diagrams using appropriate drawing software to enhance clarity and readability.

Response:
I fully agree. All former sketch/raster figures have now been replaced by scalable vector graphics produced directly in LaTeX using `TikZ` and `pgfplots`. This includes the vehicle-operation diagrams, the gearbox analogy, the lever analogy, the water-frame schematic, the formula plots, and the Popescu-glide schematic. I believe this has significantly improved clarity, consistency, and readability.


2. If available, include a photograph of a prototype of the wind-powered vehicle.

Response:
Unfortunately, I do not have an original physical prototype available for photography. Instead, I have strengthened the manuscript’s references to existing and historical physical realizations, including the Blackbird land yacht and the Kauffman--Lindahl toy boat, and I cite accessible experimental demonstrations in the literature and in publicly available videos. Since the paper is primarily conceptual and pedagogical, I judged that idealized vector schematics are better suited to isolating the mechanical principles than photographs of complex real assemblies.


3. The manuscript provided suggestions for classroom activities. Could students construct a physical model that would allow them to test the concept experimentally?

Response:
Yes, in principle they could, and I agree that such a construction project could be very instructive. However, a functioning faster-than-the-wind cart typically requires careful adjustment of drivetrain losses, rolling resistance, and propeller pitch, which goes well beyond a short classroom activity. For this reason, in the revised pedagogical section I retained the emphasis on “quick demos” that reliably illustrate the underlying principles with modest equipment. At the same time, I now explicitly reference do-it-yourself classroom demonstrations suitable for more extended hands-on work (in particular the Foxlin demonstration cited in the gearbox/upwind discussion). I leave full cart construction as an optional instructor- or student-driven project, depending on available time and resources.


4. Several frames of reference are considered when quantifying the motion. This may be confusing for the reader; consider simplifying this section or clarifying the transitions between frames of reference.

Response:
I thank the Referee for emphasizing this point. I have substantially revised the manuscript to make the frame usage explicit and more systematic. In particular:

- I added a dedicated paragraph titled “Reference frames and sign conventions” near the start of Section II;
- I now define only two main frames throughout the paper:
  - the ground/water laboratory frame \(G\),
  - the instantaneous vehicle frame \(C\);
- I added explicit transition phrases when switching frames, especially in the gearbox analogy;
- I clarified in the figure captions and surrounding text which elements are being described in which frame.

This revision was made specifically in response to the Referee’s concern about possible reader confusion.


5. On page 8, line 46, the manuscript states that mechanical advantage saves power. Mechanical advantage does not actually save energy or power. Instead, it reduces the force required by increasing the distance over which the force is applied, as in the case of a lever arm.

Response:
I agree and thank the Referee for catching this imprecise wording. I have corrected the relevant passage in the lever section. The text now states explicitly that, in the ideal lossless limit, the lever does not save energy or power, but rather trades force for distance (or force for speed). This directly addresses the distinction noted in the report.



6. Consider incorporating Bernoulli’s principle into the explanation if it is relevant to the airflow and energy transfer processes discussed.

Response:
I appreciated this suggestion and incorporated it in a limited but, I think, useful way. I added a short paragraph titled “Relation to Bernoulli’s principle” in Section II. There I explain that Bernoulli’s principle provides a complementary pressure-based fluid description, while the main explanatory framework of the paper remains Newtonian: momentum exchange between two media mediated by the drivetrain and the traction constraint. I also clarify that Bernoulli’s equation applies upstream and downstream of an ideal actuator disk, whereas across the disk itself there is a pressure jump and energy transfer.


7. Could the behavior of some of the derived formulas be illustrated graphically by plotting them as functions in the appropriate coordinate systems?

Response:
Yes. I have added a new figure with two dimensionless plots:
- one plot showing the gearbox speed amplification and corresponding force reduction as functions of the dimensionless gear ratio \(x=r_2/r_3\);
- one plot showing the apparent-wind velocity in the vehicle frame as a function of the vehicle speed ratio \(v/v_w\).

These plots are intended to make the force–speed tradeoff and the tailwind-to-headwind crossover more immediately visible.


More Detailed Suggestions

8. In Section IV, include a diagram that illustrates the laboratory frame of reference introduced there.

Response:
Done. I added a dedicated schematic in the water section showing the velocity conventions in the laboratory frame \(G\) and the instantaneous boat frame \(C\), including the true wind speed \(v_w\), boat speed \(v_b\), and apparent-wind velocity \(v_a=v_w-v_b\).


9. On page 8, line 47, the term “apparent wind” is used. Does this mean the velocity of the apparent wind? Please clarify.

Response:
Yes. I now use the more precise phrase “apparent-wind velocity” where appropriate, especially in the frame-defining passages and in the water section. I retained the shorter phrase “apparent wind” only where it is standard and harmless in context, but clarified the actual quantity being used.


10. On page 8, line 45, explain what is meant by “wake formation.”

Response:
Done. In the water section I now explicitly explain wake formation as the creation of a trailing region of disturbed flow behind the hull or propulsor.


11. On page 4, line 2, the manuscript states that “these devices exchange both change of momentum (experienced as a force) and energy (experienced as power).” Consider clarifying this statement. The exchange of momentum is associated with the impulse of a force, while the rate of change of energy is perceived as power. Including equations and diagrams would help support and clarify this explanation.

Response:
I have revised this discussion substantially. In the section on mechanically coupled air and water propellers/turbines, I now introduce a simple one-dimensional control-volume idealization with the explicit relations
\[
F=\dot m\,(u_{\rm out}-u_{\rm in}), \qquad
P=\frac12 \dot m\,(u_{\rm out}^2-u_{\rm in}^2),
\]
and explain clearly that:
- the force corresponds to the rate of momentum transfer,
- the power corresponds to the rate of energy transfer.

This is now stated more carefully and in a more standard form.


12. Replace the phrase “kinetic energy grows” with “kinetic energy increases.”

Response:
Done.


13. On page 4, line 13, replace “power loss” with “energy loss.”

Response:
Done. In the “penalty of slip” discussion I now refer to energy loss rather than power loss where that distinction is more appropriate.


14. Reposition Figure 2 so that it appears closer to the relevant discussion in the text.

Response:
I have moved the figures in the LaTeX source so that they are now placed as close as reasonably possible to the corresponding discussion. Because the manuscript uses the APS `revtex4-2` `reprint` style, float placement is still partly constrained by the journal class, but the revised ordering and placement are significantly improved.


15. If possible, support Figure 2 with a photograph or illustration of a physical model.

Response:
I do not have an original physical prototype available for photography. Instead, the former hand-drawn/sketch-style figures have been replaced by much clearer vector illustrations. I believe that, for the mechanical analogies, these idealized schematics are pedagogically more effective than photographs, since they isolate the kinematics and sign conventions more clearly. I also continue to refer readers to external demonstrations of physical models in the cited literature and videos.


16. The manuscript refers to the “instantaneous rest frame of the gearbox.” Please clarify why the term instantaneous is used.

Response:
Done. I now explicitly state that, because the gearbox/vehicle may accelerate in the laboratory frame, “instantaneous rest frame” means the momentarily comoving frame at a given instant, used only for local kinematics and sign conventions.


17. The font size in Figure 2 is not consistent and should be standardized.

Response:
Addressed. Since all figures have now been redrawn using LaTeX-native vector graphics, the figure typography is fully standardized and consistent with the manuscript fonts.


18. Improve Figure 3 and clearly label the respective velocities discussed in that section.

Response:
Addressed. The gearbox figures were completely redrawn in `TikZ`, with clearly labeled velocities and a rewritten caption clarifying the role of the laboratory frame and the local tangential speeds. I also added a pair of dimensionless plots that graphically summarize the corresponding formulae.


19. Is there a simulation of the phenomenon described? Including one could greatly aid reader understanding.

Response:
A full numerical simulation was not added, because the present paper is intended as a compact conceptual and pedagogical treatment rather than a computational or engineering study. However, I have strengthened the manuscript in two related ways:
1. by adding the dimensionless formula plots discussed above, and
2. by pointing more explicitly to the existing simulation and modeling literature, particularly the aerodynamic analyses already cited.

I hope this provides readers with both a clearer conceptual picture and pointers to more detailed numerical treatments.


20. On page 10, under “Power Flow,” did the manuscript mean “rate of energy flow”? Please clarify the terminology.

Response:
Yes. I have clarified the terminology in the pedagogical section. Where appropriate, I now refer explicitly to the rate of mechanical energy transfer (power) and to power-flow diagrams as diagrams of these rates of transfer.


21. On page 10, line 29, the manuscript states “where power enters and leaves the system.” Does this mean when the fluid enters and leaves the system?

Response:
I meant the locations where mechanical energy enters and leaves the system, not the physical entry and exit of fluid parcels. To avoid ambiguity, I have revised the wording to:
 “identify where mechanical energy enters and leaves the system and the corresponding rates of transfer (power).”


22. On page 11, regarding the lever analogy, consider including an image or photograph of a prototype that illustrates this concept.

Response:
As with the gearbox analogy, I do not have a physical prototype photograph available. Instead, I have replaced the former sketch by a much clearer vector schematic and preserved the explicit bench-top classroom demonstration in the pedagogical section (meter stick with fixed pivot). Since the lever analogy is especially geometric, I believe the idealized diagram is well suited to the explanatory purpose.


I again thank the Referee for the careful and constructive report. The suggestions have led to substantial improvements in the presentation, the terminology, the figure quality, and the treatment of frames and power/energy language throughout the paper.
