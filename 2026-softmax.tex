...
maximum-entropy characterization of softmax~\cite{Jaynes1957}. If a context \(C\) is
fixed and one maximizes Shannon entropy over probability assignments
\((p_a)_{a\in C}\), subject to normalization and a linear constraint
on the expected score,
\[
\sum_{a\in C}p_a=1,
\qquad
...
Generalized-softmax path between midpoint states and half-weights:
...
\]
This is a valid two-valued weight. On an odd cycle, however, it does
not lift to an admissible three-coloring in which the midpoint atoms
are assigned one fixed color: the two remaining colors would have to
alternate around an odd cycle, which is impossible. For \(r(t)=1\),
all three atoms in each context have equal positive coordinate and
\[
...
For the ordinary exponential link \(g(u)=e^{\beta u}\), a convenient
one-parameter family is obtained by choosing the global scores
\[
u(a_i)=0,\qquad u(x_i)=t
\quad (i=1,\ldots,n),
\]
with \(\beta>0\). In each context
\(C_i=\{a_i,a_{i+1},x_i\}\), the normalizing factor is
\[
Z_i=e^{\beta u(a_i)}+e^{\beta u(a_{i+1})}+e^{\beta u(x_i)}
=2+e^{\beta t},
\]
so that
\[
...
\qquad
p_t(x_i)=
\frac{e^{\beta t}}{2+e^{\beta t}}.
\]
This family is single-valued on the shared atoms \(a_i\) and
interpolates between three distinguished regimes. For \(t=0\), all
three atoms in each context have equal score and one obtains the
uniform assignment
\[
p_0(a_i)=p_0(a_{i+1})=p_0(x_i)=\frac13.
\]
For \(t\to-\infty\), the context-specific atoms \(x_i\) are
suppressed and one obtains the exotic boundary half-weight
\[
p_t(a_i)\to\frac12,\qquad p_t(x_i)\to0,
\]
which is Eq.~\eqref{eq:odd-cycle-half-weight}. For \(t\to+\infty\),
the midpoint atoms dominate and one obtains the limiting assignment
\[
p_t(a_i)\to0,\qquad p_t(x_i)\to1.
\]

For finite real-valued scores the exponential softmax remains strictly
positive and therefore approximates, but does not exactly attain, the
boundary half-weight. For a general positive link \(g\), the same
limiting construction is obtained by choosing scores such that
\[
g(u(a_i))=1,\qquad g(u(x_i))=\varepsilon,
\]
and then taking \(\varepsilon\to0\), provided \(0\) lies in the
closure of the range of \(g\).
