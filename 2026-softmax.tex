...
usual Lov{\'a}sz/KCBS quantum value~\cite{lovasz-79,Klyachko-2008,
Bub-2009}. Such a weight is obtained as a limiting softmax
assignment. But the beyond-classical or beyond-quantum character
belongs to the weight's location in the polytope, not to the
...
formulation, \(A\) and \(\mathcal M\) are taken as primitive. The
event structure is ``pasted'' in the sense that different contexts
...
With the usual operational convention in which quantum models are
allowed to represent classical deterministic assignments, one has
\[
\mathcal C(L)\subseteq\mathcal Q(L)\subseteq\mathcal W(L).
\]
The inclusion \(\mathcal Q(L)\subseteq\mathcal W(L)\) follows
directly from blockwise completeness of the projectors. The inclusion
\(\mathcal C(L)\subseteq\mathcal Q(L)\) is convention-dependent if
one fixes a particular Hilbert-space representation in advance: not
every abstract two-valued state need be represented inside that fixed
representation. Throughout, claims of being beyond quantum are
therefore relative to a specified quantum representation or to a
specified graph-theoretic quantum set, such as the Lov{\'a}sz-theta set
for exclusivity graphs.
...
\]
If \(q\) is measurable, or monotone, or locally bounded on a set of
...
h(u+v)=h(u)+h(v).
\]
Under any of the stated regularity assumptions, the regular solutions
of Cauchy's equation are linear~\cite{Aczel1966}:
\[
h(u)=\beta u
\]
...
\(t\to-\infty\), respectively. Equivalently, setting \(t=-M\) and
sending \(M\to\infty\) gives the limiting construction
\[
p_t(a_i)\to\frac12,\qquad p_t(x_i)\to0.
\]
...
logic in the three distinguished regimes of the softmax path:
the midpoint limit (\(t\to+\infty\)), the uniform state (\(t=0\)),
and the half-weight (\(t\to-\infty\)).}
...
this condition is \(e^{\beta t}\to0\), i.e. \(t\to-\infty\).
...
This distinction is close to the older Bell--Peres intuition about
contextuality. If one insists on assigning outcomes to unperformed
experiments, Bell-type violations force the value assigned to an
...
\(r_{j\alpha}\) by a context-dependent result
\(r_{j\alpha}(\beta)\), or more generally
\(r_{j\alpha}(\beta,\gamma,\ldots)\), once Bell's inequality is
violated~\cite{peres222}. Similarly, in an explicitly contextual
Clauser--Horne--Shimony--Holt (CHSH) inequality notation one writes
\(E(x,y)\) as \(E(x_y,y_x)\), with contextuality manifesting itself
through \(x_y\ne x_{y'}\)~\cite{svozil-2011-enough}.

In the present language, context-wise softmax without gluing is a
probabilistic instance of explicit context-dependence: the same
nominal atom is represented by different context-indexed quantities,
just as \(x_y\) and \(x_{y'}\) are kept distinct in an explicitly
contextual CHSH notation. A single-valued admissible weight imposes
the opposite requirement, namely the identification of shared atoms
across contexts. If such a single-valued assignment nevertheless
fails to lie in the classical polytope, the remaining obstruction is
the usual Bell-, KCBS-, or Kochen--Specker-type contextuality.

This also clarifies the relation to the Contextuality-by-Default
(CbD) treatment of cyclic systems~\cite{Dzhafarov-19PhysRevA}. In CbD,
random variables measuring the same content in different contexts are
not identified at the outset; they belong to different bunches and
are connected only through couplings. Consistent connectedness, or
no-disturbance, is then the special case in which same-content
variables have identical distributions. This is structurally parallel
to the distinction made here between free context-wise generalized
softmax and single-valued generalized softmax. A free context-wise
softmax assigns probabilities separately in each context, just as CbD
treats context-indexed measurements as distinct random variables.
Imposing single-valuedness on shared atoms is the analogue of passing
to a coupling in which same-content variables are identified as far as
the marginal distributions permit.

The two approaches therefore address complementary questions. CbD
asks whether a family of context-indexed random variables admits an
overall coupling satisfying the required maximal-connection
constraints, and it measures contextuality by distances to the
corresponding noncontextuality polytope. The present paper asks what
role, if any, a softmax or generalized-softmax normalization plays
once such cross-context compatibility is imposed. The answer is
deflationary: after single-valuedness/no-disturbance has been imposed,
the generalized softmax link is only a coordinate parametrization of
the resulting admissible weight. Thus CbD provides a natural language
for the disturbed or inconsistently connected case, whereas the
present theorem concerns the strictly glued, admissible-weight case.

A natural next step is therefore not to ask whether generalized
