...
normalization does not by itself yield a global probability weight:
an event shared by two contexts may be assigned two different
probabilities. We show that imposing single-valuedness on shared
atoms---equivalently, no-disturbance or consistent connectedness---
reduces generalized softmax rules to coordinate parametrizations of
the strictly positive part of the admissible-weight polytope. Every
strictly positive admissible weight is so representable, with
context-independent scores, and boundary weights arise as limits.
Exotic weights that exceed classical or quantum bounds are therefore
properties of the event structure and the chosen weight, not of the
normalizing link. The resulting hierarchy separates four issues that
are easily conflated: local normalization, cross-context gluing,
...
structure there is an additional compatibility problem. Several
overlapping contexts may be present, and the same event may occur in
more than one of them. Normalizing separately inside each context
does not guarantee that shared events receive the same probability in
all contexts in which they occur.
...
A probability weight on such a structure is not merely a family of
normalized distributions, one for each context. It is a single
function on atoms that normalizes on every context. If an atom is
shared by two contexts, it has one value, not two. This cross-context
agreement will be called \emph{single-valuedness on atoms}. It is the
finite-test-space form of \emph{no-disturbance}; in Bell-type
scenarios the analogous condition is no-signaling, and in the
contextuality-by-default literature it is closely related to
...
softmax is obtained by \(g(x)=\exp(\beta x)\); other choices of \(g\)
give logistic, probit, polynomial, power-law, or \(q\)-exponential
response models on suitable domains.
...
separately normalized response distributions, which may violate
no-disturbance: the same shared atom may receive different
probabilities in different contexts. If single-valuedness is imposed,
the construction becomes exactly a parametrization of an admissible
weight on the logic. Conversely, every strictly positive admissible
weight can be written in generalized softmax form---indeed with
context-independent scores---provided the link function has a
suitable positive range. The softmax wrapper therefore adds no new
probability theory once global consistency is required; its
explanatory force begins only when the score functions are themselves
constrained by additional modeling assumptions.
...
is obtained as a limiting softmax assignment. But its
beyond-classical or beyond-quantum character belongs to the weight's
location in the polytope, not to the exponential function.
...
Born-rule representations; Wright and Weigert have made the
connection between such Gleason-type arguments and Cauchy's
functional equation explicit~\cite{Wright2019}. The present result
does not derive Born-linearity. It concerns the prior question of
when locally normalized softmax responses glue into a single
admissible weight. Cauchy's equation can nevertheless enter again at
the level of the link function: if additive score contributions are
required to induce multiplicative unnormalized weights, the
exponential link is singled out under the usual regularity
assumptions.
...
rules and the distinction between local normalization and global
gluing. Section~\ref{sec:representation-theorem} proves the
representation theorem and records the gauge freedom and boundary
cases. Section~\ref{sec:cauchy} clarifies the relation to Cauchy's
...
empirical and modeling implications, Section~\ref{sec:discussion}
collects broader consequences, and Section~\ref{sec:conclusion}
concludes.
...
We reserve the word ``gluing'' for the probabilistic compatibility
condition considered below: locally normalized context-wise
distributions glue if they assign the same probability to every atom
shared by two or more contexts, thereby defining a single admissible
weight on the already pasted event structure.
...
atom; single-valuedness on shared atoms is thus built into the
notion of a weight.
...
1,
\]
and because the same projector \(\Pi_a\) is assigned to the same atom
\(a\), the probability \(p(a)\) is single-valued across contexts.
Thus every such Born-rule model lies in \(\mathcal W(L)\), and
\[
\mathcal Q(L)\subseteq\mathcal W(L)
\]
follows directly from blockwise completeness of the projectors. If,
in addition, one adopts the usual operational convention that
classical deterministic assignments are included among the allowed
quantum models, one obtains the familiar chain
\[
...
represented by scores \(u_C(a)\). The substantive question is
...
\(\sum_{a\in C}P_{g,C}(a)=1\), and respects additivity for unions of
atoms inside that context. Generalized softmax thus solves the
\emph{local} normalization problem.
...
the latter membership in \(\mathcal C(L)\).
...
\label{eq:ratio}
\end{equation}
In particular, if two contexts share more than one atom, all shared
atoms must induce the same normalizer ratio; and if contexts
\(C_1,\ldots,C_k,C_{k+1}=C_1\) form a cycle in which each consecutive
pair shares an atom \(a_i\in C_i\cap C_{i+1}\), then
\[
\prod_{i=1}^{k}\frac{q_{C_i}(a_i)}{q_{C_{i+1}}(a_i)}=1 .
\]
\end{proposition}

\begin{proof}
The first statement is the definition of single-valuedness of the
normalized probabilities, and rearranging Eq.~\eqref{eq:gluing}
gives Eq.~\eqref{eq:ratio}. If two contexts share several atoms, the
same ratio \(Z_C/Z_{C'}\) must be obtained from each of them. Around
a cycle, the product of the ratios \(Z_{C_i}/Z_{C_{i+1}}\)
telescopes to \(1\), which by Eq.~\eqref{eq:ratio} is the displayed
condition.
\end{proof}

Proposition~\ref{prop:gluing} makes explicit why context-independent
\emph{coordinates} are not sufficient: even if
\(q_C(a)=q_{C'}(a)\) for a shared atom, the normalizers \(Z_C\) and
\(Z_{C'}\) may differ, and then the shared atom receives different
probabilities. The representation theorem below shows what additional
structure resolves this: if the coordinates are proportional to a
\emph{globally} admissible weight, all normalizers coincide.
...
\begin{proof}
For each atom \(a\in A\), choose any context \(C\in\mathcal M\) with
\(a\in C\) and set \(p(a)=P_{g,C}(a)\).
Condition~\eqref{eq:single-valued-softmax} makes this definition
independent of the chosen context, and for every \(C\in\mathcal M\),
\(\sum_{a\in C}p(a)=\sum_{a\in C}P_{g,C}(a)=1\) by normalization of
Eq.~\eqref{eq:generalized-softmax}. Hence \(p\in\mathcal W(L)\).
\end{proof}

The converse holds under mild assumptions on the link function, and
in a stronger form than the definition requires: the representing
scores can be taken \emph{context-independent}.
...
representation with atom-only scores: there exists
\(u:A\to I\) such that
\begin{equation}
p(a)=
\frac{g(u(a))}
...
\(\alpha\in(0,r)\); then \(\alpha p(a)\in(0,r)\) for all \(a\in A\).
Because \(g\) is strictly monotone, it is injective with a
well-defined inverse on its range. Define
\(u(a)=g^{-1}(\alpha p(a))\), so that \(g(u(a))=\alpha p(a)\). For
any context \(C\),
\[
\sum_{b\in C}g(u(b))
=
...
p(a),
\]
and the resulting probabilities are single-valued on atoms.
\end{proof}

The construction is explicit: \(u(a)=g^{-1}(\alpha p(a))\) for any
sufficiently small \(\alpha>0\). Note that admissibility of \(p\) is
used exactly once, to make all normalizers equal
(\(Z_C=\alpha\) for every \(C\)); this is how the cycle conditions of
Proposition~\ref{prop:gluing} are discharged simultaneously.

The theorem is deliberately deflationary, and the atom-only form
sharpens the point: even the most restrictive natural score
family---one score per atom, no context dependence at all---already
suffices to represent \emph{every} strictly positive admissible
weight. If the scores are unrestricted, generalized softmax has no
structural content beyond positivity, local normalization, and the
imposed gluing condition. Its explanatory force begins only when the
scores are constrained \emph{independently of the target
probabilities}: by low-dimensional parametric forms, smoothness
conditions across contexts, or a mechanistic model of how utilities
or evidential strengths are generated.
...
\label{rem:gauge}
The representation is not unique. Call two contexts adjacent if they
share an atom, and consider the connected components of the resulting
context graph. Multiplying all positive coordinates \(q(a)=g(u(a))\)
of the atoms in one component by a common positive constant scales
every normalizer in that component by the same constant and leaves
all probabilities unchanged. For the exponential link
\(g(u)=e^{\beta u}\) this corresponds to adding a constant to all
scores in the component, \(u(a)\mapsto u(a)+c\). Generalized softmax
is thus a many-to-one parametrization of the strictly positive part
of \(\mathcal W(L)\), not a unique coordinate chart.
\end{remark}
...
\label{prop:boundary}
Let \(g\) satisfy the hypotheses of
Theorem~\ref{thm:representation}. Then every weight in the closure of
the strictly positive part of \(\mathcal W(L)\) lies in the closure
of the generalized-softmax image. In particular, if \(\mathcal W(L)\)
contains at least one strictly positive weight, this closure is all
of \(\mathcal W(L)\).
\end{proposition}

\begin{proof}
If \(p_k\in\mathcal W(L)\) are strictly positive with \(p_k\to p\),
each \(p_k\) has a representation by
Theorem~\ref{thm:representation} (with a common \(\alpha\in(0,r)\)),
and the represented probabilities converge to \(p\). For the second
statement, let \(p_+\in\mathcal W(L)\) be strictly positive and
\(p\in\mathcal W(L)\) arbitrary; then
\((1-\varepsilon)p+\varepsilon p_+\) is strictly positive for every
\(\varepsilon\in(0,1]\) and converges to \(p\) as
\(\varepsilon\to0\). Atoms with \(p(a)=0\) correspond to coordinates
\(g(u_k(a))\to0\); for the exponential link this means
\(u_k(a)\to-\infty\).
\end{proof}
...
In summary: once single-valuedness is imposed, generalized softmax is
not an additional probability theory but an analytic, many-to-one
coordinate system on the positive part of \(\mathcal W(L)\). The
classification of a weight as classical, quantum, or exotic is not
determined by the link function \(g\), but by the position of the
resulting \(p\) within \(\mathcal W(L)\).
...
glue, one obtains an admissible weight \(p\in\mathcal W(L)\): a
single-valued function on atoms normalizing on every context.
Additivity inside a Boolean block is then imposed by defining the
probability of a coarse-grained event as the sum of the probabilities
of its mutually exclusive atoms.
...
Weigert~\cite{Wright2019} explicitly exploit this connection to give
an alternative proof of Busch's Gleason-type theorem for
effects~\cite{Busch-2003,caves-fuchs-2004}.

The softmax result proved here is different. It derives neither
linearity of a probability assignment nor the Born rule. It shows
that, once single-valuedness is imposed, generalized softmax
normalization is merely a parametrization of an admissible weight;
whether that weight is classical, Born-rule realizable, or exotic is
a further geometric question about its location in
\(\mathcal W(L)\).
...
\(q(u+v)=q(u)q(v)\) for all \(u,v\in\mathbb R\). If \(q\) is
measurable, or monotone, or bounded above on a measurable set of
positive measure, then there exists \(\beta\in\mathbb R\) such that
\(q(u)=e^{\beta u}\).
\end{proposition}

\begin{proof}
Define \(h(u)=\log q(u)\), well defined since \(q(u)>0\). Equation
\eqref{eq:multiplicative-link} gives \(h(u+v)=h(u)+h(v)\), which is
Cauchy's equation. Under any of the stated regularity assumptions,
\(h\) is a regular solution: measurability and monotonicity are
inherited from \(q\), while boundedness above of \(q\) on a
measurable set of positive measure implies boundedness above of
\(h\) there. The standard regularity theorem for Cauchy's
equation~\cite{Aczel1966} gives \(h(u)=\beta u\), hence
\(q(u)=e^{\beta u}\).
\end{proof}
...
\]
the resulting distribution has the ordinary exponential form
\[
p_a=
\frac{e^{\beta u(a)}}{\sum_{b\in C}e^{\beta u(b)}} .
\]
From this perspective, the exponential softmax is the canonical
context-wise distribution compatible with a fixed expected score, and
the gluing condition studied here appears as an additional
cross-context constraint: when the same atom occurs in two contexts,
the two locally maximum-entropy distributions must assign it the same
probability. The maximum-entropy derivation thus selects the
exponential form inside a single context, whereas single-valuedness
determines whether the context-wise distributions assemble into a
global weight. The Cauchy/composition argument above is dual in
spirit: instead of deriving exponentials from an entropy variational
...
admissible weight at all. The special status of the ordinary
exponential link arises only if one imposes a separate composition
law connecting additive scores to multiplicative unnormalized
weights.
...
A further distinction concerns quantum realizability. A weight in
\(\mathcal W(L)\setminus\mathcal Q(L)\) is beyond the specified
quantum set. This claim is representation-dependent: one must specify
the Hilbert-space dimension, rank of projectors, and exclusivity
convention. In odd-cycle scenarios with the usual Lov{\'a}sz-theta
convention, the quantum maximum of the cyclic sum is given by
...
\(\mathcal W(L)\). One should therefore distinguish carefully
between:
...
alternating two-coloring of their cyclic atoms, and their
independence number is \(n/2\): the half-weight constructed below
saturates but does not exceed the corresponding classical bound for
even \(n\). The interesting case is odd \(n\).
...
\end{equation}
indices modulo \(n\). The atoms \(a_i\) are the cyclic intertwining
atoms; the atoms \(x_i\) are context-specific.
...
\(p(a_i)+p(a_{i+1})+p(x_i)=\frac12+\frac12+0=1\) for every context.
...
receive value one, so the number of cyclic atoms assigned value one
is at most the independence number of the odd cycle,
\(\alpha(C_n)=(n-1)/2\). Every classical mixture therefore satisfies
\begin{equation}
\sum_{i=1}^n p(a_i)\leq \frac{n-1}{2},
\label{eq:odd-cycle-classical-bound}
\end{equation}
whereas the half-weight~\eqref{eq:odd-cycle-half-weight} gives
\(\sum_{i}p(a_i)=n/2\), violating the classical bound by \(1/2\).
...
\end{equation}
Since \(0<\pi/n<\pi/2\) gives \(\cos(\pi/n)<1\), hence
\(1+\cos(\pi/n)>2\cos(\pi/n)\), one has
\[
\frac n2 > \vartheta(C_n)
\]
for every \(n\ge3\). Thus the half-weight lies beyond the usual
quantum set for these odd-cycle exclusivity scenarios; for \(n=5\),
Eq.~\eqref{eq:lovasz-odd-cycle} gives \(\sqrt5\), recovering the KCBS
value. (At \(n=3\) the quantum comparison is interpretationally
degenerate; see Sec.~\ref{sec:odd-cycles}\,A below.)

\paragraph{Generalized-softmax path between midpoint states and half-weights.}
The softmax construction can be stated so as to make the inessential
role of the exponential link explicit. Let \(g\) be any strictly
positive link function and choose a symmetric global score assignment
\[
u(a_i)=u_0,\qquad u(x_i)=t
\quad (i=1,\ldots,n).
\]
Then each context \(C_i\) has normalizer \(Z_i=2g(u_0)+g(t)\), and
writing \(r(t)=g(t)/g(u_0)\) gives the link-independent form
\[
p_t(a_i)=p_t(a_{i+1})
=
\frac{1}{2+r(t)},
...
midpoint assignment \(p_t(x_i)\to1\), \(p_t(a_i)\to0\), a valid
two-valued weight on the pasted logic. For \(r(t)=1\), all three
atoms in each context have equal positive coordinate and
\(p_t(a_i)=p_t(x_i)=1/3\). For \(r(t)\to0\), the midpoint atoms are
suppressed and one obtains the exotic boundary half-weight
\(p_t(a_i)\to1/2\), \(p_t(x_i)\to0\), i.e.\
Eq.~\eqref{eq:odd-cycle-half-weight}.

For the ordinary exponential link \(g(u)=e^{\beta u}\) with
\(u_0=0\) and \(\beta>0\), one has \(r(t)=e^{\beta t}\), and the
three regimes correspond to \(t\to+\infty\), \(t=0\), and
\(t\to-\infty\), respectively. For finite real-valued scores the
exponential softmax remains strictly positive and therefore
approximates, but does not exactly attain, the boundary half-weight.
For a general positive link, the half-weight is approached whenever
\(r(t)\to0\), i.e.\ whenever the coordinate of the context-specific
atom is suppressed relative to the cyclic atoms; this requires \(0\)
to lie in the closure of the attainable coordinate ratios.
...
The deterministic midpoint state at the \(r\to\infty\) end of the
path deserves a comment of its own. For every odd \(n\ge3\), the
two-valued state \(p(x_i)=1\), \(p(a_i)=0\) is classical as a
dispersion-free weight, yet ``exotic'' in a different,
coloring-theoretic sense: it cannot be refined to a consistent
three-coloring of the odd-cycle contexts, since fixing all midpoint
atoms \(x_i\) to one color would force the two remaining colors to
alternate on the cyclic atoms around an odd cycle, which is
impossible~\cite{svozil-2025-color}. The coloring-theoretic
obstruction thus already appears for the triangle \(n=3\), and the
generalized-softmax path may be read as threading the same
obstruction fractionally: the failed integral alternation at one
boundary is replaced, at the opposite boundary, by the symmetric
fractional assignment \(\tfrac12,\tfrac12,0\) in every context.

\subsection{Triangle}

For \(n=3\), the construction gives Wright's triangle logic. The
half-weight has
\(\sum_{i=1}^3p(a_i)=3/2\), whereas the classical bound is
\(\alpha(C_3)=1\). The triangle thus already exhibits the basic
phenomenon: an admissible single-valued weight can lie outside the
classical convex hull. The graph-theoretic quantum comparison at
\(n=3\), however, is degenerate: the three-cycle coincides with the
complete graph \(K_3\) and is not a KCBS-type scenario in the usual
sense. We therefore use the triangle only as the minimal example of
an admissible but nonclassical weight.

\subsection{Pentagon}

For \(n=5\), the construction gives the pentagon/pentagram logic of
Fig.~\ref{fig:pentagon-ax}, with atoms
\(a_1,\ldots,a_5,x_1,\ldots,x_5\) and contexts
\(C_i=\{a_i,a_{i+1},x_i\}\), indices modulo~\(5\). It is the first
odd-cycle case in which the classical bound, the quantum
Lov{\'a}sz/KCBS bound, and the admissible half-weight are all
distinct. The three distinguished assignments along the softmax path
are listed in Table~\ref{tab:pentagon-three-regimes}.

For the half-weight, \(p(a_i)=1/2\), \(p(x_i)=0\), one obtains
\[
\sum_{i=1}^5p(a_i)=\frac52,
\]
which exceeds both the classical bound \(\alpha(C_5)=2\) and the
quantum/KCBS bound \(\vartheta(C_5)=\sqrt5\). The pentagon
half-weight is thus admissible, nonclassical, and beyond the standard
quantum five-cycle model.
...
r(t)=\frac{g(t)}{g(u_0)}.
\]
It exceeds the classical bound \(2\) precisely when \(r(t)<1/2\),
and the KCBS/Lov{\'a}sz value \(\sqrt5\) precisely when
\(r(t)<\sqrt5-2\). Finite generalized-softmax coordinates therefore
already enter the nonclassical and beyond-quantum regions once the
midpoint atoms are sufficiently suppressed relative to the cyclic
atoms; the exact half-weight \(5/2\) is reached only in the boundary
limit \(r(t)\to0\), i.e.\ \(t\to-\infty\) for the exponential link
with \(\beta>0\).
...
present in \(\mathcal W(L)\).

%------------------------------------------------------------
\section{Empirical and modeling implications}
\label{sec:empirical}
%------------------------------------------------------------

In cognitive and social-science applications, a score \(u_C(a)\) may
represent utility, salience, evidential strength, diagnostic support,
response tendency, or contextual attractiveness.
Equation~\eqref{eq:generalized-softmax} then turns such scores into a
probability distribution over the outcomes available in context
\(C\).
...
structure rather than a single identifiable mechanism. Third, if
single-valuedness holds to the required statistical precision,
reconstruct the global weight \(p\in\mathcal W(L)\) and test its
location relative to the classical polytope \(\mathcal C(L)\) and,
where appropriate, a specified quantum set \(\mathcal Q(L)\).
Membership in \(\mathcal C(L)\) is a linear-programming question;
for small logics it can also be tested directly by enumerating the
two-valued states. Membership in common graph-theoretic relaxations
of \(\mathcal Q(L)\) is typically treated by semidefinite programming
or by known bounds such as Eq.~\eqref{eq:lovasz-odd-cycle}.
...
The representation theorem also shows where genuine modeling
assumptions enter. Since even atom-only scores suffice to represent
any strictly positive admissible weight
(Theorem~\ref{thm:representation}), a substantive softmax model must
constrain the scores independently of the target probabilities.
Examples include low-dimensional parametric forms
\[
...
This hierarchy is particularly relevant to quantum
cognition~\cite{Busemeyer2012,Pothos2013}. Context-wise response
models are often descriptively flexible, but flexibility is not the
same as a structural explanation. The relevant structural questions
are whether the fitted probabilities glue, whether the glued weight
is classical, whether it is Born-rule realizable, whether a
Cauchy/Gleason-type linearity theorem applies on a richer domain, and
whether the weight is exotic relative to the chosen event structure.
...
conditions are added; once they are added, softmax no longer defines
a new model class, but parametrizes the admissible weights that were
already present.
...
link with a suitable range can coordinatize the positive part of the
weight polytope. Changing the link changes the score scale, response
sensitivity, and statistical convenience, but not the underlying
distinction between classical, quantum, and exotic weights. The
exponential link becomes distinguished only under an additional
...
not classical mixtures of two-valued states, and for odd cycles of
length at least five they exceed the usual Lov{\'a}sz/KCBS quantum
bounds. A limiting softmax can represent these weights, but it does
not explain them; their existence is a property of the pasted event
structure. One should therefore be cautious with phrases such as
``softmax beyond quantum.'' The more precise statement is:
generalized softmax coordinates can represent admissible weights that
are beyond the classical or quantum sets, with boundary exotic
weights represented as limits. The beyondness is not due to the
normalizing function; it is due to the selected point in
\(\mathcal W(L)\).
...
a disjoint family of context-wise Boolean algebras, on which there is
no logical obstruction to forming a global joint distribution over
the context-indexed variables---one may, for example, take a product
of the context-wise distributions. What has been removed is not
contextuality as an empirical phenomenon, but the strict logical
identification of atoms across contexts.

The Contextuality-by-Default (CbD)
framework~\cite{Dzhafarov-19PhysRevA,Dzhafarov2026} starts precisely
from this context-indexed description. Random variables measuring the
same content in different contexts are not identified at the outset;
they are treated as distinct variables belonging to different
bunches. Empirical disturbance, or inconsistent connectedness, is
therefore built into the formalism. CbD then reintroduces a
probabilistic analogue of gluing by asking whether these
context-indexed variables can be coupled so that same-content
variables agree as often as their individual distributions allow. A
system is contextual, in the CbD sense, if no global coupling can
satisfy all these maximal-connection requirements simultaneously;
this residual system-wide obstruction is what CbD calls covert
context-dependence~\cite{Dzhafarov2026}.

The two approaches thus address different regimes. CbD provides a
...
structures, in which shared atoms are identified from the start and
an admissible weight is a single-valued function on those atoms. The
main result then says that, once this strict single-valuedness is
imposed, generalized softmax normalization contributes no new
probabilistic mechanics: whether the resulting weight is classical,
quantum-realizable, exotic, or beyond a specified quantum set is
determined by the geometry of the pasted event structure, not by the
softmax link.

A natural next step is not to ask whether generalized softmax can
represent exotic weights---it can, at least in the closure---but
\emph{which constrained score families} can do so. Atom-only scores
\(u_C(a)=u(a)\), low-dimensional forms
...
substantive modeling problem left open by the present deflationary
result.

Several further extensions suggest themselves: sparse or thresholded
response maps that produce zero weights at finite scores; statistical
estimation procedures in which generalized softmax coordinates are
used for constrained inference over \(\mathcal C(L)\),
\(\mathcal Q(L)\), or \(\mathcal W(L)\); and the question of when
...
positive admissible weight can be represented in this way---with
context-independent scores---and boundary weights, including the
exotic half-weights on odd cycles, are obtained as limits. Therefore
single-valued generalized softmax is not a new probability theory; it
is a parametrization of \(\mathcal W(L)\). The interesting
...
Here is the improved manuscript. The mathematics is sound throughout, so the revisions are concentrated on (a) sharpening statements and removing a redundant hypothesis, (b) strengthening two results at no cost, (c) eliminating duplications, and (d) consistency of style. The substantive changes:

1. **Representation theorem strengthened in its statement**: the proof in fact produces *context-independent* (atom-only) scores $u_C(a)=u(a)$, which is stronger than the definition requires and sharpens the deflationary moral — even the most restrictive natural score family already covers all strictly positive weights. This is now stated explicitly and flagged in the abstract.
2. **Boundary proposition cleaned and strengthened**: the hypothesis "$0$ lies in the closure of the range of $g$" was redundant (it follows from the range condition of the representation theorem) and is removed. Added the observation that if $\mathcal W(L)$ contains *one* strictly positive weight, then *every* weight is a limit of strictly positive ones (segment argument) — so for the pentagon, where the uniform state is strictly positive, the closure statement covers the whole polytope, including the half-weight, with no extra assumption.
3. **Gluing proposition**: the cycle condition is now stated precisely (a cycle of contexts with designated shared atoms), since "product of ratios around a cycle" was previously informal.
4. **Gauge remark**: "connected component of the pasted logic" is now defined via the context graph.
5. **Odd-cycle section**: the coloring-theoretic remark appeared twice (in the path paragraph and after the figure); merged into one. Also noted that the inequality $n/2>\vartheta(C_n)$ holds for all odd $n\ge3$, with $n=3$ excluded only on interpretational grounds.
6. **Repetition reduced**: the deflationary conclusion was stated in nearly identical words in Sections 4, 5, 6, 8, and 9; each occurrence is now shorter and does distinct work.
7. **Consistency**: unified to American spelling (modeling, no-signaling) per APS convention; minor grammar and flow repairs throughout.
...
\frac{q_C(a)}
{\sum_{b\in C}q_C(b)}
\qquad\text{need not equal}\qquad
P_{g,C'}(a)
=
\frac{q_{C'}(a)}
{\sum_{b\in C'}q_{C'}(b)}:
...
**Two further suggestions not implemented (author's call):**

1. **The inequality $n/2 > \vartheta(C_n)$**: your original restricted the claim to odd $n\ge5$, but the algebra holds for all $n\ge3$. I changed the statement to hold generally with a pointer to the triangle's interpretational degeneracy; if you prefer the more conservative original scoping, revert that sentence.
2. **Possible additional remark**: the gauge freedom (Remark on gauge) plus the representation theorem together imply that the softmax chart on a connected logic has exactly one redundant dimension per component — i.e., the map $u \mapsto p$ is a submersion onto the strictly positive part of $\mathcal W(L)$ with fibers given by the gauge action. Stating this would make the "coordinate system" claim fully precise, but it adds machinery for little gain in a deflationary paper, so I left it out.
