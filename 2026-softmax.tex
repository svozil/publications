...
usual Lovasz/KCBS quantum value~\cite{lovasz-79,Klyachko-2008,
...
formulations, related relaxations are governed by Lovasz-theta-type
...
specified graph-theoretic quantum set, such as the Lovasz-theta set
...
odd-cycle scenarios with the usual Lovasz-theta convention, the
...
The half-weight is obtained as a limiting ordinary softmax by setting
\[
u(a_i)=0,\qquad u(x_i)=-M.
\]
Then
\[
P_M(a_i)=
...
\qquad
P_M(x_i)=
\frac{e^{-\beta M}}{2+e^{-\beta M}},
\]
and \(M\to\infty\) yields
Eq.~\eqref{eq:odd-cycle-half-weight}. For a general positive link
\(g\), the same limiting construction is obtained by choosing scores
such that
\[
...
\]
whereas the classical bound is \(1\). The graph-theoretic comparison
at \(n=3\) is degenerate: the three-cycle is also the complete graph
\(K_3\), and it is not a KCBS-type scenario in the usual sense. We
therefore use the triangle only as the minimal example of an
admissible but nonclassical weight. The pentagon \(n=5\) is the first
nontrivial case in which the classical bound, the quantum Lovasz/KCBS
bound, and the admissible half-weight are all distinct.

\subsection{Pentagon}

For \(n=5\), the construction gives the pentagon/pentagram logic.
The half-weight has
\[
\sum_{i=1}^5p(a_i)=\frac52,
\]
whereas the classical bound is \(2\) and the usual quantum/KCBS bound
is
\[
\vartheta(C_5)=\sqrt5.
\]
Thus the pentagon half-weight is admissible, nonclassical, and beyond
the standard quantum five-cycle model. It is nevertheless obtained as
a limiting softmax assignment. The softmax representation therefore
does not explain the beyond-quantum character of the weight; it only
parametrizes, in the closure of the positive softmax image, a point
already present in \(\mathcal W(L)\).
...
at least five, the same assignments exceed the usual Lovasz/KCBS
...
Lovasz/KCBS quantum bound. They are obtained as limiting softmax
