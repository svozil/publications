...
%------------------------------------------------------------
Appendix~\ref{app:lean-verification} records a Lean~4/mathlib formalization of the finite representation, gluing, and pentagon half-weight claims. The verified artifacts are included verbatim so that the computational proof certificate can be inspected independently of the surrounding exposition.
...
\section{Lean verification artifacts}
\label{app:lean-verification}

This appendix records the Lean 4 formalization used to verify the finite claims about gluing, generalized-softmax coordinates, and the pentagon half-weight. The project was checked with Lean \texttt{v4.30.0} and mathlib \texttt{v4.30.0}; the command \texttt{lake build} completed successfully, and the source contains no \texttt{sorry}, \texttt{admit}, or \texttt{axiom}.

The formalization separates three layers. First, Appendix~\ref{app:lean-core} defines finite event structures, admissible weights, positive coordinates, and the representation theorem for scaled link-function preimages. Second, Appendix~\ref{app:lean-gluing} formalizes local context-wise distributions, single-valuedness, the normalizer-ratio calculation, and the theorem that single-valued local distributions glue to a global admissible weight. Third, Appendix~\ref{app:lean-pentagon} formalizes the pentagon example: the half-weight is admissible, every two-valued state and every finite classical mixture obeys the cyclic bound \(2\), the half-weight has cyclic sum \(5/2\), and \(5/2>\sqrt 5\). The same file also verifies the positive softmax-coordinate path and its boundary endpoint at the half-weight.

\begin{table}[h]
\caption{Lean files included in the verification appendix.}
\label{tab:lean-files}
\begin{ruledtabular}
\begin{tabular}{ll}
File & Main content \\
\colrule
\texttt{lean-toolchain} & Lean compiler pin \\
\texttt{lakefile.lean} & mathlib dependency pin \\
\texttt{SoftmaxVerification.lean} & project import root \\
\texttt{Core.lean} & event structures and representation theorem \\
\texttt{Gluing.lean} & gluing and softmax-to-weight theorem \\
\texttt{Pentagon.lean} & pentagon half-weight and bounds \\
\end{tabular}
\end{ruledtabular}
\end{table}

\subsection{Project files}
\label{app:lean-project}

\paragraph{\texttt{lean-toolchain}.} Lean compiler pin.
\begin{LeanCode}
leanprover/lean4:v4.30.0
\end{LeanCode}

\paragraph{\texttt{lakefile.lean}.} Lake project file and mathlib dependency.
\begin{LeanCode}
import Lake
open Lake DSL
...
\paragraph{\texttt{SoftmaxVerification.lean}.} Root Lean import file.
\begin{LeanCode}
...
\subsection{Core representation theorem}
\label{app:lean-core}

\paragraph{\texttt{SoftmaxVerification/Core.lean}.} Finite event structures, admissible weights, positive coordinates, and the link-function representation theorem.
\begin{LeanCode}
import Mathlib
...
structure EventStructure (Atom : Type*) [DecidableEq Atom] where
  ...
  context_subset : forall C, C \ensuremath{\in} contexts -> C \ensuremath{\subseteq} atoms
...
structure IsWeight {Atom : Type*} [DecidableEq Atom]
  ...
  nonneg : forall a, a \ensuremath{\in} L.atoms -> 0 <= p a
  normalized : forall C, C \ensuremath{\in} L.contexts -> Finset.sum C p = 1
...
def Represents {Atom : Type*} [DecidableEq Atom]
  ...
  (forall a, a \ensuremath{\in} L.atoms -> 0 < q a) \ensuremath{\wedge}
    forall C, C \ensuremath{\in} L.contexts -> forall a, a \ensuremath{\in} C ->
...
theorem positive_weight_represents_itself {Atom : Type*} [DecidableEq Atom]
    ...
    Represents L p p := by
  constructor
  \ensuremath{\cdot} exact hpos
  \ensuremath{\cdot} intro C hC a ha
...
theorem link_representation_of_scaled_range
    ...
    (hp : IsWeight L p)
    {alpha : Real} (halpha : 0 < alpha)
    (hpre : forall a, a \ensuremath{\in} L.atoms -> exists u : Score, g u = alpha * p a) :
    exists u : Atom -> Score,
      forall C, C \ensuremath{\in} L.contexts -> forall a, a \ensuremath{\in} C ->
        normalizedCoordinate (fun b => g (u b)) C a = p a := by
  classical
  let u : Atom -> Score := fun a =>
    if ha : a \ensuremath{\in} L.atoms then
      Classical.choose (hpre a ha)
    else
      Classical.choice (inferInstance : Nonempty Score)
  refine \ensuremath{\langle}u, ?_\ensuremath{\rangle}
  intro C hC a ha
  have hCsub : C \ensuremath{\subseteq} L.atoms := L.context_subset C hC
  have hu : forall b, b \ensuremath{\in} C -> g (u b) = alpha * p b := by
    intro b hb
    have hb_atoms : b \ensuremath{\in} L.atoms := hCsub hb
    dsimp [u]
    rw [dif_pos hb_atoms]
...
\subsection{Gluing of local distributions}
\label{app:lean-gluing}

\paragraph{\texttt{SoftmaxVerification/Gluing.lean}.} Single-valued local distributions and their glued admissible weight.
\begin{LeanCode}
import SoftmaxVerification.Core
...
def LocalNonnegative {Atom : Type*} [DecidableEq Atom]
  ...
  forall C, C \ensuremath{\in} L.contexts -> forall a, a \ensuremath{\in} C -> 0 <= P C a

/-- A context-indexed family of local probabilities is normalized in
each context. -/
def LocalNormalized {Atom : Type*} [DecidableEq Atom]
    (L : EventStructure Atom) (P : Finset Atom -> Atom -> Real) : Prop :=
  forall C, C \ensuremath{\in} L.contexts -> Finset.sum C (P C) = 1
...
def SingleValuedOn {Atom : Type*} [DecidableEq Atom]
    (L : EventStructure Atom) (P : Finset Atom -> Atom -> Real) : Prop :=
  forall C, C \ensuremath{\in} L.contexts -> forall D, D \ensuremath{\in} L.contexts -> forall a,
    a \ensuremath{\in} C -> a \ensuremath{\in} D -> P C a = P D a
...
theorem gluing_condition_for_free_scores {Atom : Type*} [DecidableEq Atom]
    ...
    SingleValuedOn L (localCoordinate q) <->
      forall C, C \ensuremath{\in} L.contexts -> forall D, D \ensuremath{\in} L.contexts -> forall a,
        a \ensuremath{\in} C -> a \ensuremath{\in} D ->
...
theorem normalized_eq_iff_ratio
    {qC qD ZC ZD : Real}
    (hqD : qD \ensuremath{\ne} 0) (hZC : ZC \ensuremath{\ne} 0) (hZD : ZD \ensuremath{\ne} 0) :
    qC / ZC = qD / ZD <-> ZC / ZD = qC / qD := by
  constructor
  \ensuremath{\cdot} intro h
...
noncomputable def gluedWeight {Atom : Type*} [DecidableEq Atom]
    ...
    Atom -> Real :=
  fun a =>
    if h : exists C, C \ensuremath{\in} L.contexts \ensuremath{\wedge} a \ensuremath{\in} C then
...
theorem gluedWeight_eq_of_mem {Atom : Type*} [DecidableEq Atom]
    ...
    {C : Finset Atom} (hC : C \ensuremath{\in} L.contexts) {a : Atom} (ha : a \ensuremath{\in} C) :
    gluedWeight L P a = P C a := by
  classical
  unfold gluedWeight
  have hex : exists D, D \ensuremath{\in} L.contexts \ensuremath{\wedge} a \ensuremath{\in} D := \ensuremath{\langle}C, hC, ha\ensuremath{\rangle}
  rw [dif_pos hex]
...
theorem local_distributions_glue_to_weight {Atom : Type*} [DecidableEq Atom]
    ...
    (hsingle : SingleValuedOn L P) :
    exists p : Atom -> Real,
      IsWeight L p \ensuremath{\wedge}
        forall C, C \ensuremath{\in} L.contexts -> forall a, a \ensuremath{\in} C -> p a = P C a := by
  classical
  refine \ensuremath{\langle}gluedWeight L P, ?_, ?_\ensuremath{\rangle}
  \ensuremath{\cdot} constructor
    \ensuremath{\cdot} intro a ha
      unfold gluedWeight
      by_cases hex : exists C, C \ensuremath{\in} L.contexts \ensuremath{\wedge} a \ensuremath{\in} C
      \ensuremath{\cdot} rw [dif_pos hex]
        have hchoose := Classical.choose_spec hex
        exact hnonneg (Classical.choose hex) hchoose.1 a hchoose.2
      \ensuremath{\cdot} rw [dif_neg hex]
    \ensuremath{\cdot} intro C hC
      calc
...
\subsection{Pentagon half-weight}
\label{app:lean-pentagon}

\paragraph{\texttt{SoftmaxVerification/Pentagon.lean}.} Pentagon logic, classical cyclic bound, KCBS numerical comparison, and the positive softmax path.
\begin{LeanCode}
import SoftmaxVerification.Gluing
...
def IsTwoValued {Atom : Type*} [DecidableEq Atom]
  ...
  IsWeight L v \ensuremath{\wedge} forall a, a \ensuremath{\in} L.atoms -> v a = 0 \ensuremath{\vee} v a = 1

private theorem pentagon_adjacent_sum_le_one
    {v : PentagonAtom -> Real} (hv : IsTwoValued pentagon v) :
    v a0 + v a1 <= 1 \ensuremath{\wedge}
    v a1 + v a2 <= 1 \ensuremath{\wedge}
    v a2 + v a3 <= 1 \ensuremath{\wedge}
    v a3 + v a4 <= 1 \ensuremath{\wedge}
    v a4 + v a0 <= 1 := by
...
theorem pentagon_twoValued_cyclicSum_le_two
  ...
  rcases hadj with \ensuremath{\langle}h01, h12, h23, h34, h40\ensuremath{\rangle}
...
theorem pentagon_classical_mixture_cyclicSum_le_two
    {\ensuremath{\iota} : Type*} [Fintype \ensuremath{\iota}]
    {lambda : \ensuremath{\iota} -> Real} {v : \ensuremath{\iota} -> PentagonAtom -> Real}
    {p : PentagonAtom -> Real}
  ...
  calc
    Finset.sum Finset.univ (fun i => lambda i * pentagonCyclicSum (v i))
      ...
      exact mul_le_mul_of_nonneg_left (pentagon_twoValued_cyclicSum_le_two (hv i))
        (hlambda_nonneg i)
    _ = 2 := by
      rw [\ensuremath{\leftarrow} Finset.sum_mul]
      rw [hlambda_sum]
      ring
...
theorem pentagon_pathWeight_isWeight {r : Real} (hr : 0 <= r) :
  ...
  \ensuremath{\cdot} intro a ha
    ...
    \ensuremath{\cdot} exact div_nonneg zero_le_one hden_pos.le
    \ensuremath{\cdot} exact div_nonneg zero_le_one hden_pos.le
    \ensuremath{\cdot} exact div_nonneg zero_le_one hden_pos.le
    \ensuremath{\cdot} exact div_nonneg zero_le_one hden_pos.le
    \ensuremath{\cdot} exact div_nonneg zero_le_one hden_pos.le
    \ensuremath{\cdot} exact div_nonneg hr hden_pos.le
    \ensuremath{\cdot} exact div_nonneg hr hden_pos.le
    \ensuremath{\cdot} exact div_nonneg hr hden_pos.le
    \ensuremath{\cdot} exact div_nonneg hr hden_pos.le
    \ensuremath{\cdot} exact div_nonneg hr hden_pos.le
...
theorem pentagon_path_exceeds_kcbs_of_small {r : Real}
  ...
  have hsq : Real.sqrt 5 * Real.sqrt 5 = 5 := by
    rw [\ensuremath{\leftarrow} pow_two, Real.sq_sqrt]
    norm_num
  rw [pentagon_pathWeight_cyclicSum]
  field_simp [ne_of_gt hden_pos]
  nlinarith
...
theorem pentagon_pathWeight_strictlyPositive {r : Real} (hr : 0 < r) :
  ...
  have hden_pos : 0 < 2 + r := by linarith
  cases a <;> dsimp [pentagonPathWeight]
  \ensuremath{\cdot} exact div_pos zero_lt_one hden_pos
  \ensuremath{\cdot} exact div_pos zero_lt_one hden_pos
  \ensuremath{\cdot} exact div_pos zero_lt_one hden_pos
  \ensuremath{\cdot} exact div_pos zero_lt_one hden_pos
  \ensuremath{\cdot} exact div_pos zero_lt_one hden_pos
  \ensuremath{\cdot} exact div_pos hr hden_pos
  \ensuremath{\cdot} exact div_pos hr hden_pos
  \ensuremath{\cdot} exact div_pos hr hden_pos
  \ensuremath{\cdot} exact div_pos hr hden_pos
  \ensuremath{\cdot} exact div_pos hr hden_pos
...
theorem pentagon_halfWeight_not_strictlyPositive :
    \ensuremath{\neg} StrictlyPositive pentagon pentagonHalfWeight := by
  intro hpos
