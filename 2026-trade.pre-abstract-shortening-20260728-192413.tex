...
\begin{abstract}
Every universal-trading claim places a trader on one side and a market---a
path, generator, or probability law---on the other. Can one algorithm make
money on every computable price path? No. For any total deterministic
computable trader, its code determines a total computable countermarket:
simulate its next position and generate a bounded proportional price move with
the opposite sign. The resulting fixed path defeats that trader; no market
need watch it in real time.
A separate Halting-Problem reduction rules out a uniform computable decision
procedure for unbounded-horizon events across arbitrary total computable market
generators when the admissible language is Turing-universal. Here
``universal'' means Turing-universal; following the Church--Turing thesis,
Turing computation is used as the formal model of effective algorithmic
computation. This is not an empirical assertion that actual exchanges are
computationally universal. Busy-Beaver growth shows why no computable waiting
horizon can remove that obstruction.
For a passive record fixed independently of the trader, Martin-L\"of
randomness connects effective statistical tests, algorithmic incompressibility,
and the failure of effective nonnegative test-capital processes---including
every total computable fair-game martingale---to become unbounded. No-arbitrage
supplies a distinct financial boundary.
Repeatable success must therefore exploit a restriction, distribution,
benchmark, risk premium, or informational advantage. Time reversal is one
simple computable stress test of the claimed restriction, not an additional
