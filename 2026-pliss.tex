...
\title{Partition Logics in Social Science:\\
Complementarity Without Contextuality in Human Systems}
...
\begin{abstract}
Partition logics---nonboolean structures pasted from Boolean
subalgebras---provide a natural formalism for situations in which a
system with fully determinate properties can be observed only through
one of several incompatible modes of inquiry, each resolving different
aspects of the underlying state space. We present six detailed
scenarios from the social sciences---personnel assessment, public
opinion polling, clinical diagnosis, espionage coordination, legal
pluralism, and organizational auditing---that instantiate partition
logics ranging from the simple $L_{12}$ bowtie to the pentagon and
automaton logics. For each scenario we explain in concrete terms what
the elements of the finite set represent (e.g.\ applicant types,
respondent profiles, legal actions), what the partitions correspond to
(e.g.\ test instruments, survey frames, legal codes), and precisely
how ``intertwining'' arises when two distinct observational contexts
happen to make the same coarse-grained distinction. These examples
demonstrate that complementarity---a hallmark of quantum
mechanics---arises naturally in classical social systems, and that it
does not, by itself, entail contextuality or value indefiniteness. We
discuss the dual probabilistic interpretation available on these
structures and connect it to the emerging program of quantum
cognition.
\end{abstract}
...
pasted together at shared elements---turns out to appear naturally in
...
finite set of underlying types ${S}_n = \{1,2,\ldots,n\}$.
Each ``context'' or ``block'' is a partition of ${S}_n$ into
...
\begin{enumerate}
\item What are the \textbf{types} (elements of ${S}_n$), and
...
defined.
The first is the ``natural'' classical one, in which
...
There may exist more than two probability theories satisfying Kolmogorovian axioms on bloks aka classical mini-universes;
in particular,
...
\begin{itemize}
\item A \emph{latent state} $s \in S$ is one exact underlying
  possibility (ontology).
\item A \emph{latent social profile} is the substantive
...
in both partitions.
(They may, in principle, share more than just one atom corresponding to more than just one latent states.)
...
(often also referred to as \emph{Greechie orthogonality diagram}), each context is drawn as a smooth line and
...
\subsection{Two probability theories on the same structure}
\label{sec:twoprob}

A \emph{dispersion-free state} (or two-valued weight) is a function
$v: \text{atoms} \to \{0,1\}$ that assigns~$1$ to exactly one atom
per context. Intuitively, it says ``the system \emph{is} this type
...
${S}_n$. The resulting probabilities on atoms are convex
...
polytope} $VP(G)$ of the exclusivity
...
general \emph{strictly contains} $VP(G)$: there exist Born-rule
...
probabilities from $VP(G)$) \emph{or} by a quantum resource (yielding
...
In what follows, we develop several concrete instances and enactments of the formal scenario involving **partition logics** introduced earlier.
...
dimensions.
\end{itemize}
Thus ${S}_4 = \{1,2,3,4\}$, and every applicant \emph{is}
...
recognisable by adequate test performance), the pair
...
${S}_{11}=\{1,2,\ldots,11\}$. Each profile specifies a
...
Formally, the five cyclically intertwined contexts/cliques forming a pentagon/pentagram logic~\cite[p.~267, Fig.~2]{wright:pent}
supporting 11 dispersion free states. Constructing 5 contexts/cliques from the occurrences of the dispersion free value $1$ on the respective 10 atoms
...
\subsection{The exotic weight: clinical interpretation}

Wright showed~\cite{wright:pent} (see also Ref.~\cite[Fig.~V]{greechie-1974}) that the triangle logic supports,
in addition to its four dispersion-free weights (one per latent
state), an ``exotic'' probability weight that assigns
$\frac{1}{2}$ to each intertwining vertex. This weight lies
\emph{outside} the classical convex hull: it cannot be expressed
as any mixture of the four dispersion-free states.

What would this mean clinically? It would represent a probability
...
of the code (0 = clean, 1 = fabricated).
...
\emph{classical} (probabilities lie within $VP(G)$). The scenario
...
contexts:
\begin{itemize}
\item \textbf{Statutory (environmental) law ($C_{\rm stat}$):} Focuses
...
\{a_5,a_6\}\}$.
\item \textbf{Customary (indigenous) law ($C_{\rm cust}$):} Focuses on
...
\item \textbf{Religious law ($C_{\rm rel}$):} Focuses on stewardship
...
$C_{\rm stat}$ and $C_{\rm rel}$: both statutory and religious law agree
...
\item $\{a_4,a_6\}$ is an intertwining atom of $C_{\rm cust}$ and
$C_{\rm rel}$: both customary and religious law place commercial
...
\begin{figure}[t]
\centering
% $$$ Please draw a Tikz picture with the two intertwining atoms {1,2} and {4,6}, along the following lines and three contexts Cs, Cc, Cr
\begin{tikzpicture}[scale=0.90, transform shape, every path/.style={line width=1pt}]
...
\begin{figure}[t]
\centering
% $$$ Tikz chinese lantern picture with just two separated 2-atomic blocks    $C_a = \{\{s_1,s_2\},\{s_3,s_4\}\}$ and $C_b = \{\{s_1,s_3\},\{s_2,s_4\}\}$
\begin{tikzpicture}[scale=0.90, transform shape, every path/.style={line width=1pt}]
...
by a Wright urn (yielding $VP(G)$) or by a quantum system (yielding
...
We have demonstrated that partition logics---nonboolean structures
